...
ForgeCAD 是一个面向 3D 打印功能件的本地优先 AI CAD / DFM 桌面 Agent。它把自然语言、草图、参考图和真实尺寸转换为可编辑、可验证、可追溯的参数化设计，并输出 STEP、3MF、STL、GLB 与 DFM 报告。
...
| CAD 工作台 | 未实现 | 替换旧 3D 展台和 Patch 工作台 |

当前可运行代码仍是旧产品基线。运行和验证方法见 [操作手册](docs/OPERATIONS.md)。
...
> 设计一个 L 型传感器安装支架。底板 100 × 60 mm，立板高 70 mm，厚度 5 mm，底板有四个 M4 通孔，孔距 80 × 40 mm。材料 PETG，0.4 mm 喷嘴，打印空间 256 × 256 × 256 mm。

系统必须产出 DesignSpec、FeatureGraph、有效 B-Rep、STEP/3MF/STL/GLB、关键尺寸验证、DFM 报告和版本记录。随后接受修改：

> 底板加厚到 7 mm，两个方向孔距保持不变，再增加两个三角加强筋。

修改必须先生成 ChangeSet，保护锁定孔位，创建子版本，重建并重跑 DFM，不能覆盖父版本。
