...
CREATE TABLE agent_material_texture_objects_m108 (
  texture_asset_id TEXT PRIMARY KEY
    CHECK (texture_asset_id GLOB 'asset_tex_[0-9a-f]*'),
  texture_role TEXT NOT NULL
    CHECK (texture_role IN ('base_color', 'metallic_roughness', 'normal', 'occlusion', 'emissive', 'thumbnail')),
...
INSERT INTO agent_material_texture_objects_m108 (
  texture_asset_id, texture_role, display_name, mime_type, byte_size, sha256,
  object_path, width, height, source, license, license_ref,
...
DROP TABLE agent_material_texture_objects;
ALTER TABLE agent_material_texture_objects_m108 RENAME TO agent_material_texture_objects;

CREATE UNIQUE INDEX idx_agent_material_texture_sha_role
  ON agent_material_texture_objects(sha256, texture_role);
CREATE INDEX idx_agent_material_texture_catalog
  ON agent_material_texture_objects(texture_role, source, updated_at DESC);

INSERT OR IGNORE INTO schema_migrations(version, name)
VALUES ('0034', 'expand_material_texture_roles');
