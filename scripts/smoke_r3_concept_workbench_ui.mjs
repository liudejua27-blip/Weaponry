...
async function runWorkbenchUi(baseUrl, agentApiBaseUrl, seeded) {
  ...
  try {
    ...
    await page.getByRole('button', { name: '截面', exact: true }).click()
    await page.getByTestId('section-overlay').waitFor()
    await page.getByLabel('截面偏移').fill('35')
    await page.locator('.weapon-viewport[data-section="enabled"][data-section-offset="35"]').waitFor()
    await page.getByRole('button', { name: '选择', exact: true }).click()
    await page.locator('.weapon-viewport[data-section="disabled"]').waitFor()
