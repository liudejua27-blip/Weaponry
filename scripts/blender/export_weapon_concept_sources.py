...
def _prepare_output(output_root: Path, force: bool) -> None:
    ...
    if output_root.exists() and any(output_root.iterdir()):
        if not force:
            raise RuntimeError("edited export output is not empty; use --force deliberately")
        shutil.rmtree(output_root)
    output_root.mkdir(parents=True, exist_ok=True)


def _require_sources(source_root: Path) -> None:
    expected = {f"{module_id}.blend" for module_id in REQUIRED_MODULE_IDS}
    actual = {path.name for path in source_root.glob("*.blend")} if source_root.is_dir() else set()
    if actual != expected:
        raise RuntimeError(
...
def _export_open_source(output_root: Path, source_path: Path, expected_module_id: str):
    ...
    mesh_objects = sorted(
        (obj for obj in scene.objects if obj.type == "MESH" and mesh_pattern.fullmatch(obj.name)),
    ...
    if all_mesh_names != {obj.name for obj in mesh_objects}:
        raise RuntimeError(f"{module_id}: non-canonical mesh objects: {sorted(all_mesh_names)}")
    ...
    for obj in mesh_objects:
        ...
        if obj.modifiers:
            raise RuntimeError(f"{module_id}: apply modifiers before export: {obj.name}")
        if not _identity_transform(obj):
            raise RuntimeError(f"{module_id}: apply location/rotation/scale: {obj.name}")
...
def _metadata(scene):
    ...
    required = {"schema_version", "module_id", "category", "pack_id", "asset_id", "connectors"}
    if not isinstance(value, dict) or set(value) != required:
        raise RuntimeError("scene authoring metadata fields do not match the contract")
    if value["schema_version"] != AUTHORING_SCHEMA or not isinstance(value["connectors"], list):
...
def _connectors(scene, metadata):
    ...
    for connector_id in sorted(definitions):
        ...
        if any(abs(value - 1) > 1e-6 for value in obj.scale):
            raise RuntimeError(f"{metadata['module_id']}: apply Connector scale: {obj.name}")
        result.append(
            {
...
if __name__ == "__main__":
    try:
        ...
        print(json.dumps({"ok": False, "status": "edited_export_failed", "message": str(exc)}))
