...
def seed_agent_head(factory: Any, project_id: str, asset_version_id: str) -> None:
    ...
    with SQLiteUnitOfWork(factory) as unit:
        unit.agent_assets.add_version(
            ...
            assembly_graph_json=json.dumps(
...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="forgecad-q002-") as raw:
        ...
        status, _, body = asyncio.run(
            request(app, "GET", "/api/v1/projects/prj_q002_empty/active-design")
        )
        ...
        status, headers, agent_snapshot = asyncio.run(
            request(app, "GET", "/api/v1/projects/prj_q002_agent/active-design")
        )
        assert (
            status == 200
        ...
        assert (
            agent_snapshot["active_design"]["asset_version_id"] == "assetver_q002_agent"
        )
        assert count_rows(factory, "active_design_snapshots", "prj_q002_agent") == 1
        status, cors_headers, _ = asyncio.run(
            request(
                app,
                "GET",
        ...
        assert (
            status == 200
            and "etag" in cors_headers["access-control-expose-headers"].lower()
        )
        status, cors_headers, _ = asyncio.run(
            request(
                app,
                "OPTIONS",
                quality_path
                := "/api/v1/agent/asset-versions/assetver_q002_agent:quality",
                headers={
        ...
        assert (
            status == 200
            and "if-match" in cors_headers["access-control-allow-headers"].lower()
        )
        status, repeat_headers, repeated = asyncio.run(
            request(app, "GET", "/api/v1/projects/prj_q002_agent/active-design")
        )
        assert (
            status == 200
            and repeat_headers["etag"] == headers["etag"]
            and repeated == agent_snapshot
        )
        assert count_rows(factory, "active_design_snapshots", "prj_q002_agent") == 1

        status, navigation_headers, navigation = asyncio.run(
            request(
                app, "GET", "/api/v1/projects/prj_q002_agent/active-design:navigation"
            )
        )
        assert (
            status == 200
            and navigation_headers["cache-control"] == "no-store"
            and "etag" not in navigation_headers
        )
        assert navigation["active_asset_version_id"] == "assetver_q002_agent"

        status, legacy_headers, legacy_snapshot = asyncio.run(
            request(app, "GET", "/api/v1/projects/prj_q002_legacy/active-design")
        )
        ...
        status, _, body = asyncio.run(
            request(app, "POST", quality_path, headers={"If-Match": headers["etag"]})
        )
        ...
        status, _, body = asyncio.run(
            request(
                ...
                "POST",
                quality_path,
                headers={"Idempotency-Key": "q002-missing-revision"},
            )
        )
        ...
        quality_headers = {
            ...
            "Idempotency-Key": "q002-quality",
        }
        status, _, quality = asyncio.run(
            request(app, "POST", quality_path, headers=quality_headers)
        )
        ...
        status, _, body = asyncio.run(
            request(
                ...
                "POST",
                quality_path,
                headers={"If-Match": headers["etag"], "Idempotency-Key": "q002-stale"},
            )
        )
    ...
    print(
        "Q002 active-design contract smoke passed: bootstrap compatibility, no-store navigation, quality replay/conflict and stale CAS"
    )
    return 0
