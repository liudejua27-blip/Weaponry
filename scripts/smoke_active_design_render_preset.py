...
from smoke_active_design_api import _error_code, _request, _seed_agent_head, _seed_legacy_current, _seed_project
...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="forgecad-r001-render-preset-") as raw:
        ...
        _seed_agent_head(factory, "prj_r001_agent", "assetver_r001_agent", "mg_r001_agent", "part_r001_body")
        _seed_legacy_current(factory, "prj_r001_legacy")

        status, headers, initial = asyncio.run(_request(app, "GET", "/api/v1/projects/prj_r001_agent/active-design"))
        ...
        fingerprint = hashlib.sha256(_canonical(initial["render_preset"]).encode("utf-8")).hexdigest()
        assert fingerprint == hashlib.sha256(_canonical(initial["render_preset"]).encode("utf-8")).hexdigest()
        ...
        status, headers, updated = asyncio.run(
            _request(app, "POST", "/api/v1/projects/prj_r001_agent/active-design:render-preset", payload=change, headers=change_headers)
        )
        ...
        assert status == 200 and headers["etag"] == 'W/"active-design-2"' and replay == updated

        conflict = {**change, "camera_view": "top"}
        status, _, error = asyncio.run(
            _request(app, "POST", "/api/v1/projects/prj_r001_agent/active-design:render-preset", payload=conflict, headers=change_headers)
        )
        assert status == 409 and _error_code(error) == "IDEMPOTENCY_CONFLICT"

        stale = {**change, "client_request_id": "r001-preset-stale", "snapshot_revision": 1, "camera_view": "right"}
        status, _, error = asyncio.run(
            _request(
                app,
                "POST",
        ...
        status, _, error = asyncio.run(
            _request(
                ...
                payload={"client_request_id": "r001-legacy", "snapshot_revision": 1, "camera_view": "front", "light_preset": "soft_studio"},
        ...
        mismatched["render_preset"] = {**mismatched["render_preset"], "asset_version_id": "assetver_other"}
        try:
