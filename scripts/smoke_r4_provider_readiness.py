...
def main() -> int:
    ...
    _assert(report["ready_for_live_evaluation"] is False, "clean env unexpectedly ready")
    _assert(report["provider"]["type"] == "deterministic", "clean env provider mismatch")
    ...
    _assert("credential" in _json(missing)["missing"], "missing credential not reported")
    print(json.dumps({"ok": True, "network_calls_made": 0, "redaction_verified": True}, ensure_ascii=False, indent=2))
    return 0
