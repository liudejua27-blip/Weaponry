...
def _seed_render_agent(factory: Any, project_id: str) -> str:
    ...
    graph = {
        ...
        "connections": [
            {
                "connection_id": "conn_r002_body_cabin",
    ...
    program = {
        ...
        "outputs": [
            {
                "output_id": "output_r002_body",
                "operation_id": "op_r002_body",
                "kind": "mesh",
                "part_role": "body",
            },
            {
                "output_id": "output_r002_cabin",
                "operation_id": "op_r002_cabin",
                "kind": "mesh",
                "part_role": "cabin",
            },
...
def _assert_png(payload: bytes, width: int, height: int) -> str:
    ...
    stride = width * 4
    alphas = [
        decoded[row * (stride + 1) + 1 + column * 4 + 3]
...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="forgecad-r002-render-views-") as raw:
        ...
        status, _, snapshot = asyncio.run(
            _request(app, "GET", "/api/v1/projects/prj_r002_agent/active-design")
        )
        assert (
            status == 200
            and snapshot["active_design"]["asset_version_id"] == asset_version_id
        )
        ...
        assert (
            first["exploded_view_available"] is True
            and first["exploded_unavailable_reason"] is None
        )
        first_hashes = []
        ...
        unsafe_candidate = render_agent_views(
            ...
            height=128,
            exploded_parts=(
                ExplodedPartOffset(part_id="part_r002_body", offset=(0.0, 0.0, 0.0)),
            ),
        )
        ...
        assert (
            ...
            == f'attachment; filename="{asset_version_id}-concept-views.zip"'
        )
        with zipfile.ZipFile(io.BytesIO(package), "r") as bundle:
            ...
            package_schema = json.loads(
                (
                    ROOT
                    / "packages/concept-spec/schemas/agent-asset-render-package.schema.json"
            ...
            assert (
                manifest["non_engineering_notice"]
                == "concept_views_only_not_engineering_or_manufacturing_data"
            )
            assert [item["file_name"] for item in manifest["views"]] == expected_names[
                1:
            ]
            assert manifest["views"][-1]["part_ids"] == [
                "part_r002_body",
                "part_r002_cabin",
            ]
        ...
        repeat_status, repeat_headers, repeat_package = asyncio.run(
            _binary_request(app, package_url)
        )
        assert (
            repeat_status == 200
            and repeat_headers["x-forgecad-render-set-sha256"]
            == first["render_set_sha256"]
        )
        ...
        status, _, missing_fingerprint = asyncio.run(
            _request(
                app,
                "GET",
                f"/api/v1/agent/asset-versions/{asset_version_id}:render-package?width=128&height=128",
        ...
        status, _, legacy_error = asyncio.run(
            _request(
                app,
                "GET",
                "/api/v1/agent/asset-versions/assetver_missing:render?width=128&height=128",
            )
        )
        ...
        status, _, invalid = asyncio.run(
            _request(
                app,
                "GET",
                f"/api/v1/agent/asset-versions/{asset_version_id}:render?width=32&height=128",
            )
        )
        assert status == 422

    print(
        "R002/R003/R004 Agent PNG provenance, transparent alpha, deterministic exploded views and ZIP package smoke passed"
    )
    return 0
