...
runtime_manifest = (ROOT / "apps/desktop/src-tauri/crates/forgecad-runtime/Cargo.toml").read_text(encoding="utf-8")
...
assert 'test-render-worker-fallback = ["dep:forgecad-render-core"]' in runtime_manifest
assert 'test-geometry-worker-fallback = ["dep:forgecad-geometry-worker", "test-render-worker-fallback"]' in runtime_manifest
for symbol in (
...
assert '#[cfg(feature = "test-render-worker-fallback")]' in runtime_render_adapter
assert '#[cfg(any(test, feature = "test-render-worker-fallback"))]' not in runtime_render_adapter
assert 'forgecad_render_core::render_perspective_glb' in runtime_source
assert '#[cfg(feature = "test-render-worker-fallback")]' in runtime_source
assert '#[cfg(any(test, feature = "test-render-worker-fallback"))]' not in runtime_source
