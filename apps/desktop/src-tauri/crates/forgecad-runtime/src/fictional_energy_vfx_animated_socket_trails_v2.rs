//! Projection-v2-driven animated socket trails.
//!
//! This is an additive, bounded, structural-only producer.  V1 remains in
//! its own module.  Trails@2 consumes only the dual-candidate Particles@2 and
//! the durable Projection@2 chain, derives the local history and world-space
//! trail inventory itself, asks the sibling Worker to render it twice, and
//! only then reserves the five per-frame CAS objects represented by the
//! durable sequence contract.
...
use forgecad_contracts::{
    ...
    FictionalEnergyVfxAnimatedSocketTrailsSequenceV2FrameInput,
    FictionalEnergyVfxAnimatedSocketTrailsSequenceV2GetRequest,
    FictionalEnergyVfxAnimatedSocketTrailsSequenceV2PrepareRequest,
    FictionalEnergyVfxAnimatedSocketTrailsV2HistorySample,
    FictionalEnergyVfxAnimatedSocketTrailsV2Point, FictionalEnergyVfxAnimatedSocketTrailsV2Trail,
...
const PREPARE_SCHEMA: &str = "FictionalEnergyVfxAnimatedSocketTrailsSequencePrepareRequest@2";
const GET_SCHEMA: &str = "FictionalEnergyVfxAnimatedSocketTrailsSequenceGetRequest@2";
const PREPARE_RESULT_SCHEMA: &str = "FictionalEnergyVfxAnimatedSocketTrailsSequencePrepareResult@2";
const GET_RESULT_SCHEMA: &str = "FictionalEnergyVfxAnimatedSocketTrailsSequenceGetResult@2";
const SEQUENCE_SCHEMA: &str = "FictionalEnergyVfxAnimatedSocketTrailsSequence@2";
const FRAME_SCHEMA: &str = "FictionalEnergyVfxAnimatedSocketTrailsSequenceFrame@2";
const FRAME_RECEIPT_SCHEMA: &str = "FictionalEnergyVfxAnimatedSocketTrailsFrameReceipt@2";
const RENDER_SET_SCHEMA: &str = "FictionalEnergyVfxAnimatedSocketTrailsRenderSet@2";
...
const TRAILS_POLICY: &str = "projection-v2-driven-animated-socket-trails-dual-candidate@2";
...
#[cfg(test)]
thread_local! {
    // This is the complete public-get implementation count.  Bloom reuses
    // this exact path internally, so a Bloom source request still records one
    // public-get-equivalent read without dispatching back through Runtime.
    static TEST_TRAILS_GET_PATH_CALLS: Cell<usize> = Cell::new(0);
    static TEST_TRAILS_CONTEXT_BUILDS: Cell<usize> = Cell::new(0);
    static TEST_TRAILS_WORKER_CALLS: Cell<usize> = Cell::new(0);
}

#[cfg(test)]
pub(crate) fn reset_test_trails_counters() {
    TEST_TRAILS_GET_PATH_CALLS.with(|value| value.set(0));
    TEST_TRAILS_CONTEXT_BUILDS.with(|value| value.set(0));
    TEST_TRAILS_WORKER_CALLS.with(|value| value.set(0));
}

#[cfg(test)]
pub(crate) fn test_trails_counters() -> (usize, usize, usize) {
    (
        TEST_TRAILS_GET_PATH_CALLS.with(Cell::get),
        TEST_TRAILS_CONTEXT_BUILDS.with(Cell::get),
        TEST_TRAILS_WORKER_CALLS.with(Cell::get),
    )
}

#[cfg(test)]
pub(crate) fn take_test_trails_counters() -> (usize, usize, usize) {
    let counters = test_trails_counters();
    reset_test_trails_counters();
    counters
}

#[cfg(test)]
fn note_test_get_path() {
    TEST_TRAILS_GET_PATH_CALLS.with(|value| value.set(value.get() + 1));
}

#[cfg(test)]
fn note_test_context_build() {
    TEST_TRAILS_CONTEXT_BUILDS.with(|value| value.set(value.get() + 1));
}

#[cfg(test)]
fn note_test_worker_call() {
    TEST_TRAILS_WORKER_CALLS.with(|value| value.set(value.get() + 1));
}

const RENDER_SET_KIND: &str = "fictional-energy-vfx-animated-socket-trails-v2-render-set";
const FRAME_RECEIPT_KIND: &str = "fictional-energy-vfx-animated-socket-trails-v2-frame-receipt";
const SEQUENCE_RECEIPT_KIND: &str =
    "fictional-energy-vfx-animated-socket-trails-v2-sequence-receipt";
const COLOR_KIND: &str = "fictional-energy-vfx-animated-socket-trails-v2-trail-color";
const ID_KIND: &str = "fictional-energy-vfx-animated-socket-trails-v2-trail-id";
const DEPTH_KIND: &str = "fictional-energy-vfx-animated-socket-trails-v2-trail-depth";
const APPEARANCE_GLB_KINDS: &[&str] = &["appearance-glb", "appearance-v2-glb"];
...
fn parse_prepare(
    value: &Value,
) -> Result<FictionalEnergyVfxAnimatedSocketTrailsSequenceV2PrepareRequest, RuntimeError> {
    ...
    let request: FictionalEnergyVfxAnimatedSocketTrailsSequenceV2PrepareRequest =
    ...
    {
        return Err(invalid(
            "V2 candidate, policy or full 16-source/15-output schedule differs",
        ));
    }
...
fn parse_get(
    value: &Value,
) -> Result<FictionalEnergyVfxAnimatedSocketTrailsSequenceV2GetRequest, RuntimeError> {
    ...
    let request: FictionalEnergyVfxAnimatedSocketTrailsSequenceV2GetRequest =
        serde_json::from_value(value.clone())
...
fn validate_frame_input(
    frame: &FictionalEnergyVfxAnimatedSocketTrailsSequenceV2FrameInput,
    ordinal: usize,
...
fn read_v2_glb(
    ...
    if object.schema_version != "CasObject@1"
        ...
        || !is_allowed_glb_kind(object.kind.as_str(), allowed_kinds)
    {
        return Err(invalid(format!("{role} GLB metadata differs")));
    }
    let bytes = runtime.cas_read_bounded(hash, MAX_GLB_BYTES)?;
...
fn is_allowed_glb_kind(kind: &str, allowed_kinds: &[&str]) -> bool {
    allowed_kinds.iter().any(|allowed| *allowed == kind)
}
...
fn read_v2_json_object(
    ...
    read_canonical_json(runtime, hash, schema)
}

fn canonical_values_equal(left: &Value, right: &Value) -> Result<bool, RuntimeError> {
    let left = canonical_json_bytes(left).map_err(|error| invalid(error.to_string()))?;
    let right = canonical_json_bytes(right).map_err(|error| invalid(error.to_string()))?;
    Ok(left == right)
}
...
fn particle_frame<'a>(
    sequence: &'a FictionalEnergyVfxAnimatedSocketParticlesSequenceV2,
    index: usize,
) -> Result<&'a FictionalEnergyVfxAnimatedSocketParticlesSequenceV2Frame, RuntimeError> {
    sequence
        .frames
        .get(index)
...
fn particle_local_offset(
    runtime: &Runtime,
    particle_frame: &FictionalEnergyVfxAnimatedSocketParticlesSequenceV2Frame,
    source_id: u64,
) -> Result<[f32; 3], RuntimeError> {
    let receipt = read_canonical_json(
        runtime,
        &particle_frame.receipt_object_sha256,
        "FictionalEnergyVfxAnimatedSocketParticlesFrameReceipt@2",
    )?;
    let values = receipt
        .get("particles")
        .and_then(Value::as_array)
...
fn camera_for_base(
    runtime: &Runtime,
    request: &FictionalEnergyVfxAnimatedSocketTrailsSequenceV2PrepareRequest,
...
fn validate_projection_parent(
    ...
    if projection
        ...
        || projection
            ...
            .and_then(Value::as_bool)
            != Some(false)
        || projection.get("version_created").and_then(Value::as_bool) != Some(false)
        || projection.get("export_performed").and_then(Value::as_bool) != Some(false)
        || projection
        ...
        || projection
            ...
            != Some(false)
    {
        return Err(invalid("Projection@2 status flags differ"));
    }
    if projection
        .get("sample_count")
...
fn validate_preroll_schedule(
    projection: &Value,
    particles: &FictionalEnergyVfxAnimatedSocketParticlesSequenceV2,
    output_ticks: &[u64],
) -> Result<(), RuntimeError> {
    let projection_ticks = projection
        .get("sample_time_ticks")
        .and_then(Value::as_array)
        .ok_or_else(|| invalid("Projection@2 sample schedule is unavailable"))?;
    if projection_ticks.len() != output_ticks.len() + 1
        || particles.sample_time_ticks.len() != output_ticks.len() + 1
        || projection_ticks
            .iter()
            .zip(&particles.sample_time_ticks)
            .any(|(projection_tick, particle_tick)| {
                projection_tick.as_u64() != Some(*particle_tick)
            })
        || projection_ticks
            .iter()
            .skip(1)
            .zip(output_ticks)
            .any(|(projection_tick, output_tick)| projection_tick.as_u64() != Some(*output_tick))
        || projection_ticks
            .windows(2)
            .any(|pair| pair[0].as_u64() >= pair[1].as_u64())
    {
        return Err(invalid(
            "Projection@2/Particles@2 pre-roll schedule differs",
        ));
    }
    let projection_frames = projection
        .get("frames")
        .and_then(Value::as_array)
        .ok_or_else(|| invalid("Projection@2 frames are unavailable"))?;
    if projection_frames.len() != projection_ticks.len()
        || projection_frames.iter().enumerate().any(|(index, frame)| {
            frame.get("frame_index").and_then(Value::as_u64) != Some(index as u64)
                || frame.get("sample_time_ticks").and_then(Value::as_u64)
                    != projection_ticks[index].as_u64()
        })
    {
        return Err(invalid("Projection@2 frame pre-roll order differs"));
    }
    Ok(())
}
...
fn validate_base_bloom(
    request: &FictionalEnergyVfxAnimatedSocketTrailsSequenceV2PrepareRequest,
    input: &FictionalEnergyVfxAnimatedSocketTrailsSequenceV2FrameInput,
    base: &FictionalEnergyVfxFrameLinkRecord,
...
pub(super) struct TrailFrameContext {
    pub(super) input: FictionalEnergyVfxAnimatedSocketTrailsSequenceV2FrameInput,
    pub(super) projection_samples: Value,
    pub(super) trails: Vec<FictionalEnergyVfxAnimatedSocketTrailsV2Trail>,
    pub(super) history_samples: Vec<FictionalEnergyVfxAnimatedSocketTrailsV2HistorySample>,
...
pub(super) struct DependencyContext {
    ...
    pub(super) geometry_preservation_projection_sha256: String,
    pub(super) geometry_preservation_status: String,
    pub(super) anchor_binding_sha256: String,
}
...
fn world_trails_and_inventory(
    ...
    (
        Vec<FictionalEnergyVfxAnimatedSocketTrailsV2Trail>,
        Value,
        String,
    ),
    RuntimeError,
> {
    ...
    for (trail_index, definition) in wire.iter().enumerate() {
        ...
        for (point_index, point) in points.iter().enumerate() {
            ...
            record_points.push(FictionalEnergyVfxAnimatedSocketTrailsV2Point {
        ...
        trails.push(FictionalEnergyVfxAnimatedSocketTrailsV2Trail {
            emitter_role: emitter_role.to_owned(),
            trail_id: if trail_index == 0 { 10_000 } else { 20_000 },
...
fn compare_worker_replay(
    ...
    if first.build_cohort_sha256.is_none()
        ...
        || first.trail_passes[0].pass != "trail-color"
        || first.trail_passes[1].pass != "trail-id"
        || first.trail_passes[2].pass != "trail-depth"
        || first
            .trail_passes
            .iter()
...
pub(super) fn build_context(
    ...
    let particle_value = runtime
        .fictional_energy_vfx_animated_socket_particles_sequence_v2_get(&json!({
    ...
    if particle_value.get("schema_version").and_then(Value::as_str)
        != Some("FictionalEnergyVfxAnimatedSocketParticlesSequenceGetResult@2")
        || particle_value.get("runtime_write").and_then(Value::as_bool) != Some(false)
        || particle_value
            .get("restart_hash_verified")
            .and_then(Value::as_bool)
            != Some(true)
    {
        return Err(invalid(
            "Particles@2 get is not a read-only verified result",
        ));
    }
    ...
    if particles.schema_version != "FictionalEnergyVfxAnimatedSocketParticlesSequence@2"
        ...
        || particles.sequence_key_sha256 != request.particle_sequence_key_sha256
        ...
        || particles.canonical_sha256.is_empty()
        ...
        || particles.human_review_status != "NOT_RUN"
        || particles.commercial_engine_status != "NOT_RUN"
        || !particles.runtime_write_performed
        || !particles.restart_hash_verified
        || particles.candidate_confirmed
        || particles.version_created
        || particles.export_performed
        || particles.actual_engine_roundtrip
        || particles.production_stage_advanced
        || particles.sample_count != request.sample_count + 1
        || particles.sample_time_ticks.len() != request.sample_count as usize + 1
        || particles.frames.len() != request.sample_count as usize + 1
    {
        return Err(invalid("particle sequence parent binding differs"));
    }
    if particles.sample_time_ticks.get(1..) != Some(request.sample_time_ticks.as_slice())
        || particles
            .sample_time_ticks
            .windows(2)
            .any(|pair| pair[0] >= pair[1])
        || particles
            ...
            .iter()
            .enumerate()
            .any(|(index, frame)| frame.frame_index != index as u64)
    {
        return Err(invalid(
            "particle sequence pre-roll/sample schedule differs",
        ));
    }
    validate_preroll_schedule(&projection, &particles, &request.sample_time_ticks)?;
    let source_glb = read_v2_glb(
        runtime,
        &request.appearance_artifact_sha256,
        "appearance source",
        APPEARANCE_GLB_KINDS,
    )?;
    ...
    Ok(DependencyContext {
        ...
        geometry_preservation_projection_sha256: particles
            .geometry_preservation_projection_sha256
            .clone(),
        geometry_preservation_status: particles.geometry_preservation_status.clone(),
        anchor_binding_sha256: particles.anchor_binding_sha256.clone(),
    })
}
...
pub(super) fn frame_without_receipt(
    frame: &FictionalEnergyVfxAnimatedSocketTrailsSequenceV2Frame,
) -> Value {
...
fn make_frame_record(
    ...
    let trail_key_sha256 = canonical_json_hash(&json!({
        "schema_version":"FictionalEnergyVfxAnimatedSocketTrailsV2TrailKey@1",
    ...
    let trail_id_encoding_sha256 = canonical_json_hash(&json!({
        "schema_version":"FictionalEnergyVfxAnimatedSocketTrailsV2TrailIdEncoding@1",
        "muzzle-vfx":10000_u64,
        "energy-core-vfx":20000_u64,
        "source_particle_ids":{"muzzle-vfx":10000_u64,"energy-core-vfx":20000_u64}
    }));
    let mut frame = FictionalEnergyVfxAnimatedSocketTrailsSequenceV2Frame {
...
fn build_sequence(
    ...
    frames: Vec<FictionalEnergyVfxAnimatedSocketTrailsSequenceV2Frame>,
) -> FictionalEnergyVfxAnimatedSocketTrailsSequenceV2 {
    let mut sequence = FictionalEnergyVfxAnimatedSocketTrailsSequenceV2 {
...
fn request_matches_sequence(
    request: &FictionalEnergyVfxAnimatedSocketTrailsSequenceV2PrepareRequest,
    sequence: &FictionalEnergyVfxAnimatedSocketTrailsSequenceV2,
) -> bool {
...
pub(super) fn prepare(runtime: &Runtime, value: &Value) -> Result<Value, RuntimeError> {
    ...
    if let Some(existing) = runtime
        .store
        .get_fictional_energy_vfx_animated_socket_trails_sequence_v2(&request.sequence_key_sha256)?
    {
    ...
    let operation = (|| -> Result<Value, RuntimeError> {
        ...
        let sequence_bytes =
            canonical_json_bytes(&serde_json::to_value(&sequence).map_err(|error| {
                invalid(format!("V2 sequence receipt serialization failed: {error}"))
            })?)
            .map_err(|error| {
                invalid(format!(
                    "V2 sequence receipt canonicalization failed: {error}"
                ))
            })?;
        ...
        let stored = runtime
            .store
            .record_fictional_energy_vfx_animated_socket_trails_sequence_v2(
                &sequence,
...
pub(super) fn replay_request(
    stored: &FictionalEnergyVfxAnimatedSocketTrailsSequenceV2,
) -> FictionalEnergyVfxAnimatedSocketTrailsSequenceV2PrepareRequest {
    let frames = stored
        .frames
        .iter()
...
pub(super) fn get(runtime: &Runtime, value: &Value) -> Result<Value, RuntimeError> {
    ...
    let (stored, _) = get_with_context(runtime, &request)?;
    result_value(&stored, true, GET_RESULT_SCHEMA, false)
}

/// The complete read/replay implementation shared by the public Trails@2
/// getter and the in-request TrailsBloom@2 source path.  It is deliberately
/// not a cache: each caller supplies a fresh request and receives owned
/// values for that single call only.
pub(super) fn get_with_context(
    runtime: &Runtime,
    request: &FictionalEnergyVfxAnimatedSocketTrailsSequenceV2GetRequest,
) -> Result<
    (
        FictionalEnergyVfxAnimatedSocketTrailsSequenceV2,
        DependencyContext,
...
mod tests {
    ...
    fn animated_trail_id_encoding_is_stable() {
        let first =
            canonical_json_hash(&json!({"muzzle-vfx":10000_u64,"energy-core-vfx":20000_u64}));
        let second =
            canonical_json_hash(&json!({"muzzle-vfx":10000_u64,"energy-core-vfx":20000_u64}));
    ...
    #[test]
    fn test_trails_counters_are_thread_local_and_resettable() {
        reset_test_trails_counters();
        note_test_get_path();
        note_test_context_build();
        note_test_worker_call();
        note_test_worker_call();
        assert_eq!(test_trails_counters(), (1, 1, 2));

        let child = std::thread::spawn(test_trails_counters)
            .join()
            .expect("counter child thread");
        assert_eq!(child, (0, 0, 0));
        assert_eq!(test_trails_counters(), (1, 1, 2));

        assert_eq!(take_test_trails_counters(), (1, 1, 2));
        assert_eq!(test_trails_counters(), (0, 0, 0));
    }

    #[test]
    fn appearance_glb_kind_allowlist_is_closed() {
        assert!(is_allowed_glb_kind("appearance-glb", APPEARANCE_GLB_KINDS));
        assert!(is_allowed_glb_kind(
            "appearance-v2-glb",
            APPEARANCE_GLB_KINDS
        ));
        for kind in [
            ...
            assert!(!is_allowed_glb_kind(kind, APPEARANCE_GLB_KINDS));
        }
    }

    fn trail_input_fixture() -> FictionalEnergyVfxAnimatedSocketTrailsSequenceV2FrameInput {
