  ...
  return <div className="home-page">
    <section className="home-hero">
      ...
      <div className="home-hero-visual" aria-label="ForgeCAD 工作台预览">
        ...
        <div className="hero-viewport">
          ...
          <div className="hero-model-label"><span>当前视口</span><strong>等待模型版本</strong></div>
          <div className="hero-cursor-note"><Sparkle size={14} />语义操作会在这里可见</div>
        </div>
        <div className="hero-visual-footer"><span>模型结构</span><span>3D 视口</span><span>对象信息</span><span>版本</span></div>
      </div>
    </section>
    ...
    <section className="home-section">
      ...
      <div className="project-cards">
        <button type="button" className="project-card project-card-featured" onClick={() => onNavigate('workbench')}><div className="project-card-art project-card-art-robot"><Cube size={32} weight="duotone" /></div><div className="project-card-copy"><div><strong>Robot_A</strong><span>最近查看</span></div><p>等待 Codex 提交设计，当前没有已确认版本。</p><small>工作台 · 只读状态</small></div><ArrowRight className="project-card-arrow" size={18} /></button>
        ...
        <button type="button" className="project-card" onClick={() => onNavigate('check')}><div className="project-card-art project-card-art-check"><ShieldCheck size={28} weight="duotone" /></div><div className="project-card-copy"><div><strong>检查确认</strong><span>了解质量状态</span></div><p>先看外形、结构和材质是否有足够证据。</p><small>等待可检查的模型版本</small></div><ArrowRight className="project-card-arrow" size={18} /></button>
      </div>
    </section>
  ...
  return <div className="create-page">
    ...
    <div className="wizard-layout">
      <section className="wizard-card">
        ...
        {step === 2 && <div className="wizard-content"><div className="wizard-content-heading"><span className="wizard-number">02</span><div><h2>用一句话描述目标</h2><p>你不需要知道专业工具或几何参数；告诉 Codex 结果应该是什么。</p></div></div><textarea className="intent-input" value={description} onChange={(event) => setDescription(event.target.value)} aria-label="模型需求" /><div className="intent-templates"><span>快捷模板</span><button type="button" onClick={() => setDescription('做成适合游戏渲染的机械机器人，保持参考图的肩部比例。')}>游戏角色</button><button type="button" onClick={() => setDescription('生成一个结构清晰、方便继续修改的科幻机器人草模。')}>机械机器人</button><button type="button" onClick={() => setDescription('根据参考图做一个适合展示的影视概念模型。')}>影视概念模型</button></div></div>}
...
function CheckPage({ onNavigate, onNotice }: PageProps) {
  return <div className="check-page">
    <div className="check-heading"><div><span className="section-kicker">检查确认</span><h1>先看结果，再决定下一步</h1><p>专业指标保留在详细证据里；这里先告诉你外形、结构和材质是否可以继续。</p></div><button type="button" className="secondary-button" onClick={() => onNavigate('workbench')}><ArrowLeft size={16} />返回工作台</button></div>
    ...
    <section className="check-layout"><div className="check-compare-card"><div className="check-card-header"><div><span className="section-kicker">当前版本</span><h2>与参考图对比</h2></div><span className="status-label status-label-warn">尚未运行</span></div><div className="check-compare-grid"><div className="check-canvas"><span>参考图</span><div><ImageSquare size={28} /><strong>等待参考图</strong><small>尚未读取授权图片</small></div></div><div className="check-canvas"><span>当前版本</span><div><Cube size={28} /><strong>等待模型渲染</strong><small>检查结果由 Runtime 返回</small></div></div></div><div className="check-compare-footer"><span>固定视角 · 512×512</span><span>参考图：—</span><span>模型渲染：—</span></div></div><aside className="check-result-card"><div className="check-card-header"><div><span className="section-kicker">你能看懂的结果</span><h2>三件事</h2></div><ShieldCheck size={20} /></div><div className="result-row"><span className="result-icon result-icon-warn"><WarningCircle size={17} /></span><div><strong>外形</strong><small>需要参考图和当前版本进行比较</small></div><span>未开始</span></div><div className="result-row"><span className="result-icon result-icon-lock"><LockKey size={17} /></span><div><strong>结构</strong><small>通过外形检查后才会解锁</small></div><span>锁定</span></div><div className="result-row"><span className="result-icon result-icon-lock"><LockKey size={17} /></span><div><strong>材质</strong><small>当前阶段不提前承诺</small></div><span>未开始</span></div><div className="check-guidance"><Info size={16} /><p>发现问题后，Codex 会根据已绑定的参考图和版本证据生成下一轮修正；不会直接覆盖当前版本。</p></div><button type="button" className="primary-button primary-button-wide" onClick={() => onNotice('当前没有可安全生成的修正意图；请先让 Codex 提交模型版本和固定视图。')}>让 Codex 检查</button></aside></section>
  </div>
}
...
function ExportPage({ onNavigate, onNotice }: PageProps) {
  ...
  return <div className="export-page">
    <div className="export-heading"><div><span className="section-kicker">导出</span><h1>把已确认的版本带走</h1><p>只有确认后的版本可以导出。当前还没有可导出的版本，因此按钮保持等待状态。</p></div><button type="button" className="secondary-button" onClick={() => onNavigate('workbench')}><ArrowLeft size={16} />返回工作台</button></div>
    <div className="export-layout"><section className="export-form-card"><div className="export-form-header"><span className="export-number">01</span><div><h2>选择用途</h2><p>用途只影响推荐的导出预设，不会改变模型内容。</p></div></div><div className="purpose-grid">{['游戏', '渲染', '3D 打印', '通用'].map((item) => <button type="button" key={item} className={purpose === item ? 'purpose-option purpose-option-active' : 'purpose-option'} onClick={() => setPurpose(item)}><span>{item === '游戏' ? <PlayCircle size={21} /> : item === '渲染' ? <Sparkle size={21} /> : item === '3D 打印' ? <Cube size={21} /> : <Stack size={21} />}</span><strong>{item}</strong><small>{item === '游戏' ? '实时引擎' : item === '渲染' ? '固定渲染' : item === '3D 打印' ? '实体检查' : '保留更多信息'}</small></button>)}</div><div className="export-form-header export-form-header-secondary"><span className="export-number">02</span><div><h2>导出格式</h2><p>确认版本和文件检查完成后，格式选项会启用。</p></div></div><div className="format-select-row"><button type="button" className={format === 'GLB' ? 'format-option format-option-active' : 'format-option'} onClick={() => setFormat('GLB')}><strong>GLB</strong><span>推荐 · 当前工作台可读取</span></button><button type="button" className={format === 'OBJ' ? 'format-option format-option-active' : 'format-option'} onClick={() => setFormat('OBJ')}><strong>OBJ</strong><span>仅几何 · 材质可能分离</span></button></div></section><aside className="export-summary"><div className="summary-heading"><span>导出检查</span><DownloadSimple size={18} /></div><div className="export-target"><span className="export-target-icon"><FolderOpen size={22} /></span><div><strong>当前确认版本</strong><small>暂无已确认版本</small></div></div><div className="export-checklist"><div><span className="checklist-dot checklist-dot-warn" /><span>确认版本</span><strong>等待</strong></div><div><span className="checklist-dot checklist-dot-warn" /><span>模型文件</span><strong>等待</strong></div><div><span className="checklist-dot checklist-dot-lock" /><span>材质与许可证</span><strong>锁定</strong></div></div><div className="export-selected"><span>用途</span><strong>{purpose}</strong><span>格式</span><strong>{format}</strong></div><button type="button" className="primary-button primary-button-wide" onClick={() => onNotice('导出尚未启动：需要先由 Codex 确认一个版本。')} disabled><DownloadSimple size={17} />请求导出</button><small className="export-footnote">导出会保留使用许可和来源记录；不会把本机路径或密钥写入包内。</small></aside></div>
  </div>
}
