...
def test_deterministic_planner_emits_one_complete_direction_for_each_pack():
    ...
    for pack_id in (
        ...
        assert plan.spec["visual_intent_mapping"]["schema_version"] == "VisualIntentMapping@2"
        if pack_id == "pack_robotic_arm_concept":
            assert plan.arm_design_intent is not None
            assert plan.arm_design_intent.schema_version == "ArmDesignIntent@1"
