...
from .spec_validation import WeaponSpecValidationError, validate_patch_manifest, validate_quality_report, validate_weapon_design_spec
...
class SQLiteAssetStore:
    ...
    def create_weapon(self, request: CreateWeaponRequest, idempotency_key: str) -> JobDetail:
        ...
        with self._connect() as conn:
            ...
            try:
                concept = self.image_provider.generate_concept(request, spec, weapon_id=weapon_id, version_id=version_id)
            ...
            self._insert_step(conn, job_id, "weapon_spec_planner", "succeeded", self.llm_provider.provider_id)
            self._insert_step(conn, job_id, "prompt_builder", "succeeded", self.llm_provider.provider_id)
            self._insert_step(conn, job_id, "image_submit", "succeeded", self.image_provider.provider_id)
            ...
            prompt_file_id = self._write_asset(
                ...
                ext=".json",
                mime_type="application/json",
                metadata={"provider": self.image_provider.provider_id, "schema_version": "PromptArchive@1"},
            )
            ...
            concept_sha256 = conn.execute("SELECT sha256 FROM asset_files WHERE file_id = ?", (concept_file_id,)).fetchone()["sha256"]
            quality_report = validate_quality_report(
                _concept_quality_report(
                    concept_file_id,
            ...
            if quality_report["status"] not in {"passed", "warning"}:
                raise LLMProviderError("PROVIDER_BAD_OUTPUT", "Concept image quality gate did not pass.", recoverable=True)
            ...
            self._record_provider_task(
                ...
                provider_task_id=concept.provider_task_id,
                status="succeeded",
                metadata={"artifact_asset_id": concept_file_id, "workflow_asset_id": workflow_file_id},
                updated_at=now,
            )
            model_task_record_id = self._record_provider_task(
                ...
                status="succeeded",
                metadata={"model_id": model_id, "raw_model_file_id": model_assets["raw_model_file_id"]},
                updated_at=now,
            )
            ...
            spec_sha256 = conn.execute("SELECT sha256 FROM asset_files WHERE file_id = ?", (spec_file_id,)).fetchone()["sha256"]
            conn.execute(
                """
            ...
            event_rows = [
                ("request_guard", "succeeded", "Request accepted and persisted in SQLite.", None, 0.1, {}),
                ("weapon_spec_planner", "succeeded", "WeaponDesignSpec stored in immutable AssetStore.", spec_file_id, 0.3, {}),
                (
                    "prompt_builder",
                    "succeeded",
                ...
                (
                    ...
                    0.9,
                    {"gated_by": quality_report_file_id, "model_id": model_id, "optimized_model_file_id": model_assets["optimized_model_file_id"]},
                ),
                ("finalize_job", "succeeded", "Job completed with SQLite-backed assets.", quality_report_file_id, 1.0, {}),
            ]
    ...
    def activate_version(self, weapon_id: str, version_id: str) -> WeaponDetail:
        with self._connect() as conn:
            ...
            if version is None:
                raise AssetStoreError("VERSION_NOT_FOUND", "Version was not found for this weapon.")
            now = utc_now()
            conn.execute(
                """
                UPDATE weapons
            ...
            if conn.total_changes == 0:
                raise AssetStoreError("WEAPON_NOT_FOUND", "Weapon was not found.")
            conn.commit()
    ...
    def get_weapon_detail(self, weapon_id: str) -> WeaponDetail:
        with self._connect() as conn:
            weapon = conn.execute(
                ...
                SELECT
                  ...
                  (
                    ...
                    FROM asset_files af
                    WHERE af.weapon_id = w.weapon_id
                      AND af.role IN ('rough_preview_png', 'concept_patch', 'concept_image')
                      AND af.soft_deleted_at IS NULL
                    ORDER BY af.created_at DESC
                    LIMIT 1
                  ) AS thumbnail_asset_id,
            ...
            asset_rows = conn.execute(
                """
                SELECT file_id, role, version_id, job_id, logical_path, sha256, byte_size,
            ...
            for asset in asset_rows:
                ...
                if asset["version_id"]:
                    assets_by_version.setdefault(asset["version_id"], []).append(summary)

            version_rows = conn.execute(
                """
                SELECT version_id, parent_version_id, job_id, version_no, version_type,
            ...
            if weapon["current_model_id"]:
                model = conn.execute(
                    "SELECT model_id, provider, status, orientation_policy_json, quality_report_json FROM models_3d WHERE model_id = ?",
                ...
                if model is not None:
                    current_model = {
                        ...
                        "quality_report": json.loads(model["quality_report_json"] or "{}"),
                    }

            jobs = conn.execute(
                """
                SELECT job_id, job_type, status, current_step, created_at, updated_at, finished_at
                FROM generation_jobs
                WHERE weapon_id = ?
                ORDER BY created_at DESC
                LIMIT 10
    ...
    def resolve_asset_file(self, asset_id: str) -> Dict[str, Any]:
        ...
        return {
            "path": full_path,
            "mime_type": row["mime_type"],
            "filename": Path(row["logical_path"]).name or f"{asset_id}.{row['ext']}",
            "sha256": row["sha256"],
            "role": row["role"],
            "logical_path": row["logical_path"],
        }
    ...
    def cancel_job(self, job_id: str) -> JobActionResponse:
        with self._connect() as conn:
            row = self._job_action_row(conn, job_id)
            previous_status = row["status"]
            if previous_status == "cancelled":
                now = utc_now()
                message = "Cancel request was already applied for this job."
                action_id = self._record_job_action(
                    conn,
                    job_id=job_id,
                    action_type="cancel",
            ...
            if previous_status not in {"created", "queued", "running", "waiting_provider", "waiting_user", "retrying"}:
            ...
            now = utc_now()
            current_step = row["current_step"] or "request_guard"
            provider_task_record_id = self._mark_current_provider_cancel_requested(
                conn,
                job_id=job_id,
                step_name=current_step,
                requested_at=now,
            ...
            self._update_latest_step_attempt(conn, job_id, current_step, "cancelled", now)
            conn.execute(
                """
                UPDATE generation_jobs
                ...
                WHERE job_id = ?
                """,
                (current_step, now, now if provider_task_record_id else None, now, now, job_id),
            )
            event_id = self._insert_job_action_event(
                conn,
                job_id=job_id,
            ...
            action_id = self._record_job_action(
                ...
                event_id=event_id,
                message=message,
                metadata={"action": "cancel", "provider_task_record_id": provider_task_record_id},
                created_at=now,
            )
            return JobActionResponse(
                action_id=action_id,
    ...
    def retry_job(self, job_id: str) -> JobActionResponse:
        with self._connect() as conn:
            row = self._job_action_row(conn, job_id)
            retry_from = self._default_retry_step(conn, job_id, row["current_step"])
            return self._request_retry(conn, row=row, retry_from=retry_from, action_type="retry")
    ...
    def has_weapon(self, weapon_id: str) -> bool:
        with self._connect() as conn:
            return self._has_weapon_row(conn, weapon_id)

    def _has_weapon_row(self, conn: sqlite3.Connection, weapon_id: str) -> bool:
        return conn.execute(
            "SELECT 1 FROM weapons WHERE weapon_id = ? AND status = 'active'",
            (weapon_id,),
        ).fetchone() is not None

    def _job_action_row(self, conn: sqlite3.Connection, job_id: str) -> sqlite3.Row:
        row = conn.execute(
            """
    ...
    def _default_retry_step(self, conn: sqlite3.Connection, job_id: str, current_step: Optional[str]) -> str:
    ...
    def _request_retry(self, conn: sqlite3.Connection, *, row: sqlite3.Row, retry_from: str, action_type: str) -> JobActionResponse:
        previous_status = row["status"]
        job_id = row["job_id"]
        ...
        now = utc_now()
        attempt = self._insert_retry_step_attempt(conn, job_id, retry_from, now)
        conn.execute(
            """
            UPDATE generation_jobs
            SET status = 'retrying',
        ...
        event_id = self._insert_job_action_event(
            ...
            metadata={"previous_status": previous_status, "retry_from": retry_from, "attempt": attempt, "action": "retry"},
            created_at=now,
        )
        message = f"Retry requested from step {retry_from}."
        action_id = self._record_job_action(
    ...
    def _insert_retry_step_attempt(self, conn: sqlite3.Connection, job_id: str, step_name: str, created_at: str) -> int:
        row = conn.execute(
            "SELECT COALESCE(MAX(attempt), 0) + 1 AS next_attempt FROM job_steps WHERE job_id = ? AND step_name = ?",
            (job_id, step_name),
        ).fetchone()
        ...
        conn.execute(
            ...
            (
                ...
                attempt,
                created_at,
                _canonical_json({"step": step_name, "attempt": attempt, "resume_policy": "restart_step"}),
            ),
        )
        self._upsert_checkpoint(
            conn,
    ...
    def _update_latest_step_attempt(
        ...
        if row is None:
            conn.execute(
                ...
                (
                    ...
                    finished_at,
                    finished_at,
                    _canonical_json({"step": step_name, "status": status, "updated_by": "job_action"}),
        ...
        conn.execute(
            ...
            SET status = ?,
                ...
                cancel_state = CASE WHEN ? = 'cancelled' THEN 'cancel_requested' ELSE cancel_state END,
                checkpoint_json = ?
            WHERE step_id = ?
            """,
            (
        ...
        attempt_row = conn.execute("SELECT attempt FROM job_steps WHERE step_id = ?", (row["step_id"],)).fetchone()
    ...
    def _mark_current_provider_cancel_requested(
        ...
        *,
        ...
        conn.execute(
            ...
            WHERE job_id = ? AND step_name = ?
              AND attempt = (
                SELECT MAX(attempt) FROM job_steps WHERE job_id = ? AND step_name = ?
              )
            """,
