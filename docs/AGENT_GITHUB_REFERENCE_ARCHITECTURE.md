...
| --- | --- | --- | --- | --- |
| [OpenAI Codex](https://github.com/openai/codex) | Rust app-server 明确建模 initialize、Thread/Turn/Item、JSON-RPC 请求/通知与流式生命周期；core-skills 从 `SKILL.md` 加载 | Rust 协议所有权、任务生命周期、状态事件、可取消 Turn、背压、技能渐进加载、工具结果与批准 | 通用 shell、代码库权限、原始推理展示、直接 fork 或照搬其 coding-agent 数据模型 | 核心 Agent 运行时与交互参考 |
...
- 保留 Tauri + React + SQLite + Three.js，并按 ADR-0014 将 Agent/状态所有权从 FastAPI 迁移到 Rust app-server；
...
└── SQLite / content-addressed object ownership
          ...
          ▼
Restricted Geometry Executor（迁移期 Python）
...
├── 已采用的 Manifold Python CSG；禁止第二默认内核
...
- 当前 Tauri/React、Rust `forgecad.app-server/1`/`forgecad-core` 产品状态与 Three.js 本机 Alpha；Python FastAPI 仅作为 capability-gated `RestrictedGeometryExecutor`，不再承载 SQLite 业务状态；
- 当前 OpenAI-compatible Provider port；
- ShapeProgram 白名单和 ChangeSet 不可变版本；
- 单一 Orchestrator，不引入多 Agent 框架；
- K001–K003 已依次完成 Rust 协议/桥接、Agent 生命周期/Provider 所有权和 Project/Snapshot/ChangeSet 持久化所有权；运行时继续只有 Rust core 一个产品状态写入者。

### Rust-first 迁移边界

1. K001 已建立 `forgecad-app-server-protocol`、initialize、版本化 JSON-RPC、通知、取消、背压、cursor replay 和 Tauri bridge；它没有迁移数据库写入，Python 仍是唯一持久化写入者；
2. K002 把 Thread/Turn/Item/Approval、Provider、Product Tool 调度和用量/取消迁到 Rust，Python 只作为受限产品工具执行器；
3. K003 把 Project、AgentAssetVersion、ActiveDesignSnapshot、ChangeSet、Quality、Export、SQLite 和对象库所有权迁到 Rust，Python 几何执行器不再接收数据库路径、密钥或 Snapshot 写权限；
4. K003 已于 2026-07-18 退出；当前可准确表述为“Rust-first 产品核心 + Python 受限几何执行器”，不得误写为纯 Rust 几何、正式发布版或无 Python sidecar。
