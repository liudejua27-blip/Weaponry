...
`FGC-M108` 已由 `npm run agent:m108-visual-pbr-smoke` 覆盖四领域 12 个同源多 zone PBR fixture、色彩空间、来源/许可证、纹理缺失/损坏、clearcoat/透明兼容和环境 hash；`npm run agent:m108-gltf-validator-smoke` 使用锁定的 Khronos `gltf-validator@2.0.0-dev.3.10` 对四领域原始 GLB 断言零 error/zero warning，并验证畸形 GLB 拒绝。该外部标准报告不替代 `GeometryCompileReadback@1`。`desktop:r3-concept-workbench-smoke` 还在真实单一 Three renderer 中断言 Agent blockout 优先加载同源 GLB、实际绑定五通道 PBR 贴图并拒绝把参数 ShapeProgram 预览冒充纹理输出；没有 GLB 时才显示明确的参数外观回退。`agent:m108-gltf-transform-evaluation` 已拒绝改变 readback 的 Transform 写出；KTX2/BasisU 因缺编码、视口解码/回退和 packaged Gate 而保持未采用；真实 packaged sidecar 的 PBR/readback、preview/confirm、undo/redo、CSG、导出和重启回归已通过。`agent:m108-visual-benchmark-kit-smoke` 生成并校验四领域、评分为空的独立审阅包；`agent:m108-visual-benchmark-score-validator-smoke` 用临时合成合同 fixture 锁定独立声明、四领域覆盖、同源 PBR 视口事实、整数评分和 4/5 中位数拒绝边界。按 `evidence/M108_VISUAL_BENCHMARK_PROTOCOL.md` 收集至少三位独立评审前，人工视觉基准仍未通过，M108 仍不得解除 C105。`FGC-C105` 必须覆盖 Profile/section/feature/参数/Connector/zone/child slot 配方的循环、质量、来源和版本；`FGC-R007`、`FGC-D006` 分别覆盖只读参考不变与新资产 provenance，以及新领域包的 draft/evaluated/enabled 晋级和未知领域零万能 fallback。
