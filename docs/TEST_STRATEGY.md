...
M108 的固定车辆/航空器 A 资产现在还要由 G818 证明主壳和座舱真实使用 `loft`、固定 axis length/cross-section scale 未漂移，旧车辆 nose/cabin 与突兀后三角件不得回归。G822/G823 对基于物理周长/路径距离的 UV 保留有界 `<=64` 回读，M108 再要求实际视觉 primitive 携带 320 mm 重复元数据。旋翼小轮毂与四个叶片的数量/尺寸/材质同样由 G818 锁定，工作台 renderer smoke 对当前 6,196 triangles/96 draw calls 继续执行预算上限。Codex 代理审查报告不得被 `validate_m108_visual_benchmark_scores.py` 读作人工评分；`review-responses.json` 保持空模板，M108 状态仍取决于既定独立人工退出门。
