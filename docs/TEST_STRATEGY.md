...
2026-08-15 最新测试覆盖：合同 checker、Visual Surface negative/mesh-analysis Gate、Stage0 truth、diff check、`cargo check --workspace`、Repair focused test 和 `cargo test --workspace` 已通过；当前工具面为 38 read + 30 opt-in write = 68。`visual_surface_get` 验证 candidate-bound AOV 与同一 GLB 的 bounded surface analysis，仍不设置质量 Gate；`design_action_optimization_proposal_prepare` 已有 source-level contract/dispatch/runtime boundary，严格改进不足或视觉门失败时保持 blocked，不自动 Repair/Confirm；不能写成真实参考或视觉生产质量 Gate PASS。
...
| 010D/SurfaceProgram | 每个已实现 Operator 正/负 fixture、预算/超时/崩溃、mirror/Part lineage；当前 16 项 catalog（primitive@2、hard-surface 与 SurfaceProgram）和 fixed sibling Worker Gate PASS；Manifold C API/FFI、union/difference/intersection、MeshGL64 topology/readback、determinism、ASan/UBSan、资源/移除和 raw stdio adoption PASS；surface-patch/surface-shell/subd-cage source Gate PASS；packaged rebuild、视觉/PBR/human/export/360 仍 deferred |
...
| Visual Surface slice | `VisualSurfaceRequest@1`/`VisualSurfaceResult@1` closed fields、canonical hashes、candidate/reference/artifact/RenderSet/camera/quality lineage、AOV/ROI 与 `SurfaceSignalReadback@1`、extra-field/duplicate/cross-candidate/binding-mismatch/non-manifold negative gates；只读、不创建 candidate/version；SubD/NURBS/质量晋级仍未运行 |
