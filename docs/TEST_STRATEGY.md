...
`FGC-M108` 后端聚合门为 `npm run agent:m108-gate`，它依次运行 PBR、锁定 Khronos Validator、glTF Transform 拒绝决策、无评分 benchmark kit、评分合同 self-test 和 G826 表面 readback，并接入 `backend-and-contracts` CI。PBR Gate 覆盖四领域 12 个同源多 zone fixture，对每份资产直接断言至少 3 个稳定 zone，并参数化拒绝 baseColor、metallicRoughness、normal、occlusion、emissive 五通道的引用缺失与字节损坏；删除 IOR、双重 alpha/transmission 和篡改已使用 clearcoat 也须拒绝。`agent:m108-gltf-validator-smoke` 对四领域原始 GLB 断言零 error/zero warning；该外部报告不替代 `GeometryCompileReadback@1`。`agent:m108-gltf-transform-evaluation` 先对比标准读取阶段的 Part/zone/material/VisualTextureSet 映射，再要求 writer 删除显式默认 PBR 值后的四份写出全部由 ForgeCAD readback 以该精确原因拒绝，并以 `decision=reject_core_writer_as_export_transform` 正常退出；该工具仍未被采用为优化器。
