...
`FGC-M108` 后端聚合门为 `npm run agent:m108-gate`，它依次运行 PBR、锁定 Khronos Validator、glTF Transform 拒绝决策、无评分 benchmark kit、评分合同 self-test、G818 细节语法和 G826 表面 readback，并接入 `backend-and-contracts` CI。PBR Gate 覆盖四领域 12 个同源多 zone fixture，对每份资产直接断言至少 3 个稳定 zone，并参数化拒绝 baseColor、metallicRoughness、normal、occlusion、emissive 五通道的引用缺失与字节损坏；它还逐相位拒绝 8/12/16/18/28/32 px 硬格线、单资产 v1/v2 混版、未知/错位材质、采样状态和伪索引，真实 v1 vehicle 回读使历史材质 index 联集覆盖 `0..7`，fresh-process cache 断言为 `0→8→16`。评分合同 self-test 会从 GLB readback 再次拒绝少于五套、非 `_builtin_v2` texture-set、非 `_v2_` map、非 128×128 或五通道 role 不完整的审阅资产。G818 另要求四个航空器旋翼支柱在最终 POSITION accessor 中各与对应机翼的 Z 范围至少重叠 0.07 m。删除 IOR、双重 alpha/transmission 和篡改已使用 clearcoat 也须拒绝。`agent:m108-gltf-validator-smoke` 对四领域原始 GLB 断言零 error/zero warning；该外部报告不替代 `GeometryCompileReadback@1`。`agent:m108-gltf-transform-evaluation` 先对比标准读取阶段的 Part/zone/material/VisualTextureSet 映射，再要求 writer 改变固定采样状态或删除显式默认 PBR 值后的四份写出全部由 ForgeCAD readback以明确原因拒绝，并以 `decision=reject_core_writer_as_export_transform` 正常退出；该工具仍未被采用为优化器。

M108 的固定车辆/航空器 A 资产现在还要由 G818 证明主壳和座舱真实使用 `loft`、固定 axis length/cross-section scale 未漂移，旧车辆 nose/cabin 与突兀后三角件不得回归。航空器左右主翼必须使用 Z 主轴的非对称 airfoil Profile、固定 16 点重采样、600 mm 轴长和 420×24 mm 截面尺度；每份 Profile 固定四段 tangent quadratic 且 `symmetry=none`，旧厚 wedge 主翼不得回归。G822/G823 对基于物理周长/路径距离的 UV 保留有界 `<=64` 回读，M108 再要求实际视觉 primitive 携带 320 mm 重复元数据。四个 52×48 mm 小轮毂和八片交叉叶片的数量/尺寸/材质同样由 G818 锁定，旧 `lift_hub_*` role 必须为空；工作台 renderer smoke 对当前航空器 6,508 triangles/96 draw calls 继续执行既有预算上限。Codex 代理审查报告不得被 `validate_m108_visual_benchmark_scores.py` 读作人工评分；`review-responses.json` 保持空模板，M108 状态仍取决于既定独立人工退出门。

M108 的最终几何检查必须解码真实 BIN POSITION，并要求 accessor 声明 bounds 与实际有限坐标相等；不得再从 ShapeProgram 参数或 accessor metadata 重复估算。readback 与 smoke 分别验证 accessor/bufferView 的严格非负整数索引、正 count、offset、显式 buffer、4–252 且 4 字节倍数的显式 stride、component alignment、view/BIN 末端和 PBR image view；负例至少覆盖实际 POSITION 篡改、短 view、负 accessor、缺失/外部 image buffer、bool/float node 引用、2/6/256 stride、view/accessor 错位和非整数 image offset。当前 ShapeProgram GLB 还必须是单 mesh、单 scene、单 identity node；第二 mesh/scene/node 或 TRS/children/instancing 均拒绝。12 份 M108 fixture 各要求一个最终 GLB AABB 视觉分量，但该门不得被描述成实体连接或全部 catalog 覆盖。G818 对航空器 B pod、机械臂 B wrist 与机械臂 C rail/carriage 外罩另外从目标部件重算中心/轴向/尺寸或半径高度，并要求双边范围、目标正重叠和外露体积，防止缩小、平移或错轴仍通过。

`npm run desktop:m108-workbench-renderer-smoke` 会从当前源码重新生成临时四领域 kit，再复用 R3 的真实浏览器工作台依次载入；它校验 GLB metre→millimetre 换算没有被 fit scale 覆盖、展示对角线为 520 mm、实时应用的完整环境 recipe SHA-256 等于 GLB 环境 hash、PBR 颜色空间正确、唯一 renderer/context 不被替换，并对 geometries/textures/draw calls/triangles/实际 PBR textures/估算 mip-chain 显存执行固定上限。截图前还必须证明 legacy ModuleGraph root 隐藏、当前 blockout root 可见、axes/grid/transform helper 均隐藏且 renderer line 数为 0；捕获 manifest 保存相同运行时事实。该 Gate 已接入 `workbench-e2e` CI；`desktop:r3-concept-workbench-smoke` 与 packaged smoke 继续独立覆盖同源视口和重启/undo/redo/导出路径。`agent:m108-visual-benchmark-score-validator-smoke` 只使用临时合成合同 fixture，不产生人类评分；身份及独立性仍须按 `evidence/M108_VISUAL_BENCHMARK_PROTOCOL.md` 人工核验。自动 GPU/环境门通过不等于视觉达标；收集至少三位真实独立评审前，M108 仍为 `in_progress`且不得解除 C105。`FGC-C105` 必须覆盖 Profile/section/feature/参数/Connector/zone/child slot 配方的循环、质量、来源和版本；`FGC-R007`、`FGC-D006` 分别覆盖只读参考不变与新资产 provenance，以及新领域包的 draft/evaluated/enabled 晋级和未知领域零万能 fallback。
...
M108 评分合同中的“至少五套”按至少五个不同 material index、texture-set ID 和规范 texture material 计算；重复 authored alias 不计数。self-test 另覆盖五通道 role 缺失/重复。工作台捕获的 renderer line 属性必须存在、为非负安全整数且等于 0，属性删除或改名不能通过默认值伪装。
