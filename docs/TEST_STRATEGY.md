...
`FGC-G819` 的操作清单 Gate 已使 Schema、Pydantic、运行时、质量入口与生成类型只接受同一操作集合；未知或缺执行器在副作用前返回 `UNSUPPORTED_RUNTIME_OPERATION`。`FGC-Q003` 的 `agent:q003-compile-readback-quality-smoke` 已覆盖四领域真实 GLB readback、质量/导出一致、编译或 readback 失败、未知操作、旧报告隔离和重启幂等；质量不再以 box/cylinder 常数平行估算。

ADR-0011 的几何子链必须逐项建立独立 Gate。G820 已覆盖合同。`FGC-G821` 已由 `agent:g821-profile-solid-fidelity-smoke` 覆盖曲线重采样、带孔/无孔 Extrude、开放 ribbon、完整/部分 Revolve、轴点、封盖、seam、bounds/triangle、UV0/normal accessor、逐三角 surface ranges、closed/boundary/non-manifold/degenerate readback、重复字节和预算/非法输入拒绝，并回归旧 G802/G803。`FGC-G822` 已由 `agent:g822-loft-smoke` 覆盖四领域壳体、不同主轴/截面/尺寸/位置/有限 twist、固定 seam、封盖、重复字节、triangle/bounds/normal/UV0/surface/topology readback，以及排序、采样点数、翻转、自交、退化、bounds 和写出前三角预算拒绝。`FGC-G823` 已由 `agent:g823-sweep-smoke` 覆盖直线、折线、多点平滑近似、有限 twist、开/闭 path、cap、frame 连续、重复字节、UV0/normal/surface/topology readback，以及零长度/过短段、180° 翻转、自交、点数/bounds/triangle 超限。`FGC-G824` 已由隔离 adapter、`evaluations/csg-g824/report.json` 与 `agent:g824-csg-benchmark-smoke` 比较当前 Worker、Manifold Python/WASM 的版本/许可证/体积/性能、四领域布尔、coplanar/near-degenerate 和确定性，并由 ADR-0012 不采用。`FGC-G824A` 再由 `evaluations/csg-g824a/report.json` 与 `agent:g824a-csg-adoption-evidence-smoke` 验证两个 binding 的 source/material/zone/face/backside property、simplify、确定性 GLB、ForgeCAD readback、近退化写出前拒绝和隔离进程 cancel/timeout。`FGC-G824B` 由 `evaluations/csg-g824b/report.json` 与 `agent:g824b-csg-promotion-boundary-smoke` 使用真实临时 SQLite/对象库/UnitOfWork 验证事务外 staging、三个中断窗口零权威状态提升及 Version/head/Snapshot 原子回滚/提交。`FGC-G824C` 再由 `evaluations/csg-g824c/report.json` 与 `agent:g824c-packaged-candidate-smoke` 验证当前 sidecar 入口真实冻结/导入/启动 Python 候选、arm64 archive、同轮基线/候选包体与冷启动、完整进程树 RSS、许可证文件 hash、零 Provider 调用和生产依赖隔离，并明确 WASM 不适配现有 Python sidecar host。`FGC-G824D` 的 Windows job 必须在真实 x64 frozen executable 内执行相同六组 provenance/readback、near-degenerate 拒绝、三个中断窗口和临时 SQLite/对象库/UnitOfWork 原子提升，并上传无密钥 JSON；workflow 或本机 hook 通过不能替代远端 artifact。A–D 都不接入默认 handler。`FGC-G825` 只有解除 ADR-0012 剩余条件后才验证唯一 CSG handler、不可变 feature node/input hash、失败零部分 GLB 和 surface provenance；`FGC-G826` 验证 edge finish、法线、UV0、tangent、face→part/zone 映射及优化后保留。HTML/SVG/GSAP 都不能作为这些 Gate 的几何证据。

`FGC-A003` 必须覆盖 Provider metadata/Keychain preflight、实际网络调用标记、stream/cancel、usage/cache 和 DeepSeek 400/401/402/422/429/500/503、空 JSON、Schema 错误；选择真实 Provider 后失败不得静默回退成功。`FGC-F025` 必须有 Agent-active/legacy-read-only 两条 UI 回归，前者不显示或调用 Graph Inspector、旧参数与旧导出，后者保持显式只读转换边界。`FGC-D005` 必须覆盖四领域语义比例/Style Token 配方、范围/步长、预览取消/确认与越界拒绝；不得出现自由工程尺寸。

ADR-0010 已将 `FGC-V002` 标记为 superseded，不再为三方向解释/重混增加 Gate。`FGC-A004` 必须覆盖受限 Tool Item 生命周期、G819/G826 交集、Profile/Recipe 工具 Schema、上限/取消/批准和 DeepSeek thinking Tool Call 的 `reasoning_content` 短生命周期续传。`FGC-V003` 必须覆盖从产品结构到已实现建模语法/Recipe 的路由、内部多候选、真实编译/readback/概念渲染硬门、确定性评分/唯一 winner/无 winner、候选零版本副作用，并断言默认 UI 方向卡数为 0、最佳结果卡为 1。`FGC-F026` 必须覆盖左上 mini 与中央 focus 共用同一 canvas/renderer，快速切换、焦点、窗口/项目/抽屉/模型重载和资源计数；GSAP 动画中断与 reduced-motion 分支不得改变状态真值。`FGC-A005` 必须覆盖专属 Skill Schema、严格工具交集、examples/evals、版本/provenance、越权拒绝和 dry-run 零副作用。
