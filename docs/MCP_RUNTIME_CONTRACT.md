...
2026-08-15 Stage Batch transport 增量：`design_stage_run_prepare` 的每个 action entry 可选 `view_spec` object；Runtime 将它与 typed `parameter_changes` 一并纳入父批次/子 ActionRun hash binding，并在无完整 proposal 时自动选择 bounded RuntimeParameterPatch。真实同 cohort receipt `docs/evidence/mcp010f/design-action-run-real-reference-stage-view-spec-20260815-b37.json` 通过，子 ActionRun 到 evaluate 后因 `QUALITY_TARGET_NOT_MET` 停止，checkpoint/replay/source immutability 通过；confirm/version/export 仍锁定。

2026-08-15 Runtime-owned parameter patch 增量：`design_action_run_prepare` 的 geometry proposal 现在支持受限 `RuntimeParameterPatch@1`（当前策略 `primitive-dimensions-v1`）。Codex 只提交 hash-bound session/candidate、单 Part `parameter_changes`、`view_spec` 和 camera；Runtime 从当前 `GeometryProgram@2` 唯一解析 `primitive@2`/`panel@1` 源节点，校验单位、范围、单节点/单 Part、operator 与 lineage，物化新程序并自行生成 `RepairIntent@1`，随后复用 `compile → readback → render → compare → review-candidate/checkpoint`。源 candidate 不变，confirm/export/version 仍锁定；调用方提交完整 GeometryProgram 的旧路径保留兼容。单测回执为 `docs/evidence/mcp010f/design-action-run-runtime-parameter-patch-unit-20260815.json`；它只证明 bounded execution/lineage，不证明视觉 likeness 或高质量通过。

相机绑定补充：Runtime 自动 framing 后会将 `CameraCalibration@1` 通过 Runtime canonical bytes 做一次 round-trip，再计算 `camera_hash` 与 `canonical_sha256` 后写入 CAS。`RenderSet@2.camera_object_sha256` 是该相机对象的显式绑定；`design_action_run_prepare` 读取 VisualBindings 时会回读并验证相机对象，且只接受与 session camera hash 一致的 proposal camera，随后以 Runtime-owned 对象替换调用方浮点 JSON。这样可以防止 Codex/MCP/Runtime 间相同数值的不同 JSON 文本表现造成 proposal 阶段的假性 camera mismatch；它不改变视觉质量门。

2026-08-15 最新源代码覆盖：当前为 123 Schema、40 read + 30 opt-in write = 70 tools。新增只读 `visual_surface_get`/VisualSurfaceResult@1 candidate-bound diagnostic projection，surface backend 为 `NOT_RUN`；`SilhouetteEvaluationObjective@1` 现可作为 `OptimizationIntent@1` 的 hash-bound policy 输入，Runtime 在 checkpoint/resume/final compare/result 中复用 Global non-regression + Part strict-improvement；`repair_apply_confirm` 只消费单视图且 fresh-approval/lineage revalidated 的 RepairApplyIntent，并经 Runtime Store 创建 immutable version；`design_action_optimization_proposal_prepare` 只物化严格且 non-regressing 的 ActionRun-bound CADFit review candidate，要求显式 ViewSpec，不自动 Repair/Confirm；多视图仍由 `cross_view_promotion_confirm` 处理。真实同 cohort receipt `docs/evidence/mcp010f/optimization-job-unified-objective-real-reference-20260815-cohort-v12.json` 完成 39 次 `32/4/3` 评估并保持 proposal-only；视觉仍 `QUALITY_TARGET_NOT_MET`。

2026-08-15 CADFit 源代码增量：`scripts/probe_mcp010c_codex_cli.py --cadfit-optimization` 通过真实 Codex/MCP 触发单一 `chest-shell` Part 的 6 参数、32/4/3 多保真 `OptimizationJob`，只产生 strict-improvement proposal，不确认候选、不创建版本。Runtime 的 `optimization_job_prepare`/`optimization_job_resume` 先返回持久 `queued` snapshot，再启动共享 SQLite/CAS Worker；MCP 对 Codex JSON 往返造成的连续数值短写法允许 exact 或确定性 normalized hash rebind，随后重绑定 nested camera/Rig hash，仍拒绝任意 hash。当前 source cohort 已重建；AssetPack transport smoke 因非用户参考且无严格改善保留为 BLOCKED，真实参考的 39 evaluation receipt 已通过 Job transport/proposal gate，但不能改写视觉质量。

2026-08-15 reference-coverage ActionRun boundary：`request-reference` 已成为独立的 orchestration action。Runtime 先校验 action target 必须等于当前 session 的 authorized `reference_id`，然后在 `prepare` 阶段直接持久化 `BLOCKED_REFERENCE_COVERAGE`；compile/readback/render/evaluate、checkpoint、OptimizationJob、candidate/version/export 均保持未运行或为空。`DesignSession.next_actions` 的该 action 现在输出 `scope_kind=reference`、`target_id=reference_id`、typed empty parameter list，可被 Codex 重新组装为可执行 ActionRun；它不执行导入、不接受 geometry proposal。

计数校正：下方旧 reconciliation 中的 115 Schema、37 read + 28 opt-in write = 65 是历史快照；Runtime/MCP 当前合同和工具面以 123 / 40 + 30 = 70 为准，`visual_surface_get` 与 `silhouette_evaluation_objective_prepare`/`silhouette_objective_compare` 是当前 candidate/objective diagnostics surface，backend/视觉 quality gate 仍按各自 receipt 判定。

版本：2026-08-14
状态：MCP005–MCP009 MVP functional core 和真实 Codex CLI host golden path 已完成；MCP010B structural source Gate PASS 但 Darwin OS memory hard cap deferred/NOT_RUN；MCP010C 当前源码为 37 read + 30 opt-in write = 67 个工具，fixed renderer/九 AOV/reference comparison/review raw Gate PASS；MCP010D/E offline Operator/AssetPack/UV/PBR/MikkTSpace raw Gate PASS，D 另已激活固定 Manifold C API Worker 的同一 Part bounded union/difference/intersection；MCP010F Viewer source 与轮廓目标/相机拟合/边界误差/多 Part 误差表 source Gate PASS；Agentic observe/plan projection 与 durable session/checkpoint/RepairIntent prepare/readback、bounded single-Part Repair proposal、逐视图 evidence inventory、`CrossViewEvidenceBundle@1` promotion boundary、bounded independent stage batch、hash-linked `cumulative-program` composition merge prepare、CADFit checkpoint/resume、`OptimizationResidual@1` hash-bound typed residual、`repair_apply_prepare` CAS-backed apply-intent prepare、`repair_apply_confirm` fixture/runtime transaction、`design_action_optimization_proposal_prepare` strict-improvement review-candidate continuation、fail-closed promotion transaction 也已通过各自隔离 source/transport/restart/focused receipt；positive merge、Repair 实际应用、完整组合式 orchestrator、跨视图同 cohort conformance、用户批准后的候选持久变更、通用 mesh Boolean、packaged/live C/D/E/F、真实用户 likeness、Viewer/PBR likeness/360仍 NOT_RUN/BLOCKED
...
当前工具账本校正：源码默认 38 个只读工具，显式 write opt-in 后共 68 个（38 read + 30 write）；下文旧数量仅指历史中间 receipt，不覆盖当前 manifest。

Stage 0 运行合同快照：当前共有 118 Schema、37 read + 30 opt-in write = 67 tools，唯一 `in_progress` 是 `FGC-MCP010F`，统一机器入口为 `docs/evidence/mcp010f/current-benchmark-truth.json`。Agentic observe/plan/critic/evidence 只读 projection 的隔离证据为 `docs/evidence/mcp010f/agentic-runtime-observe-plan-20260813.json`；durable session/checkpoint/RepairIntent prepare/readback 的隔离重启证据为 `docs/evidence/mcp010f/agentic-runtime-session-checkpoint-20260813.json`，它只证明受限持久化和 CAS-only restore intent；bounded Repair proposal、逐视图 evidence inventory、独立 stage batch、带 hash-linked `cumulative-program` merge prepare 的 composition proposal、`repair_apply_prepare` CAS-backed apply-intent、`repair_apply_confirm` fixture/runtime transaction、`design_action_optimization_proposal_prepare` strict-improvement review-candidate continuation、CADFit 恢复与 promotion boundary 另有独立 receipt/focused test，不证明 positive merge、Repair 实际应用、完整组合式 orchestrator 或用户批准后的候选变更。attempt35 仅是 provisional retained observation，它的结果为 `QUALITY_TARGET_NOT_MET + INCOMPLETE_TRUTH_BINDING`，benchmark eligibility 为 `BLOCKED_INCOMPLETE_BINDING`：camera-fit hash `354caf27…f95788` 与 reference-compare camera hash `8cd20605…a535` 为 `MISMATCH`；当前 packaged Viewer CLI read-model 已绑定 current cohort，但正式 UI/accessibility E2E 仍未运行。因此 source/raw/transport/build/AX smoke 只能证明对应合同或链路，不能声明视觉、人评或 packaged UI PASS。
...
2026-08-15 当前口径校正：Stage 0 marker 与 `current-benchmark-truth.json` 的合同计数为 118，工具面为 37 read + 30 opt-in write = 67；`BooleanRequest@1`/`BooleanResult@1` 已绑定 product-owned Manifold Boolean Worker，当前为同一 Part bounded union/difference/intersection。`DesignCompositionRequest@1`/`DesignCompositionResult@1`/`design_composition_prepare`、`RepairApplyPrepareRequest@1`/`RepairApplyPrepareResult@1`/`repair_apply_prepare` 与 `OptimizationProposalPrepareResult@1`/`design_action_optimization_proposal_prepare` 已新增显式 composition/apply-intent/CADFit review-candidate boundary 和可选 hash-linked `cumulative-program` merge prepare；CADFit `OptimizationResult@1` 另分离 highest-completed-fidelity best-so-far 与 proposal object hashes，并可接受经 Critic/PartError hash revalidation 的 `OptimizationResidual@1`。`CrossViewEvidenceBundle@1` 和 `cross_view_promotion_confirm` 仍只提供 bounded evidence 与 promotion fail-closed boundary；positive merge、Repair 实际应用、Repair promotion 正向成功和视觉质量状态不变。
...
MCP003 的工具清单固定排序并声明 `readOnlyHint=true`、`destructiveHint=false`、`idempotentHint=true`、`openWorldHint=false`。当前 MCP010B–F 源码默认有 37 个只读工具，显式 write opt-in 后为 28 个写工具；V2 authoring、AssetPack 查询、轮廓目标读取、相机拟合证据、边界误差、多 Part 误差表、render image 和 durable session/checkpoint readback 工具只校验或读取，`design_stage_run_prepare` 只按批准的 ordered batch 驱动独立 ActionRun，绝不把 MCP 适配器变成状态写者。`artifact_readback_get` 已读取 GLB header/lineage/part/triangle/UV/tangent metadata，`material_pack_get` 返回离线 AssetPack manifest，`quality_get`、`silhouette_target_get`、`camera_fit_prepare`、`boundary_error_get`、`session_get` 和 `checkpoint_get` 已可用但质量比较保守标记 limited；不得以自然语言把 limited、fixture 或 unavailable 伪装成视觉 PASS。`CameraCalibrationRef@1` 是 `SilhouetteFitIntent@1` 的闭合只读引用，只携带 Runtime-owned camera hash pair，Runtime 按 candidate/target evidence 解析完整 calibration，拒绝 hash 漂移或跨 candidate 引用。MCP010F Viewer 的 AOV、对比、部件/材质区筛选、爆炸图、热图和轮廓画布仅为只读 ephemeral projection；Viewer durable lookup 同样不提供写入入口。MCP010A Dev.app 的 30-tool activation receipt、MCP003 历史的 17-read snapshot，以及 MCP010B 的 3c/f488 package receipts 都必须保留为历史。当前总源合同为 115（历史合同 + MCP010B/C/D/E/F 与 Agentic contract family），B/C/D/E/F source-focused Gate 均已按各自范围通过；历史 `bfa56ac…de9` package receipt和 `d9c23b…ac0bd` Dev.app package/Worker/raw/real-Codex structural Gate及 live Desktop structural activation均已通过。
...
`quality_get` 保持既有只读名称，现可读回 candidate-bound `QualityReport@2`；source synthetic/raw PASS 不等于用户图片 likeness PASS。当前 source manifest 为 37 read + 28 opt-in write = 65；Agentic projection 只读、可重建，durable session/checkpoint/RepairIntent、逐视图 evidence inventory、独立 stage batch、`repair_apply_prepare` 和 composition proposal 只代表已验证的 prepare/readback/focused orchestration receipt，不是 QualityReport 或 confirmed version 真值；空工具、自然语言结果或 target Schema不能改变 capability 状态。同一 provisional observation 绑定的 packaged Viewer E2E、live C/D/E/F、人评阈值、真实 PBR likeness 和 HQ_360 仍必须独立记录。
...
CADFit 特例：`optimization_job_prepare` 创建 bounded `OptimizationIntent@1`；`optimization_job_resume` 只允许在存在且能回读的 `OptimizationResult@1` checkpoint 时恢复。checkpoint 必须绑定 job/intent，并包含 `next_stage`、评估对象 hash、候选 program/GLB 对象 hash；Runtime 逐项校验后跳过已完成 fidelity，永不从缺失状态静默重启。此特例不改变其他 Job 的通用恢复状态，也不允许 candidate/version/export 变更。
...
- `npm run mcp003:stdio` 的历史 receipt 校验四个响应、17 个只读工具、能力资源和协议不兼容 fail-closed。历史 19-read MCP010B raw probe 仍只作为中间 receipt 保留；当前 source manifest 应由 MCP010C–F 的 raw probes 校验 33 个只读工具和 18 个写工具。两者都是传输层证据，不等于 required Codex Desktop/CLI 宿主 E2E，也不把 IDE 变成当前 P0 Gate；MCP005 的 reference CLI admission 另由 `script/test_mcp005.sh` 和对应 Codex receipt 覆盖。
