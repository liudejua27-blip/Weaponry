...
2026-08-15 最新源代码覆盖：当前为 120 Schema、38 read + 30 opt-in write = 68 tools。新增只读 `visual_surface_get`/`VisualSurfaceResult@1`，只做 candidate-bound AOV/轮廓诊断投影，surface backend 为 `NOT_RUN`；`repair_apply_confirm` 为单视图 Repair apply intent 的 fresh-approval/lineage revalidation/immutable-version 消费入口；`design_action_optimization_proposal_prepare` 只把严格且 non-regressing 的 ActionRun-bound CADFit 结果物化为独立 review candidate，不自动 Repair/Confirm；多视图仍必须走 `cross_view_promotion_confirm`。合同、Visual Surface negative Gate、Stage0 静态检查、Rust focused test 与既有回归均已通过；不改变真实机器人视觉失败和 benchmark 阻断事实。

计数校正：下方生成时快照中仍出现的 115 Schema、37 read + 28 opt-in write = 65，不覆盖当前 Stage 0；当前交付口径统一为 120 / 38 + 30 = 68。
...
Stage 0 当前交付口径：118 Schema、37 read + 30 opt-in write = 67 tools，唯一 `in_progress` 为 `FGC-MCP010F`；机器证据入口为 `docs/evidence/mcp010f/current-benchmark-truth.json`。Agentic observe/plan/critic/evidence projection 证据为 `docs/evidence/mcp010f/agentic-runtime-observe-plan-20260813.json`，真实 Runtime 嵌套只读 projection conformance 证据为 `docs/evidence/mcp010f/agentic-runtime-projection-conformance-20260813.json`，由 `scripts/check_agentic_projection_receipt.py` 校验；durable session/checkpoint/RepairIntent prepare/readback 证据为 `docs/evidence/mcp010f/agentic-runtime-session-checkpoint-20260813.json`，由 `scripts/check_agentic_runtime_receipt.py` 校验；显式 bounded multi-view authoring、逐视图 evidence inventory、`CrossViewEvidenceBundle@1` Promotion boundary 与 geometry-stage ActionRun 证据为 `docs/evidence/mcp010f/agentic-runtime-authoring-context-action-boundary-20260814.json`、`docs/evidence/mcp010f/cross-view-evidence-boundary-20260814.json`，promotion transaction boundary 证据为 `docs/evidence/mcp010f/cross-view-promotion-transaction-20260814.json`，composition proposal/merge-chain、`repair_apply_prepare` apply-intent 和 `design_action_optimization_proposal_prepare` continuation 由 Runtime/MCP focused tests 覆盖，bounded independent stage batch 由 Runtime/MCP focused tests 覆盖。上述证据证明 bounded producer/readback、单动作 reviewable proposal、独立 batch stop/replay、composition stop/replay、cumulative merge-chain validation、CADFit strict-improvement proposal boundary、Repair apply source/proposal revalidation 和跨视图 source/validator/promotion fail-closed boundary；不代表 positive merge、跨视图同 cohort render/compare、完整组合式多动作 orchestrator、Repair 实际应用或视觉 PASS。attempt35 仅是 provisional retained observation，不具备 benchmark 资格：它是 `QUALITY_TARGET_NOT_MET + INCOMPLETE_TRUTH_BINDING`，eligibility 为 `BLOCKED_INCOMPLETE_BINDING`，camera-fit `354caf27…f95788` 与 reference-compare `8cd20605…a535` 为 `MISMATCH`；current-cohort packaged Viewer CLI read-model 已绑定 lineage，但正式 UI/accessibility E2E 尚未运行。source/raw/transport/build/AX smoke 不能替代视觉、人评或 packaged E2E Gate。
...
ADR-0026 与本轮架构重规划补充：后续高质量交付必须先建立清晰模块边界和废弃隔离，再把 projection 和 durable prepare/readback 推进为有完整 producer conformance 的 ReferenceCanvas、DesignSpec、SemanticSceneGraph、DesignSession、Visual Evidence 和 Critic/Repair loop。当前 Runtime 的嵌套只读 projection conformance 已通过独立 checker，ReferenceCanvas/DesignSpec 已有 bounded explicit multi-view producer/readback，但仍缺完整独立 schema conformance、跨视图视觉证据和最终 Repair loop；该目标不改变当前 MVP done 状态，bounded single-action ActionRun 仍不等于完整多动作 orchestrator 或 Repair promotion。

2026-08-14 当前口径校正：下文历史段落中的 106/107/115/65 计数属于生成时快照；当前为 118 Schema / 37 read + 30 opt-in write = 67。当前已新增 bounded cross-view evidence boundary、2–6 项线性 ordered composition proposal、hash-linked `cumulative-program` merge prepare、`repair_apply_prepare` CAS-backed apply-intent、`repair_apply_confirm` 单视图消费边界和显式 fail-closed promotion transaction；Boolean request/result 与 product-owned bounded Manifold Worker 也已通过 D source Gate，但 positive merge、Repair 实际应用、promotion 正向成功和视觉 PASS 仍未完成。
...
当前完成证据：`packages/forgecad-skills/registry.json` 保留历史 `0.1.0` Skills，并新增 `primitive-blockout@0.2.0`、`hard-surface-detail@0.2.0` 和 `uv-pbr@0.2.0`；Bundle metadata、Runtime Skill integrity 和 source Gate保持既有范围。当前源码共 115 contracts；MCP010C fixed renderer/九 AOV/reference compare/review raw Gate、MCP010D Operator/Boolean Worker、MCP010E AssetPack/UV/PBR/MikkTSpace raw Gate、MCP010F Viewer source/轮廓目标/`CameraCalibrationRef@1`/相机拟合/边界误差 Gate，以及 Agentic durable session/checkpoint/RepairIntent prepare/readback、逐视图 evidence inventory、bounded stage batch、composition merge-chain 和 Repair apply prepare 分别记录在对应 source/focused evidence 中。正式 distribution signature/revocation、xatlas/Validator、Viewer package 和真实几何/视觉 benchmark不属于 MCP006，分别留给 MCP012–013、MCP010F；不得用 Skill metadata代替 producer。
...
- `FGC-MCP010D`：受限高细节 Operator、固定 Manifold C API 的同一 Part bounded union/difference/intersection Worker adoption 和 geometry Skill `0.2.0`；
...
ADR-0026 后续重构 backlog 不插队改变当前任务链。当前 durable session/checkpoint/RepairIntent prepare/readback、bounded multi-view authoring/readback、逐视图 evidence inventory、single-action geometry ActionRun 和 bounded independent stage batch slice 已完成，但不改变 F 的唯一 `in_progress`；后续仍应拆分为：剩余 producer/consumer conformance、跨视图 render/compare、组合式多动作 orchestrator、Repair 应用、Parametric Design Kit、完整 Visual Evidence、Critic/Repair loop 和真实 stage-gated robot loop。
...
当前源码以 manifest/目录计数为 115 个 JSON Schema；历史 B/C/E/F subtotal 不能再相加作为当前总量。`operator_catalog_get`、`geometry_program_hash`、`material_pack_get`、`render_pass_get`、`silhouette_target_get`、`camera_fit_prepare`、`silhouette_fit_prepare`、`part_contour_fit_prepare`、`silhouette_part_error_get`、`silhouette_candidate_compare`、`boundary_error_get`、`session_get`、`checkpoint_get` 以及 Agentic observe/plan/critic/evidence projection tools 是默认只读工具。B source Gate 已通过 contracts、Skill integrity、Worker isolation、V2 restore hardening 和 closed GLB profile；D 已通过 product-owned bounded Manifold Boolean Worker；E source Gate 已通过 AssetPack manifest/provenance、512px UV atlas、fixed mikktspace、embedded PBR and nine AOV；F source Gate 已通过 hash-bound contour target、Runtime-owned camera ref、bounded 64-render coarse-to-local camera search、Rig/SDF/Part/candidate compare 和 directional boundary error；Agentic projection、bounded multi-view authoring/readback、逐视图 evidence inventory、single-action geometry ActionRun、bounded independent stage batch、ordered composition proposal、显式累计程序/父哈希链 merge prepare、`repair_apply_prepare` apply-intent 与 durable prepare/readback 已通过隔离/focused probe，但正向 composition merge、Repair 实际应用仍未完成；Darwin OS memory hard cap deferred/NOT_RUN。Codex 先读取 catalog，再提交严格、无 `canonical_sha256` 的 V2 draft 到 hash 工具；Runtime 返回唯一 canonical hash，且不编译、不创建 candidate/Job、不写 SQLite/CAS。
...
当前源码 manifest/目录共有 115 个合同，默认工具面为 37 read + 28 opt-in write = 65。`script/test_mcp010c.sh` 已通过固定 512×512 perspective/z-buffer、scene transform、确定性九 AOV、CAS RenderSet@2、local reference mask/metrics、`render_pass_get` MCP image block、Codex typed review、human receipt schema 和 deterministic raw stdio；`script/test_mcp010d.sh` 还通过 product-owned Manifold bounded Boolean union/difference/intersection Worker 与 strict readback；`script/test_mcp010e.sh` 已通过 AssetPack/provenance、512px atlas、fixed mikktspace、embedded PBR textures、strict readback 和九 AOV image block；`script/test_mcp010f.sh` 还通过哈希绑定 silhouette target、`CameraCalibrationRef@1`、64-render coarse-to-local camera fit（37 个粗候选 + top-3×9 局部探针；候选排序在 64px 内部二值栅格/128px 合同输出完成，最终指标回到 512px）、bounded Rig/SDF/Part/candidate compare、directional boundary error、只读 Viewer source checker、TypeScript/Vite/Tauri 构建和 IPC write-boundary negative；Agentic projection、逐视图 evidence inventory、durable session/checkpoint/RepairIntent prepare/readback、bounded independent stage batch、ordered composition merge-chain 与 `repair_apply_prepare` apply-intent 另通过合同 checker、preflight 顺序、空 reference fail closed、Runtime/MCP 重启、focused Runtime/MCP tests 和隔离持久化 probe。上述仅为 source/raw/build/readback/focused orchestration 范围。真实机器人 attempt35 虽完成 11-turn transport，但为 `QUALITY_TARGET_NOT_MET + INCOMPLETE_TRUTH_BINDING`；fit/compare camera `MISMATCH`，最新 `real-codex-cli-current-20260814-viewer-bound.json` 已补齐 current-cohort packaged Viewer exact project/candidate/artifact/reference/render-set/comparison lineage read-model binding，但不改写 attempt35 的视觉结果，详见 `docs/evidence/mcp010f/current-benchmark-truth.json`。这些 receipt 不创建 confirmed version，也不构成 `PARTIAL_VISIBLE_VIEW_PASS`；packaged Viewer UI/accessibility、人评阈值、真实 PBR likeness、xatlas/Validator、export/restart hash 和 HQ_360 仍 `NOT_RUN/BLOCKED`。单张三分之四图最多只能产生 `PARTIAL_VISIBLE_VIEW_PASS`，且必须先通过阈值。
...
出现于清单不等于已安装。每项必须在对应任务固定精确 tag/commit、LICENSE hash、依赖 SBOM、恶意输入/资源/确定性 Benchmark 和移除方案；Luna 不得运行仓库安装脚本或整仓复制。用户已授权 build123d、BlenderMCP、CadQuery、Manifold、MaterialX 的选择性源文件研究；其中 Manifold 已完成 accepted product-owned isolated Worker adoption，其他项目仍按 `LUNA_GITHUB_REPLICATION_PLAYBOOK.md` 进入隔离缓存和 `research-authorized` receipt，不能修改 lockfile、安装包、Runtime allowlist 或 active Skill。
...
| [Manifold](https://github.com/elalish/manifold) | vendored isolated Worker source | robust mesh boolean/manifold | MCP010D | fixed C API/FFI、限制面数/时间/source IDs、MeshGL64 readback；accepted 后支持同一 Part union/difference/intersection，任意 mesh 仍 unavailable |
