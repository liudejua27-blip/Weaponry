...
| `surface_adornment` | “给外壳增加雕刻纹样、流线和细微图案” | 仅选择已审核的 visual-only Detail Skill、Material Zone 与受限 detail grammar，形成预览 | 确认后建版本 |
...
ForgeCAD 的 `allowed_tools` 是严格限制，不是“免确认列表”。Product Tool ID、ShapeProgram operation、Recipe ID 与材质 ID 属于不同命名空间，不能把它们直接求交集。有效调用先由同一命名空间内的 Product Tool 授权决定：
...
- 零基础用户 5 分钟内查看 Agent 生成并通过硬门的唯一结果、完成首次修改和首次 GLB 导出；不要求先理解或选择三方向。
