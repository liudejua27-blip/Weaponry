...
2026-08-16 current real Codex ReferenceCanvas/Observation/Primary Form truth：当前源码 cohort `70badfd9d4e07a374aee994ad42604980ccf77e3f572b6d2926b46eccca6f72a` 的真实 Codex boundary probe 已返回 `PASS_BOUNDARY_ONLY`。Runtime 默认 authoring 产生并持久化 `ReferenceCanvas@1`/`DesignSpec@1`，`AgenticSessionResult@1` 的 observation hash 与同 candidate 的 `AgenticSceneObserveResult@1.canonical_sha256` 精确一致；receipt 为 `docs/evidence/mcp010f/supplemental/real-codex-cli-current-20260816-reference-canvas-observation-pass-v3.json`。同一 final candidate 上，Runtime-owned `pelvis → upper-arm-pair` 两步 Primary Form repair 均为 `accepted + strict_improvement`、64 evaluations/step，loss 分别 `0.406576965276 → 0.403114748765`、`0.403114748765 → 0.400487759484`；composition lineage、target/camera/Rig/observation binding exact，receipt 为 `docs/evidence/mcp010f/supplemental/real-codex-cli-current-20260816-primary-form-runtime-fallback-20260816.json`。最终 boundary-only compare 为 IoU `0.752731669182`、Boundary F1 `0.355021780326`、bbox edge `0.00390625`、centroid `0.001753436389`；silhouette gate 仍 `QUALITY_TARGET_NOT_MET`，`quality_visual_status=NOT_RUN`，没有 AOV/review/confirm/version/export/PBR/人评证据，不能升级为 likeness/HQ PASS。
