...
2026-08-15 current-source package truth：提交 `a153dc02` 已重建并安装同源码 Dev.app，MCP/Runtime/Geometry Worker/Render Worker exact cohort 为 `04bbb4d6…ce81f3`；ad-hoc deep-strict、隔离 Runtime Ready/project/preflight 与 packaged MCP010B raw stdio structural Gate PASS，Skill registry 以 checked-in registry 精确为 12 项，GeometryProgram@2 strict readback/candidate binding/负向 Part sink PASS。现有 Codex Desktop MCP session 仍绑定旧 `7f9e4c…ee518`，故 `live_session_rebind=BLOCKED_RESTART_REQUIRED`；本次没有参考字节、candidate、RenderSet 或新的视觉质量结论。权威质量继续为 `QUALITY_TARGET_NOT_MET`、camera `MISMATCH`、`BLOCKED_INCOMPLETE_BINDING`，人评/PBR/export-restart/360 未运行或阻断。
