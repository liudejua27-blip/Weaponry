...
2026-08-14 Render Worker process boundary 修复：Runtime `render_fixed` 先经 Geometry Worker 生成编译 GLB，Render Worker 的窄 façade 拒绝 geometry compile payload；隔离单测、compiled-GLB 负向 raw probe、MCP010C focused gate 和 Runtime fixed-render regression 均通过。该原子修复只把 process/protocol boundary 记为 PASS；`forgecad-render-worker` 仍复用 geometry-worker renderer source，crate-level source extraction、packaged/live C/F、真实视觉、人评和 360 仍保持 OPEN/NOT_RUN。
