...
2026-08-15 `FGC-MCP010F` canonical ReferenceViewSpec → Primary Form binding：`silhouette_fit_prepare`/`primary_form_repair_prepare` 接收可选 hash-bound `ReferenceViewSpec@1`，校验目标 reference/project scope，并让 transient ranking 与最终 same-camera staged compare 共享 exact landmarks/regions；旧调用保留 landmark-only fallback。Runtime/MCP focused regression、contracts、source/package `mcp010f:test` full Gate PASS；该修复尚未产生新的真实视觉 receipt，F 仍唯一 `in_progress`，质量继续为 `QUALITY_TARGET_NOT_MET`、camera `MISMATCH`、`BLOCKED_INCOMPLETE_BINDING`，人评/PBR/export-restart/360 未运行或阻断。
