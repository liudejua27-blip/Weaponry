...
- 运行时操作白名单、实际 GLB 编译/回读质量、ProfileSketch、增强 Extrude/Revolve、受限 Loft/Sweep、单一 Manifold Python CSG，以及几何侧 edge finish/UV0/tangent/稳定 face→part/zone 事实已由 G819/Q003/G820–G826 实现。A003/A004 已实现 DeepSeek Provider 可观察性和受限单 Turn Product Tool Action Loop；F025/D005 已完成 Agent-first/legacy 隔离及四领域语义比例配方。G826 将稳定 face/source-face ID 写成 GLB 顶点属性并从同一产物回读，但完整纹理、多区 PBR 和展示环境仍须由 M108 实现；内部多候选唯一最佳结果、Codex 式工作台和专属 Skill 仍是目标设计。原 `FGC-V002` 已被不显示三方向选择的 `FGC-V003` 取代，不得在当前 Alpha 中宣传为已支持。
...
- Khronos glTF-Validator：锁定的开发/CI 标准合规门；glTF Transform core/extensions：锁定的开发评估依赖。四领域评估已证明其 writer 虽保留所需 Part/zone/material 映射，却会重编号资源并改变 ForgeCAD readback，故禁止作为生产 export transform；functions 的 dedup/prune 与 KTX2/BasisU 仍未通过当前平台/编码/解码 Gate；
