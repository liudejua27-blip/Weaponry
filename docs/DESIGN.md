...
恢复演练层不得实现第二套复制逻辑。`ForgeCADLibraryRecoveryDrillReport@1` 复用上述 backup/verify/restore 函数，对静止源库执行 1–10 轮，并用恢复库启动真实 Agent，回读 Project/Version/Module、下载全部注册 Module GLB 并验证 hash。每轮记录数据库与对象集合指纹、容量、耗时、吞吐和完成目录大小；跨轮指纹变化必须失败。报告默认不保留副本、不记录源绝对路径，只在显式 `--retain-artifacts` 时保留相对 artifact path。正式资产证据除 10–12 个 Module 外还必须拒绝已知 reference/smoke generator，但自动检查不能替代人工最终资产审阅。
