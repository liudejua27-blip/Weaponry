...
当前 R2 已实现 `ConceptExportManifest@1` 源包：ZIP 中包含不可变模块 GLB、WeaponConceptSpec、ModuleGraph、可选最新质量报告、README 和逐文件 SHA-256；数据库保存 package asset、artifact link 与 completed JobEvent。它用于证明版本可追溯和资产不被覆盖，不等同于已经完成的组合模型。
