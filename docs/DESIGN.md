...
2026-08-01 第一阶段把“在唯一工作台稳定展示高质量 3D 结果”置于导出之前：导出真值链继续保留，但首屏、主要 CTA 和 U004 退出条件以参考理解、完整主体、macro/meso/micro 表面细节、Material Zone/UV/tangent/五通道 PBR、同一 renderer 验收和唯一结果为中心。DeepSeek 多轮/缓存、Appearance Compiler、工作台布局和四 Luna 文件分工统一见 [U004 第一阶段高质量工作台总图](U004_STAGE1_HIGH_QUALITY_WORKBENCH_PLAN.md)。该顺序调整不代表当前已经达到目标图质量。
...
U002/U003/U004 当前实现边界：`UniversalAuthorRequest@1` 由 Rust 从真实 Turn/Project/Snapshot、sealed evidence、选择/锁定和代码所有 capability manifest 构造；Provider 一次返回 profile/feature/plan/outcome，新项目首轮只可调用 `author_universal_asset`。除经过审查的 `procedural.robotic_arm_visual_v1`、`procedural.generic_hard_surface_v1`、`deformable.local_lattice_shell_v1` 及其逐部件本地硬表面 Hybrid 外，开发期本机还开放 `procedural.generic_visual_exterior_v1`，由 Rust 代码所有的 box/cylinder/capsule/wedge/bevel 组合为车辆、家具、建筑、植物、动物、角色、机械设备、虚构游戏道具和未知对象的轻量外观代理；它不等同照片级重建、角色/生物专用形变或正式跨类别质量。lattice 只允许既有硬表面程序化源的固定 `2×2×2` cage 外壳形变，Hybrid 要求 Rust 将每个 deformable Part 与 sealed plan、terminal lattice operation、offset、Part binding 精确重算，未声明部件继续保持程序化终端。mesh seed、任意 mesh 和未登记专用表示仍不可用。limitation 是正常、零几何副作用结果。可执行结果的 `UniversalAssetSource@2` 只能由 Rust 从已验证合同和程序 revision 派生，编译后精确绑定 ShapeProgram、GLB、语义/编译 readback、renderer 和固定视图；无拟合证据的相机保持 unresolved。

本机开发代理的自由度边界：`procedural.generic_visual_exterior_v1` 保持固定的语义部件角色和 Rust 几何预算，但使用完整用户描述与类别的确定性哈希改变每个部件的受限比例、位置、旋转和倒角，并把候选 seed 纳入 program identity。因此不同描述不会复用同一固定轮廓；这只是本机候选多样性增强，不是视觉模型、照片重建或专用角色/生物表示。默认产品运行仍应走 DeepSeek 文本 author 与千问视觉理解/比较；`FORGECAD_LOCAL_VISUAL_AUTHOR=1` 只允许开发验收显式启用本机代理。
...
U004 当前只建设自主可控的本地表示：千问从 sealed 图片提取 `observed | inferred | unknown` 视觉证据，Rust 封存合同，DeepSeek 编写受限 `ForgeVisualProgram@2`，本地编译器负责 procedural/deformable/local-hybrid 几何与 Appearance。工作台参考图只向同一个 Universal author Turn 提交 `author_context`；旧视觉面板生成的 `VisualEvidenceGraph@1` 在 SubjectProfile 产生后由 Rust 按 macro/meso/micro 投影为绑定 request/profile 的 `VisualEvidenceGraph@2`，不再让 Universal candidate 同时进入 Legacy multimodal comparison source。当前 P0 在 GLB/readback 后将 Turn 置为 `waiting_for_capture`，同一工作台 renderer 的八视图 evidence 被 Rust 一次性采用后才恢复 evaluate；同一 capture 的 silhouette/normal/depth/part-ID/material-ID 五个 auxiliary tile 还会由 Rust 提炼 `CandidatePbrAuxiliaryQuality@1`，作为 UAS@2 `gpu_auxiliary_semantics` hard gate 与 render fingerprint 的瞬态证据；它只验证 GPU 证据完整性，不替代千问相似度或真人评分。失败时只允许一次 DeepSeek typed patch，patch 后必须以新 GLB 重新 capture，第二次失败终止。该内存 continuation 不跨重启恢复。通过此链的非游戏 `UniversalAssetSource@2` preview 使用独立 `NativeUniversalPreviewProvenance@1`，Rust 在确认前重验 request/source hash、Project/Turn、representation direction 与精确 GLB，随后沿现有单 GLB 原子版本事务持久化该 UAS provenance；它不创建旧 V003 decision，也不使用 Domain Pack/C111 填充身份。每个新 capability 仍必须具有 Part/Zone、严格 readback、固定输出多视图比较、confirm/version/Snapshot/export 和跨类别净质量证据后才能改为 available。
同一条 P0 capture 还必须绑定 Rust 发放的 `visual_environment_id/hash` 与固定 `render_manifest_sha256`。Tauri 不接受前端自报 renderer 之外的环境真值；上传的每张图必须回传当前工作台环境身份并与 session 精确相等。Universal comparison 输入携带 `VisualReferenceRenderContract@1`，由 Rust 重验 `forgecad-workbench-pbr@1`、`env_forgecad_room_studio_v2`、sRGB、ACES Filmic 和固定 manifest；该合同缺失、漂移或篡改时，图像不能进入千问比较、patch、preview 或版本路径。Legacy E005/C111 comparison 保留旧输入兼容，不被这条通用合同静默改写。

游戏资产交付不是新的网格来源：当 Rust-sealed `GameAssetProfile@1` 已在 UAS@2 编译前确定，受限 worker 先产出可视觉验收的 source LOD0 GLB，Rust 再以同一 Part terminal binding 派生 `MSFT_lod`、off-scene collision、socket 和实测 texel-density 的 delivery GLB。source GLB 始终是视觉比较及 readback 真值；UAS 只保存 hash-bound delivery receipt。游戏确认通过专用双工件原子事务同时保存 source LOD0、delivery GLB 和 interactive preview，`production_glb` 继续指向 source 以保证质量与编辑真值不变，`game_delivery_glb` 才是用户交付物。`:model.glb`、`:export` 与 `ForgeAssetPackage@2` 必须重新校验 source/profile/binding/receipt 后仅消耗对应 delivery bytes，并同时披露 source hash；缺少 profile、readback、receipt 或任一 object role 时 fail-closed。当前已有 desktop app-server 的确定性有效 GLB fixture 完整覆盖 source capture→preview→confirm→export/package，receipt 篡改验证、确认重放幂等、source/delivery CAS 存储漂移拒绝和确认后 ChangeSet 的 native compatibility `:preview → :preview.glb → :confirm` 全路径回归也已通过；仍缺 packaged 设备 loader、真实千问比较和用户级视觉质量证据。

## ForgeVisualProgram@2 高自由度编译架构（目标）

在当前 generic hard-surface 实现中，Appearance Compiler 会将 Rust sealed 的部件材质特征、视觉特征描述、显著性和目标通道编译为受限且可复现的 `SurfaceLayerProgram@1` motif（例如 relief、覆盖范围、粗糙度和条件式发光）。这是一层 feature-conditioned lowering，不是 Provider 自报材质、任意 shader 或照片纹理恢复；新 motif 必须通过 Core lowering、Worker PBR 输出和 readback 回归后才能进入候选链。

U004 P1.4 将几何细节语言中的 `surface_panel` 扩展为六面局部附着：源盒体或倒角盒体的 `±X/±Y/±Z` 面均可承载一个受限 panel，面内位置可偏移，法向位置由 Rust/Worker 根据源尺寸和 panel 厚度派生。轴向、尺寸、法向偏移和源类型由 Rust 与 Python 双重校验，保持同一 Part/Zone/readback lineage；这解决侧装甲、侧检修板和垂直饰面必须堆叠盒体的表达缺口，但仍不是任意曲面投影、自由网格编辑或角色/生物表示。

P1.5 将同一局部几何语言扩展到“倒角盒体作为壳体源”：`shell` 可直接消费 `box`，也可消费已通过 `bevel_approx` 的盒体，从而让装甲罩、设备外壳和游戏道具护罩保留圆角边缘并生成真实闭合薄壁。倒角半径由 Rust/Python/Worker 统一限制为源盒体 X/Z 最短边的 `25%` 以内，shell 厚度还必须为内腔保留 `2r` 净空；Worker 仍只使用本地 Manifold CSG 和既有 ShapeProgram，不接受任意 offset、任意 CSG 或外部 mesh。这个组合提升外观质量而不创建第二 DSL，但仍是受限硬表面能力，不等同通用 CAD 壳体、曲面 offset 或跨类别表示。

P1.6 增加受限 `Groove` 几何语言以表达真实面板凹槽、检修槽和散热嵌槽。它不把任意 subtract 暴露给 Provider，而是只允许一个直接盒体/倒角盒体源、一个 `±X/±Y/±Z` 面、二维面内 footprint、面内偏移和法向深度。Rust 将其展开为固定 cutter box 与 `subtract`，cutter 在源面外溢 `0.1mm` 以避免共面布尔不稳定；法向深度不超过源尺寸 `25%`，footprint 不越界，且静态 CSG 深度/三角预算继续生效。由于结果是真实凹 geometry，GPU/PBR 渲染能产生比单纯 vector path 更可信的边缘遮蔽和高光断裂；它仍不是自由曲面雕刻、任意网格编辑或照片重建。

P1.7 关闭高层几何语言与执行器的合同漂移：公开 `ForgeVisualGeometryProgram@2` Schema、生成 artifacts、Rust lowering 和受限 Worker 必须共同声明当前 20 个操作，且用 operation-coverage fixture 防止新增 Rust/Worker operation 只存在于运行时而无法由 Provider 合法表达。新增的 `local_mesh_patch` 只对较早的审查 mesh 做归一化球形局部变形；基础体和 profile 节点可携带有界静态 Euler rotation，每轴限制在 `[-π,π]`；零旋转不写入 JSON，从而保留既有程序的 canonical identity，越界或非有限值在 Rust lowering 前拒绝。它直接提升装甲、武器、载具部件的姿态组合自由度，但仍是审计过的局部硬表面语言，不是任意 mesh、自由曲面或跨类别通用表示。

可见标识同样遵循证据边界：只有 `Observed`/`Inferred`、声明 `BaseColor` 的特征才能编译为受限 `warning_stripe`、`hex_badge`、`chevron_mark` 或 `panel_label`，固定文字 token 和 UV 布局由 Rust 选择；`Hidden`/`Conflicting` 只进入不确定性，不得改变可见材质。这样能增加局部外观自由度，同时避免把遮挡区域或 Provider 任意文本当成已观察事实。

接缝和饰条同样不是自由 SVG 输入：`Observed`/`Inferred` 且声明 BaseColor、Normal、Roughness 或 Metallic 的特征，才可由 Rust 生成一个固定预算的 `VectorPath` retained layer。路径使用固定 normalized UV 命令进入既有五通道 PBR Worker，并保留 source/retained hash；它可以增加局部面板线、凹槽和轮廓的视觉变化，但不改变 ShapeProgram 几何、不创建第二 DSL，也不代表 CAD 草图、任意曲线或照片投影能力。`Hidden`/`Conflicting` 特征不会生成可见路径。

同一材质区的参考外观允许一个更高但仍受限的融合层：最多两份不同 sealed evidence/camera identity 的 `ReferenceCameraUvRasterBake@2`，分别对同一最终三角形集做相机空间遮挡 raster，再按固定 canonical 顺序对重叠观察 texel 等权平均，未观察区域继续保留程序化底图并记录 mask 并集。`ReferenceCameraUvRasterFusionReceipt@3` 必须封存两份 source/camera/matrix lineage；第三视图、重复身份和旧矩形投影混入均拒绝。该能力只减少同区单视角遮挡造成的外观缺口，不是任意照片拼接、自动相机求解、UV 展开、照片级材质恢复或跨类别表示能力。
...
P2.7 补齐参考外观从 Worker 到 UAS 的可审计回执边界：桌面 Rust bridge 接收 Worker GLB 中的原始 `schema_version`，在放行前重验 imported base-color 的纹理 hash/尺寸、target Material Zone、sealed evidence、camera provenance、world-to-clip 和融合顺序；只输出有界 hash/count DTO。UAS@2 再生成 `ReferenceAppearanceProjectionReceipt@1`，把 source request/program、最终 GLB、compile readback 和 worker receipt 绑定在同一 `AppearanceEvidenceBundle@1` 中。缺少回执或任何 lineage 漂移都不产生 candidate/preview/version 副作用。该回执证明参考图像素确实参与被接受的 PBR artifact，不把 lineage 误报为 Qwen 相似度、照片级重建或真人质量。

P2.12 补齐“回执不等于像素”的最后一个 Rust 边界：bridge 不再只信任 Worker 的 `visual_texture_sets` 和 projection receipt，而是从 exact final GLB 独立解析 `material → baseColorTexture → image → bufferView`，重新计算 embedded PNG 的 SHA-256、字节数和尺寸，并对 `unobserved_texel_mask` 做同样校验。核心 GLB readback 仅允许 base-color 使用 `imported_reference/unknown`，metallic-roughness、normal、occlusion 和 emissive 仍必须保持内置来源。U004 通用 sealed-image fixture 也改为在 GLB 中嵌入与 receipt 一致的 base-color/mask PNG；`desktop:u004-universal-image-bridge-e2e` 因此证明最终 GLB 真含有回执绑定的像素，而不是只证明 metadata lineage。该切片仍不证明视觉相似度、照片级重建、真实千问、packaged GPU 或跨类别质量。

P2.13 增加 Rust-owned 低维轮廓占用验收：sealed 参考图在候选相机拟合时可由同一份原始字节派生 16 个水平前景占用采样，候选则由同一 GPU auxiliary silhouette tile 派生相同采样；只有包围盒与中心误差、且在两侧 profile 都存在时的轮廓误差同时通过，才接受离散 `SilhouetteFit`。profile 不是像素 mask、纹理或视觉相似度分数；旧 fixture/缺失 sealed bytes 仍只走已标明上限的包围盒兼容路径，错误长度、越界值和显著轮廓冲突 fail-closed。该切片提升相机和后续 UV/PBR 外观编译的视角稳定性，不证明照片级重建、真实千问、packaged GPU、未见输入或跨类别质量。

P2.14 增加 Rust-owned 参考/候选低维视觉指标：在同一 universal PBR capture 的 exact sealed reference 与 GPU beauty/auxiliary bytes 上，Rust 按候选视图计算 16-bin silhouette profile error、foreground bounds IoU、dominant color bucket overlap、brightness/edge-density agreement，并选择确定性的最佳视图。摘要只保存 availability、bounded metrics、failure codes 和摘要 hash，不保存像素，也不替代千问语义比较；可解码且轮廓/包围盒明显偏离时，失败码合并进入 `VisualReferenceConvergenceEvidence`，阻止预览。旧的最小测试 PNG 只产生 `not_available` 诊断，不被误报为质量通过。
...
- 真实 Provider truth set、持久化运行恢复和正式审阅纹理资产仍未完成；M108A 已建立 preview/production 双档五通道 PBR、同源 GLB/视口/readback、质量/导出和派生缓存自动门，但 M108B 的 Recipe 驱动独立人工视觉基准尚未完成；U004 已将候选验收和 macOS Agent 资产概念视图绑定到工作台同一 GPU/PBR renderer，Python `forgecad-agent-software-raster@1` 仅保留为浏览器兼容诊断路径，不得满足通用质量门；R002–R004 的软件概念渲染与 PNG/manifest 图包已完成，但不等于工程渲染、装配或制造说明；
...
├── ThreeViewport（唯一 WebGL canvas；中央 standard / focus 二态，状态字段兼容 docked | focus）
├── SelectionCard
├── ConfirmationBar
├── ComponentDrawer
├── MaterialDrawer
...
展示最低要求：固定可复现的 HDRI/工作室环境、物理正确灯光、线性色彩工作流、统一 tone mapping、接触阴影和受控环境遮蔽；不得用强轮廓线、过曝高光或随机背景掩盖几何问题。中央 standard 与 focus 使用同一个场景、renderer、相机状态和材质资源。
...
```text
┌─────────────────────────────────────────────────────────────────────────┐
│ 品牌  项目名  自动保存                AI生成  修改  展示  更多/兼容导出 │
├──────────────┬─────────────────────────────────────┬────────────────────┤
│ 项目/最近作品 │ 中央唯一 3D viewport（最大弹性区域） │ AI 助手            │
│ + 新建设计    │ 视角、渲染、选中部件上下文动作       │ 当前步骤展开       │
│ 模板（次要）  │                                     │ 已完成步骤折叠     │
│              │ 完整主体、材质与表面细节             │ 唯一快速修改入口   │
│              │                                     │ 固定输入框         │
├──────────────┴─────────────────────────────────────┴────────────────────┤
│ 仅在有结果后出现的版本/修改历史                                        │
└─────────────────────────────────────────────────────────────────────────┘
...
中央与 focus 不是两张视图；状态机只切换 `viewport_dock = docked | focus` 并扩大同一个 canvas 容器。`>=1280px` 使用 264px 左栏、弹性中央、360px 右栏；`<1024px` 左栏和助手变为互斥 drawer，不能同时遮住 3D。底部历史只在有结果时占位。属性、检查和兼容导出按需打开。用户不选择 Mode、Agent、内部候选、Skill 或 Domain Pack；技术详情只在明确的管理表面出现。
...
### 14.1 在线 author 的高层几何编写契约

`ValidatedUniversalAuthorContext::provider_projection` 向 DeepSeek 暴露只读的 `geometry_authoring_playbook`。它把可执行的 `ForgeVisualGeometryProgram@2` 组织成几条可组合链：基础体或 profile 先形成几何，再通过 `part → material_zone → output` 形成可追踪部件；面板和凹槽只能在直接盒体/倒角盒体的局部坐标中表达；阵列、镜像、放样、旋扫和布尔操作必须引用更早节点，且每个 output graph 必须互不共享节点。

这不是新增模板，也不是给 Provider 执行权限，而是把 Rust 已经存在的高层语言边界压缩成一份可直接遵循的编写规范。规范同时要求先花预算表达 macro silhouette、比例、负空间和身份部件，再表达 meso 分件与 micro 外观；看不见的结构必须保持 inferred/hidden。不同部件可以在同一程序中组合已登记的程序化能力，例如硬表面主体与透明/外观外壳；每个部件仍必须绑定自己的 capability、output、part 和 material zone，不能把能力混成一个无语义的整机模板。Rust 仍重新解析、验证、lowering、readback 和拒绝不合法程序，因此该投影只能改善在线首轮合同通过率，不能替代真实 DeepSeek 生成、千问比较或跨类别视觉质量门。

2026-07-31 U004 P2.15 reference-conditioned Appearance Compiler：为避免参考图只进入比较指标、却不影响外观计划，Rust 从 sealed `ReferenceEvidence.observations.image_surface_facts` 派生 `ReferenceSurfaceAppearanceBinding@1`。分析器新增前景主色桶，编译器将其与 brightness/edge density 绑定到 `GenericHardSurfaceAppearanceCompilation@2` 的 source hash；仅当 profile/feature 没有更明确的材质语义时，才选择有限 color/finish/roughness token。source 重建和一次 typed patch 都重新计算绑定，篡改事实、证据 hash、Project 或 token 会 fail-closed。它改善的是低维、可回读的参考条件 PBR fallback，不是照片材质恢复、自由 RGB、真实 Qwen 或跨类别视觉质量。
2026-07-31 U004 P2.16 reference fallback scope：参考图表面事实是整图低维摘要，不能当作每个 Material Zone 的颜色真值。Appearance Compiler 现在先按显式 feature/material 语义选值，再由 Rust semantic role 与 reviewed base material 决定是否允许 reference fallback；外壳/装甲/外部面板可用，内部结构、accent trim 和 rubber/glass/emissive/signal 等特殊区拒绝全局 hint。该策略减少多区 PBR 串色，同时不增加第二套资产真值或 Provider 权限；仍不等同逐区照片材质恢复。
2026-07-31 U004 P2.17 exact observed feature-to-zone appearance scope：P2.16 的材质语义作用域进一步绑定到 `ReferenceAppearanceBinding@1`。`with_reference_surface_facts` 先从同一 sealed request 生成 observed、appearance-bearing feature→evidence/view→Subject Part/Material Zone 映射，Appearance Compiler 只有在当前 zone 命中该映射时才读取低维 surface facts；同 Part 下无 observed region 的 sibling zone 不会被整图 palette、finish 或 roughness motif 改写。该切片仍不保存像素、不开放自由 RGB、不改变 UAS 真值或 Schema 版本。

2026-08-01 本机通用外观代理自由度补强：开发期 `procedural.generic_visual_exterior_v1` 保留开放类别的 SubjectProfile/Part 语义，但不再只输出一份类别固定轮廓。完整 brief 的确定性 seed 驱动受限体块比例、位置、旋转和倒角；角色匹配的部件还可派生受限 array/radial-array（如轮组、四肢、窗格、分枝），材质选择按 reviewed role 与显式外观词绑定到银色/红色/蓝色/石墨/复合/玻璃/橡胶/发光等已有 PBR capability。该增强只证明本地候选不会因输入变化而只改 program seed，并没有把本机代理升级成 Qwen 视觉理解、照片纹理恢复、自由 mesh、角色/生物高保真表示或真人质量门；默认在线运行仍必须经过 DeepSeek/Qwen Gateway 和 Rust 合同。
