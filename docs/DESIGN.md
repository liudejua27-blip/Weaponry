...
- 运行时操作白名单、实际 GLB 编译/回读质量、ProfileSketch、增强 Extrude/Revolve 与受限 Loft/Sweep 已由 G819/Q003/G820–G823 实现；G824A–G824D 已完成候选 CSG 的双平台 packaged/provenance/readback/生命周期证据，ADR-0013 选择 Manifold Python 作为 G825 唯一生产候选。候选仍未接入生产内核。稳健生产布尔、完整表面 provenance/UV/tangent、DeepSeek Provider 可观察性、Agent-first/legacy 隔离、四领域语义比例配方、内部候选评审与单一最佳结果、Codex 式工作台和高真实度 PBR 管线仍是目标设计；实施顺序见 `CODEX_EXECUTION_PLAN.md`。原 `FGC-V002` 已被不显示三方向选择的 `FGC-V003` 取代，不得在当前 Alpha 中宣传为已支持。
...
- Manifold：ADR-0013 已选择 Python 作为唯一实体/稳健布尔生产候选但尚未集成；四领域 benchmark、provenance/readback、隔离取消、真实临时权威状态提升及 macOS/Windows packaged 证据已通过 G824–G824D，仍须由 G825 接入并完成发布审查；
...
当前受限 box boolean 保留为已验证子集。G824 已比较现有 Worker、Manifold Python 与 WASM；G824A–G824C 补齐 macOS provenance/readback、隔离进程、真实临时权威状态提升和 packaged 预算/许可证，G824D 又在 run `29383382978` 的真实 Windows x64 frozen sidecar 内通过同源证据。ADR-0013 已选择 Manifold Python 作为唯一生产路径；G825 负责实际集成，完成前当前产品仍不得宣称稳健通用布尔。任何 boolean 失败都必须返回稳定节点 ID 和错误码，不输出部分模型。
