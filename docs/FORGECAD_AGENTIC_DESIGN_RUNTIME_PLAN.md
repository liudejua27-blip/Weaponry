...
当前源码为 `103 Schema / 37 read + 24 opt-in write = 61 tools`。`RenderSet@2` 已携带 Runtime-authenticated Render Worker cohort/status，Viewer 只读消费；durable prepare/readback 与 Primary Form 窄范围 prepare/evaluate slice 已有各自 focused/source receipt；建议下一批文档/代码任务：
