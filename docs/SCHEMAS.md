...
2026-08-15 最新源代码覆盖：当前为 120 Schema、38 read + 30 opt-in write = 68 tools；新增 `VisualSurfaceRequest@1`/`VisualSurfaceResult@1` 与只读 `visual_surface_get`，以及 `RepairApplyConfirmRequest@1`、`RepairApplyConfirmResult@1` 和 `OptimizationProposalPrepareResult@1`。Visual Surface 仍只投影 candidate-bound AOV/轮廓诊断，完整曲率/特征线 surface backend 明确为 `NOT_RUN`，不创建 candidate/version；`GeometryProgram@2` 已有 `surface-patch@1` open patch 与 `surface-shell@1` bounded constant-thickness shell Worker source Gate，详见两个 MCP010F Surface receipt。CADFit review candidate 仍要求显式 ViewSpec，且不自动 Repair/Confirm；单视图 apply intent 仍要求 fresh approval、lineage revalidation 和 immutable-version 消费；多视图继续使用 `CrossViewPromotionRequest@1`。合同 checker、Visual Surface negative gate、Surface Patch/Shell focused test 与 Stage 0 truth 已通过。

版本：2026-08-13
状态：MCP005–MCP009 functional core 已落地；MCP006 历史 receipt 为 44 个 JSON Schema；MCP010B/C/D/E/F source 合同和 Agentic Design Runtime contract family 已进入当前 manifest，当前源合同总数为 118，其中包含 bounded Boolean request/result、Repair apply confirm 和 CADFit review-candidate continuation 合同。唯一 `in_progress` 为 `FGC-MCP010F`；历史 package/live receipt 仍按 cohort 单独保存。Agentic observe/plan 的真实 Runtime 嵌套只读 projection 已通过 producer/consumer conformance checker；durable session/checkpoint/RepairIntent prepare/readback、默认或显式 bounded authoring_context、逐视图 evidence inventory、`CrossViewEvidenceBundle@1` boundary、cross-view promotion transaction boundary、geometry-stage single-Part proposal、bounded independent stage batch、ordered composition 的 hash-linked `cumulative-program` merge prepare、`repair_apply_prepare` apply-intent boundary、`repair_apply_confirm` 单视图消费边界和 `design_action_optimization_proposal_prepare` review-candidate boundary 已各自有 source/runtime/MCP/隔离重启/focused 证据，但不等于 positive merge、Repair 实际应用、跨视图同 cohort Visual Evidence conformance、完整组合式 orchestrator、MaterialZone/UV-PBR executor 或完整视觉质量。

Stage 0 机器真值为 `docs/evidence/mcp010f/current-benchmark-truth.json`；当前源码口径同时固定为 120 Schema、38 read + 30 opt-in write = 68，并绑定 120 个 Schema 文件内容集合哈希。attempt35 只是 provisional retained observation，为 `QUALITY_TARGET_NOT_MET + INCOMPLETE_TRUTH_BINDING`，benchmark eligibility 为 `BLOCKED_INCOMPLETE_BINDING`，fit/compare camera 为 `MISMATCH`，当前 packaged Viewer CLI read-model 已绑定 current cohort 但正式 UI E2E 仍未运行。Schema/producer 已实现不能补齐缺失 receipt 字段，也不能越过 PBR likeness、正式真人、export/restart 或 360 门。
...
## 2.2 MCP010 与 Agentic 合同（当前源合同 120；历史 44 receipt 不改写）
...
| MCP010F | `ReferenceMaskPrepareResult@1`、`SilhouetteTarget@1`、`CameraFitResult@1`、`CameraCalibrationRef@1`、`SilhouetteRig@1`、`SilhouetteRigHashRequest@1`、`SilhouetteRigHashResult@1`、`SilhouetteFitIntent@1`、`SilhouetteFitResult@1`、`PartContourFitResult@1`、`SilhouettePartErrorResult@1`、`SilhouetteCandidateCompareResult@1`、`BoundaryErrorResult@1`、`VisualSurfaceRequest@1`、`VisualSurfaceResult@1`（内含 `VisualSurfaceReadback@1`/`SurfaceSignalReadback@1`） | Runtime-owned target/mask、bounded camera/Rig fit、hash-only calibration reference、single/multi-Part contour attribution、candidate compare、same-candidate mask/edge/ROI/AOV readback、candidate-bound mesh-derived curvature/feature-line diagnostics |
...
当前 high-quality contract path 固定为 `GeometryProgram@2` detail → `ArtifactReadback@2` strict readback → `AppearanceProgram@2`（受前序门控制）→ `RenderSet@2` 九 AOV → `ReferenceComparisonReport@1` strict compare → `VisualSurfaceReadback@1`（含 `SurfaceSignalReadback@1`）→ `DesignCriticReport@1` → `VisualReviewReport@1` / `QualityReport@2`。`VisualSurfaceRequest@1`/`VisualSurfaceResult@1` 是该路径上的只读诊断投影，不是第二套质量真值；它要求 candidate/reference/artifact/RenderSet/camera/compare/quality binding 同源，backend 可为 `candidate-bound-aov-diagnostics@1` 或 `candidate-bound-surface-analysis@1`。后者只提供从已通过 GLB readback 解码的 triangle-dihedral curvature proxy 与 boundary/crease feature-line summary，不是 SubD/NURBS principal curvature、zebra 或视觉质量 Gate。`surface-patch@1` 与 `surface-shell@1` 仍只是 `GeometryProgram@2` 的 bounded geometry source operators，不能把该状态改成 Visual Surface quality PASS。`DesignCriticReport@1.visual_surface` 把 readback canonical hash 传给 CADFit residual revalidation；`[transition-v1]` `GeometryProgram@1` primitive-only、`AppearanceProgram@1` 与 `RenderSet@1` 四 pass 只属于历史兼容，不得提升为当前 high-quality contract path。

## 2.3 ADR-0026 合同与当前落地层级

Agentic contract family 已进入当前 manifest 并通过正/负 fixture checker，但必须区分“合同定义”和“producer conformance”。当前 Runtime 同时提供 `AgenticSceneObserveResult@1` 可重建只读 envelope，以及受批准的 `DesignSession@1`/`DesignCheckpoint@1`/`RepairIntent@1` prepare/readback、bounded `authoring_context` producer/readback、逐视图 evidence inventory、geometry-stage single-Part proposal slice、`DesignActionBatchResult@1` 独立 stage batch、2–6 项线性 `DesignCompositionRequest/Result@1` proposal slice 和 `RepairApplyPrepareRequest/Result@1` apply-intent slice；当请求显式 `merge.mode=cumulative-program` 时，Runtime 还会验证完整 GeometryProgram@2 的父哈希链，并在动作批次成功后编译一个独立合并候选。真实 Runtime 产生的 `AgenticSceneObserveResult@1` 与 `DesignStagePlanProjection@1` 嵌套只读投影已由 `scripts/check_agentic_projection_receipt.py` 对隔离回执完成 producer/consumer 校验；durable 对象已经在 Runtime SQLite/CAS 持久化并经 Runtime/MCP 重启 receipt 校验，stage batch、composition 和 Repair apply focused test 证明 approval/hash/scope/stop/replay/merge-chain/源候选不变边界，但不代表 positive merge、Repair 实际应用、跨视图 Visual Evidence conformance、MaterialZone/UV-PBR executor 或 Repair promotion。隔离证据见 `docs/evidence/mcp010f/agentic-runtime-observe-plan-20260813.json`、`docs/evidence/mcp010f/agentic-runtime-projection-conformance-20260813.json`、`docs/evidence/mcp010f/agentic-runtime-session-checkpoint-20260813.json` 与 `docs/evidence/mcp010f/agentic-composition-proposal-20260814.json`。
...
| `DesignCriticReport@1` | evidence-bound issue、metric、threshold、part/material target、VisualSurfaceReadback canonical hash/binding | Codex typed critic 或 deterministic critic 输出，必须引用同一 candidate 的 evidence/readback hash；CADFit residual 进入前重新校验 |
