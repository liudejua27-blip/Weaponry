...
顶栏只显示四态：`未配置（离线）`、`正在连接`、`已连接`、`需要处理`。点击后诊断抽屉依次显示：
...
错误至少区分：请求格式、API Key、余额不足、参数/模型、请求过多、服务故障、超时/网络、空 JSON、无效 JSON 和 Schema 不符。选择真实 Provider 后失败不得静默回退为“DeepSeek 已成功”；已有资产保持不变，由用户显式重试。
