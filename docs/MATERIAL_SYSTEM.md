...
base_color_texture_asset_id? / normal_texture_asset_id? / normal_strength
...
允许用途：`base_color`、`normal`、`thumbnail`。来源/许可证必须真实填写：内置纹理只能是 `forgecad_builtin/not_applicable`；本人创作只能是 `user_created/self_declared_original` 或未能确认时的 `unknown`；第三方必须是 `imported_reference/third_party` 或 `unknown`，并由作者提供 `license_ref`。系统不会访问 URL、读取任意本地路径或推断许可证。
...
每个通道记录内容 hash、用途、色彩空间（sRGB 或 linear）、分辨率、采样/重复方式、来源、许可证、版本和回退。base color/emissive 使用 sRGB；metallic-roughness、normal、occlusion 等数据纹理使用 linear。网格使用稳定 UV0；normal/clearcoat normal 生效前必须有可验证 tangent space。没有 UV 或 tangent 时明确降级，不能把纹理字段写入 GLB 后仍声称已生效。

glTF 2.0 metallic-roughness 是默认互操作基线。汽车漆、喷涂外壳和保护涂层可使用 `KHR_materials_clearcoat`；纹理压缩可使用 `KHR_texture_basisu`/KTX2，但只有 Three.js、GLB readback、Khronos Validator 和目标平台 smoke 全部通过后才能成为默认。glTF Transform 只用于 inspect、validate、dedup/prune、tangent/unwrap 和经过 benchmark 的压缩，不成为新的材质或版本真值。
...
真实感还依赖一致展示：使用经许可的 HDRI/工作室环境、物理正确灯光、线性色彩工作流、统一 tone mapping、接触阴影和受控环境遮蔽。环境资源同样进入内容寻址与许可证清单。Poly Haven 等 CC0 来源可以成为**候选导入源**，但应用不得违反其 API/网站条款自动抓取；资产需经过显式导入、hash、许可证和离线打包审查。
