...
当前内置集合使用 128×128、材质专属且完全确定性的程序化 PNG。八个 GLB 材质包括 primary、aluminum、signal red、composite、rubber、dark glass、emissive blue 与独立 automotive paint；machined、brushed、coated、composite、rubber、glass、emissive 微表面函数生成 base color、粗糙/金属、normal、occlusion 和 emissive。新生成集合的 texture-set ID 以 `_builtin_v3` 结尾、map ID 含 `_v3_`、`version=3`。v3 在 v2 周期连续的基础上进一步使用高频、低振幅、多尺度细节：拉丝/机加工痕迹、复合材料细纹、橡胶细颗粒和涂层橘皮主要进入 roughness/normal，baseColor 只保留很弱的色差，避免真实 renderer 中出现宽条带、金属波纹或明显棋盘。M108 smoke 解码八种材质的全部五通道，拒绝 8/12/16/18/28/32 px 边界梯度突变；只有 metallicRoughness/normal 必须存在微变化，baseColor/AO/emissive 允许物理上合理的纯色，不以噪声冒充细节。汽车漆虽同属 coated 语法，但有独立颜色、metallic/roughness/normal、纹理 hash/index 和 `clearcoatFactor=0.86`，不会复用 aluminum。

历史兼容按版本精确冻结：v2 的 40 张 map、原 ID 和原字节由固定聚合 SHA-256 `045f788cce7bdb8a83cfa8bbdfec0e554a2914e4637b63ef526ecb136aaab661` 证明可回读；v1 的固定聚合 SHA-256 为 `0b4701fe31946dfc9572990daa5e1e9260d05ddcfcfdef640c9eac776e10b62f`。两者都不会用于新生成。全量加载 v1/v2/v3 时，PNG cache 上限为 24 个材质×版本集合、共 702,750 字节压缩图像，不建立逐像素缓存。128×128 只是当前轻量视觉预算，不代表高分辨率照片纹理或工程表面测量。

readback 不只核对自报 SHA：它按 material index 精确核对 texture-set ID、规范 `texture_material_id`、五个 texture ID、完整 map metadata 与实际 PNG 字节；v3 的五个 texture ID 必须含 `_v3_`，历史 v2 必须含 `_v2_`，v1 仅接受固定历史清单中的原无版本 ID，同一资产实际使用的材质不得混用多个版本。篡改 PNG 后同步更新自报 SHA 仍会拒绝。primitive 的 authored `material_id` 必须命中穷举目录→规范纹理材质映射，未知 ID 或 ID/index 不一致直接失败，不再静默落到 index 0。TextureInfo 固定使用 UV0，当前 writer 不写 sampler 或 `KHR_texture_transform`；readback 拒绝自定义 sampler、非零 texCoord、texture transform、额外采样字段和布尔伪索引，避免 GLB 在字节可信但实际采样效果已改变时仍被接受。正常 v3 首次编译只加载 8 个当前集合，真实读取 v2/v1 后最多扩到 24 个；精确 v1/v2 质量报告可以只做受限兼容迁移，但在当前 v3 合同下以 `stale_compile_readback/unavailable` 隔离。
