...
先运行 `npm run release:packaged-sidecar-preflight` 阅读 P008 报告。它不读取 Provider Key、不联网或执行外来二进制；本轮 macOS arm64 P002 的 frozen sidecar 已由 `desktop:packaged-sidecar-alpha-smoke` 验证真实启动、`GET /api/health`、首次初始化、受限几何归属和重启恢复，且 `provider_calls=0`；`desktop:packaged-tauri-alpha-smoke`、K002 和 K003 packaged native smoke 随后验证真实 LaunchServices `.app`、Rust-owned K001/K002/K003 状态、GLB/readback/render package 和重启语义 hash，且 `provider_calls=0`。这仍是程序化本机 Alpha 证据，不替代新机器安装、升级、签名、公证或外部发布。
