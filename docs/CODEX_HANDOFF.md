...
- 评测包原先只保留空的 `review-responses.json`，无法机械证明未来人工结论没有绕开 PBR 视口或门槛。现新增 `validate_m108_visual_benchmark_scores.py`：它只读取已提交的人工记录，要求 manifest hash、一人一次、至少三位与实现独立的评审者、每人四领域覆盖、`ready/glb_pbr/embedded_pbr_material_count>0`、无 PBR load failure、完整 1–5 分数与三维度中位数至少 4。其 smoke 只用临时合成 fixture 检验规则，绝不产生人类评分或改变 M108 状态。
...
- 已重建 tracked arm64 frozen sidecar，并扩展 `npm run desktop:packaged-sidecar-alpha-smoke`：showcase PBR 的五通道、固定工作室环境、ChangeSet preview/confirm 后导出、undo 恢复初始 PBR GLB、redo 恢复编辑后 PBR GLB、CSG 后 GLB readback 及重启后同 asset GLB SHA-256 全部通过。`agent:m108-visual-benchmark-kit-smoke` 现可生成并核对一份无分数、四领域同源 GLB 审阅包；人工过程与至少三位独立评审的 4/5 中位数门槛固定在 `docs/evidence/M108_VISUAL_BENCHMARK_PROTOCOL.md`。M108 仍是 `in_progress`，直到真实独立评分被收集并复核；不能因此把当前 Alpha 描述为照片级真实产品或解除 C105 阻塞。
