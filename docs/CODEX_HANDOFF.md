...
- R3 现分别断言普通外部参考为 `external_reference/ready`、生成候选为 `compiled_agent_pbr/glb_pbr/embedded_pbr_material_count>0`；失败时写出 stage、load state、GLB kind、render source、状态消息和全页截图到 `output/playwright`，避免 CI 只留下超时行。本机已重建当前 frozen sidecar 与 `.app`，`desktop:packaged-sidecar-alpha-smoke`、`desktop:packaged-tauri-alpha-smoke` 和两轮 Agent-first R3 均通过；远端结果仍以本提交推送后的 PR checks 为准。
...
- `npm run agent:m108-visual-pbr-smoke` 覆盖四领域各 3 个 showcase 多 zone fixture（共 12 个）、确定性重复字节、Schema、五通道、色彩空间、文件预算、环境 hash、clearcoat/透明 extension、缺 normal 与 image hash 损坏拒绝。本轮最终回归已通过 `agent:check`、`contracts:types:check`、M108 PBR/Validator/评测包/每领域评分器、G7、F009、desktop typecheck/build、T002（14/14）、Agent-first R3、packaged sidecar、packaged Tauri、文档 walkthrough、repository integrity、safety scope、secret files 和 `git diff --check`。`release:packaging-readiness` 也已运行并按预期以 `SIDECAR_BINARY_INVALID` 拒绝 Intel macOS、Windows、Linux 的空 sidecar；该生产发布阻断没有被隐藏或放宽。远端仍以推送后 PR checks 为准。
