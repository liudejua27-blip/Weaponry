...
同一 HEAD 的独立 Linux `workbench-e2e` job 随后在 D003 首个页面挂载的 20 秒等待上限失败。D003 smoke 现区分 45 秒服务冷启动与 60 秒页面渲染预算，并在根节点未出现时输出页面 console/pageerror/失败请求、body、Vite 与 Agent 尾部日志；选择领域后的旧“当前临时结果”断言也已收敛到现行 V003 边界：legacy deterministic Planner 必须显示“未返回正式的单一结果决策”、不得恢复三方向或创建第二个 canvas，并保持 ActiveDesignSnapshot 完全不变。浏览器按 Context→Browser 顺序有界回收，本机 `desktop:d3-domain-clarification-smoke` 已通过；最终关闭仍以该修复提交对应的 GitHub `workbench-e2e` 和 ForgeCAD Core 聚合结果为准。

修复 D003 后，本机独立 job 命令链继续发现 F001 仍等待 legacy Planner 的候选、提交与导出；这与 K003/V003 后的 fail-closed 产品合同冲突。F001 现只负责自身的工作台 characterization 边界，并验证领域选择后的 V003 显式拒绝、零 Snapshot 写入、单 canvas、legacy 详情只在显式进入时读取且无 legacy mutation；正式 V003 的 Rust 结果、确认、质量、导出和重启仍由专门的 V003 fixture、R3 与 T002 覆盖，不使用假 Planner 结果替代。最终本机通过 `desktop:build`、D003、F001、R3、T002 14/14、T003（单 context、bundle/heap 预算）和 M108 renderer 四夹具；M108 renderer 报告仍为 `score_status=not_scored`，不得误写成人工视觉通过。
