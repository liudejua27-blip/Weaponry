...
`npm run release:packaged-sidecar-preflight` 会先输出不读取 Provider Key、不联网且不执行二进制的 P008 结构报告；当前 macOS arm64 input 的预期状态为 `ready_for_local_alpha`。`npm run desktop:packaged-sidecar-build` 冻结该 target，`npm run desktop:packaged-sidecar-alpha-smoke` 验证独立 frozen binary；`npm run desktop:packaged-tauri-alpha-smoke` 通过 LaunchServices 验证真实 `.app` 的 `packaged-sidecar`。本轮 sidecar 为 32,610,080 bytes、SHA-256 `c751993f88c86d81de094e379f2393deaf1d2e915687196aeb42e9a835422c3c`，并已通过真实 sidecar alpha 与 Rust ownership Gate；K002/K003 原生双启动还验证 Rust-owned lifecycle/product state、GLB/readback/render package 和重启语义 hash。K003 smoke 对 supervisor 合法动态归属字段保持严格字段集合与格式校验。`npm run k002:packaged-gate` 同时保留 K001 WebView 业务/重启链，并增加 K002 原生双启动：未配置 Provider、`network_call_made=false`、稳定 `PROVIDER_NOT_CONFIGURED`、两个有序 Item、旧 lifecycle POST 410、无持久化 reasoning 且 `provider_calls=0`。这些 Gate 均不自动调用 Provider。`release:packaging-readiness` 仍预期失败：其他发布目标 sidecar、全新机器安装/升级/卸载、签名和公证尚未完成。不得通过删除检查、降低严重级别或改文案绕过。
