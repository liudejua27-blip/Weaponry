...
`r1:gate` 继续执行桌面生产构建和上下文连续性 smoke。`r2:contracts-gate` 只证明首批 Concept Schema、Pydantic 不变量和生成类型一致；`r2:gate` 进一步证明 fresh/repeat migration、Project/Version、不可变 GLB 模块注册、Connector 兼容检查、ModuleGraph 持久化、幂等冲突和重启恢复，但仍不证明 R3 工作台交互或正式模块资产质量。
