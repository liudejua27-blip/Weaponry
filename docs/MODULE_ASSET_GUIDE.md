...
- 镜像修改器必须在最终 LOD0 上应用；左右变化由 ModuleGraph/Connector 语义表达，不能依赖负 scale；
...
它用 9 个带真实 triangle/UV/material 的最小 GLB 验证 dry-run、release 覆盖、批量导入、幂等重放、重启恢复，以及哈希、路径、许可证、Connector 和 pack_id 负向用例。这些 fixture 只证明工具链，不代表正式资产质量。
