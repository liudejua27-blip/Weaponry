...
大模型负责交流、规划、候选评审和“编写模型程序”，不直接返回可信网格；本机负责执行白名单几何操作、验证结果和保存版本。目标输出是生产级概念资产：外观接近真实产品的比例、曲面转折、接缝、纹理、材质和光照，同时保持非功能、可编辑、可回读、可撤销和轻量；它仍不是工程或制造模型，也不承诺照片级或任意提示普遍达标。
...
### P0：生产工件管线与生产级视觉基线尚未同时退出

G819/Q003/G826 已完成，M108A 当前工作区也已建立双档工件：`interactive_preview` 使用 24 段、128×128 v3；`production_concept` 使用 48 段、512×512 v4、平滑 Loft/Sweep normal、`GeometryCompileReadback@2`、production 质量/导出和内容寻址缓存。它解决的是正式 GLB/PBR/readback/传输真值，不再应把输出笼统写成“Alpha 模型”。

剩余 P0 是视觉语言：当前固定 showcase 仍主要由绝对坐标 primitive、Loft 和 Sweep 拼装，没有 C105 Recipe 的 child slot、connector/pivot、局部变换、语义比例和可复用完整部件，因此高分辨率纹理不能单独证明真实、精致和细节丰富。M108B 仍需每领域至少 3 份 Recipe-backed production fixture，并由至少 3 位独立真人对每个领域的 `proportion`、`material_readability`、`surface_detail` 分别达到中位数 `4/5`。

处理：M108A 与 K001–K003 已完成，Rust app-server/core 现单一拥有 Agent/Provider、产品状态与持久化；Python 只执行受限几何。当前主线是 C105→M108B。DeepSeek 只负责任务规划、评审和工具调用；几何、纹理、环境、readback、质量和视觉基线必须由受限资产管线与独立证据证明。参考引导重建继续归 R007，不能用未授权外部资产代替 Recipe 语言。

### P0：三方向把 Agent 评审责任交给用户

当前 Planner 合同固定返回三个方向，工作台再要求用户选择并在每个方向的三项外观中轮换。零基础用户既无法从低分辨率预览判断哪个更完整，也不应承担运行时质量、Brief 覆盖和可编辑性的比较。

处理：ADR-0010 取代 `FGC-V002`。F026 先移除三方向 UI，并只允许第一条 legacy 文本方向适配为一个临时 3D 结果；目标 `FGC-V003` 只进行一次完整合成、真实 Gate 和最多两次同意图原位修复，默认只展示一个通过结果。用户可继续修改或开启“换一个思路”的新 Turn，不展示三张选择卡，也不生成多个完整模型比较。
...
处理：F025 先隔离 legacy 并继续拆薄父层，V003 移除三方向选择，F026 再实施 Codex 式布局：左侧放项目/对话记录与组件库，中心展示会话、步骤和单一最佳结果，右侧持续显示唯一 3D viewport，底部固定输入框并由“+”打开 Style Token、视觉材质和参考入口；点击 3D 时只把同一个 canvas 移到中央 focus，右侧不常驻属性面板。
...
| GPU 推理依赖 | 0 |
| WebGL canvas | 1 |
| 交互预览三角预算 | `interactive_preview` 维持独立轻量上限，不因 production 放宽 |
| 生产概念三角预算 | 受 ShapeProgram 源预算×4 与 250k 绝对上限共同约束，并由真实 readback/GPU Gate 拒绝超限 |
...
- 只配置一个大模型 API 即可交流、生成、修改和导出 production-concept GLB；软件未配置 Provider 时已有项目仍可使用；
- 四个首批领域各至少 20 条固定 Brief 可生成完整、结构不同且可继续编辑的结果，并由 Recipe-backed 视觉基准约束代表输出；
