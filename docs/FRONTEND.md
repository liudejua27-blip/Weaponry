...
桌面应用只有一个入口：**ForgeCAD CAD 工作台**。

```text
Tauri Desktop
└── CadWorkbenchPanel
    ├── 左侧：项目、版本历史、AI 设计助手、参数草案
    ├── 中央：单一 Three.js 3D 视口与 CAD 操作工具
    ├── 底部：组件检视器、方案、版本与 ChangeSet 时间线
    └── 右侧：属性、连接器、质量结果与导出
```
...
2. 生成并选择 A/B/C 受控模块方案；方案只改变已注册模块的组合和允许范围内的变换。
...
- 组件、版本、方案和时间线是同一底部工作区的标签，不是全局页面导航。
