...
| MCP stdio/resources | 已实现（MVP host golden path）；MCP010A lifecycle + Desktop activation PASS | MCP 2025-11-25 canonical + Codex 2025-06-18 compatibility initialize、默认 17 个只读 tools；authenticated/dynamic IPC + explicit env opt-in 时 30 个 tools（17 read + 13 write，含 MCP009 change）；内置 supervisor、Starting/Ready/Degraded/Restarting、Runtime 缺失/崩溃时 stdio survival、resources/list/read/templates、annotations、URI/contract fail closed | Codex Desktop/CLI discovery、connection、read-only E2E、第二次 Desktop 30-tool/Ready/cohort/project readback 和真实 Codex CLI 3D 主链 PASS；attempt 1 FAIL receipt 保留；IDE/其他 Client 非阻塞未来范围 | signed install/config |
...
| 首个硬表面 MVP | host golden path PASS / MCP010A done / visual benchmark OPEN | MCP005–MCP009 focused PASS；MCP010A remediation/rebuild/package/isolated probe/second Desktop activation PASS；010B–F blocked | real Codex CLI full chain PASS；Desktop activation PASS；单图最高 `PARTIAL_VISIBLE_VIEW_PASS`，360 `BLOCKED_REFERENCE_COVERAGE` | 不要求签名分发 |
