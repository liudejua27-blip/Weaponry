...
范围：证明 Tauri 发布门不会将空的“存在即通过” sidecar 当作可发布 Agent。本页不证明 Rust 编译、应用签名、安装/卸载或干净机 C10 E2E。
...
3. 在含 Cargo/Rust 的对应平台运行 `npm run desktop:tauri-check` 和 Tauri bundle，然后完成签名、安装/卸载、无源码干净机的 Brief 到导出 E2E。
