...
2026-07-11 对十模块 Blender visual candidate 运行隔离 Connector 技术矩阵：其仅有的 front 01→02→01 两个 eligible replacement 都完成了 edge Connector remap 与 `1e-6 mm / 1e-5°` 内对齐；八个可编辑节点均完成 X 镜像、combined GLB mirror scale 与 Agent 重启回读。root core 的镜像预览被锁定策略正确以 `CHANGE_SET_INVALID` 拒绝。`--individual-matrix` 将单节点分支汇总为可复现报告：grip/top/side/armor 质量通过；front/rear 分别报告 2 个、lower/storage 分别报告 1 个 `assembly.unconnected_triangle_intersection` warning。连续八镜像压力分支共 8 个同类 warning，说明 Connector 对齐和操作提交本身不保证组合无网格相交；这些 warning 分支不是可交付组合。`--mirror-node <node_id>` 可重跑单节点，输出固定 `evidence_class=unclassified` 与 `formal_asset_evidence_eligible=false`。
