...
版本：2026-07-29
状态：当前工作台视觉回归规格，`formal_eligible=false`。本文件只证明 F026 的布局、交互与单 renderer 边界；A005、R007 和 V003 已有各自工程 Gate，但本文件仍不证明模型质量、通用类别或真实 Provider 成功。
...
- `+` 菜单只提供风格、材质和参考入口。U002 已统一图片/文字/活动资产的理解入口，U003 已建立当前程序化 source lineage，但通用图片生成仍等待 U004–U005；不能把 GLB 导入、source hash 或 typed limitation 冒充参考图重建成功。
