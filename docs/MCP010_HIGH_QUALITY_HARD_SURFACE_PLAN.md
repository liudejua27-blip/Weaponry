...
多视图基线已由用户授权的 perspective/front/back/left/right/rear-three-quarter 六张参考建立：同一 `39e71fd…` source cohort 的 `real-codex-cli-current-20260816-multiview-reference-canvas-observation-runtime-v2.json` 证明 `EXPLICIT_CODEX_AUTHORING`、durable ReferenceCanvas/DesignSpec、六视图 complete/eligible coverage；独立 360 coverage probe 仍为 coverage `PASS`。新的 Runtime receipt `real-codex-cli-current-20260816-multiview-primary-form-runtime-v3.json` 已实际执行 `PrimaryFormMultiViewEvaluation@1`，对 6 视图逐一渲染、比较和聚合；right/perspective 改善，但 front/back/left/rear-three-quarter 退化，proposal 被 `retained_source`，source/proposal aggregate loss 为 `0.530908240794 → 0.541965433208`。因此仍 `QUALITY_TARGET_NOT_MET`、hard gate=false、没有 confirm/version/export；下一项从“接入多视图 acceptance”推进为 Runtime-owned joint multi-view search 与 typed camera calibration，而不是进入 detail/UV/PBR。

上述六视图的左/右标签按用户上传顺序记录；两张侧面图视觉上相近，未进行独立解剖学方向验证。覆盖 eligibility 不等于 likeness、PBR、人评、导出重启或通用 `HQ_360_PASS`。

三部位 supplemental `docs/evidence/mcp010f/supplemental/real-codex-cli-current-20260816-primary-form-chest-forearm-shin-v1.json` 的 Runtime 执行链已完成 3 个严格改善 steps，但 probe 因 2 个未批准的 `unapproved_external_event` 返回 `status=BLOCKED`、`unrelated_side_effects=true`。该 receipt 不得作为 clean Codex transport、视觉 PASS 或 best-known 覆盖；它仅证明这条 Runtime/Worker 路径在阻断前实际跑完了 jobs，仍保持 `QUALITY_TARGET_NOT_MET`、无 confirm/version/export、无持久用户数据。

2026-08-16 current-source Primary Form supplemental evidence（独立于最新 observation receipt）：同一源码 cohort `d52261ffdf44e07a2a4762b16c80f433a824080f0530fad64bbd32a153eb9c04` 的 `pelvis → upper-arm-pair` receipt `docs/evidence/mcp010f/supplemental/real-codex-cli-current-20260816-primary-form-pelvis-upper-arm-v1.json` 完成 2/2 `accepted + strict_improvement`、64 evaluations/step，最终 IoU `0.752731669182`、Boundary F1 `0.355021780326`，仍 `QUALITY_TARGET_NOT_MET`、review=`needs_revision`、未 confirm/version/export。此前 `head-shell → thigh-pair` receipt 作为 previous trial 保留，`chest-shell → shoulder-armor-pair` receipt 仍是单视角 best-known（IoU `0.754862679391`、Boundary F1 `0.369927879976`）。这证明 Primary Form 搜索已从 Codex 连续参数搜索移回 Runtime，并且每一步都重新观察/绑定 candidate；当前只有 `perspective`，缺少 `front/back/left/right/rear-three-quarter`，360 继续 `BLOCKED_REFERENCE_COVERAGE`。
...
2026-08-16 current source/async Job correction：当前源码 manifest 以 `docs/evidence/mcp010f/source-tool-manifest-summary.json` 为准，为 125 Schema、41 read + 32 opt-in write = 73 tools；`primary_form_repair_job_prepare`/`job_get`/`job_result_get` 已在真实 Codex supplemental receipt 中完成 64-evaluation transport。下方 2026-08-15 的 36/23、37/24 或旧 package 数量只保留为历史 cohort 快照，不覆盖当前 manifest。真实 async receipt 仍为 `QUALITY_TARGET_NOT_MET`，不解锁 confirm/version/export、人评、PBR 或 360。
...
当前账本校正：源码合同为 125 个 JSON Schema，工具面为 41 个默认只读工具和 32 个显式 opt-in write 工具（共 73）。新增 `job_result_get` 与 `primary_form_repair_job_prepare`；同步的 `primary_form_repair_prepare` 保留为受限 Runtime 内部/兼容入口，真实 Codex 使用异步 Job，把 bounded fit→typed GeometryProgram→strict readback→Render Worker→compare 收口为 Runtime-owned staged prepare，不 confirm/version/export。长时间搜索由 `primary_form_repair_job_prepare` 排队，`job_get` 轮询，`job_result_get` 回读 `RuntimeJobResult@1` 绑定的终态 CAS 结果。Agentic `scene_observe_get`/`critic_report_get` 现在在同一只读观察中携带该 Part-error table、推荐焦点和单一 RepairIntent scope；无 Part slice 时保留 unknown/null。Agentic projection 与 durable session/checkpoint/RepairIntent prepare/readback 另有独立 receipt；它们仍是结构/编排能力，不是 likeness 通过。
...
MCP006 的 44-contract、MCP010B 已保存的 50/52-contract aggregate，以及 3c/f488/bfa56/d9 Dev.app/raw/CLI receipt 都是历史或结构事实，原样保留。当前源码总计 **125 个 JSON Schema**：历史合同、MCP010B/C/D/E/F 当前合同、Agentic contract family 与 `RuntimeJobResult@1`。当前 source Gate 已通过 B 的 V2 geometry/readback/Worker isolation、C 的 fixed renderer/九 AOV/reference compare/review raw path、D 的真实 Operator、E 的离线 AssetPack/UV/PBR/MikkTSpace raw path，以及 F 的哈希绑定轮廓目标、扩展相机搜索、Runtime-owned camera reference、受限 Rig/SDF fit、Runtime-owned bounded Primary Form search、单/多 Part contour proposal、Part error table、candidate compare 和异步 `primary_form_repair_job_prepare`/`job_get`/`job_result_get` staged prepare/evaluate path；Agentic projection 与 durable prepare/readback 也通过合同/Viewer/隔离重启 probe；当前工具面为 41 read + 32 opt-in write。C/E/F receipt 使用 synthetic 或结构性 reference，只证明绑定、传输、持久化和 deterministic bytes，不证明 PBR likeness、用户 robot likeness、Viewer/package/live、人评或 360。Agentic durable receipt 也不证明通用单动作 orchestrator、Repair execution 或视觉 PASS。下文任何未特别注明的旧“50/52/65-contract/current Dev.app”叙述都应按本段分层。
...
|---|---|---|
| 合同 | MCP006 历史为 44 个 JSON Schema；当前 MCP010B/C/D/E/F source contracts、Agentic contract family 与 `RuntimeJobResult@1` 使当前 manifest 为 125 个 JSON Schema（含 `CameraCalibrationRef@1`、`SilhouettePartErrorResult@1`、`DesignSession@1`、`DesignCheckpoint@1`、`RepairIntent@1`） | 维持版本化合同；后续任务只可新增有证据的 Viewer/闭环合同 |
| MCP | 当前源码为 36 read + 23 opt-in write（57）；F 新增 Runtime-owned `silhouette_rig_hash` 以避免 Codex 本地重算 Rig canonical hash、`silhouette_part_error_get` 多 Part 误差表和 `primary_form_repair_prepare` 单动作 staged prepare/evaluate；Agentic 新增 projection tools、session/checkpoint readback 和 approval-gated prepare tools；C/E/F/Agentic source raw/restart Gate 已按各自范围通过，历史 Dev.app receipts仍按 cohort 保存 | 真实用户 likeness、同一 candidate 的 packaged Viewer、人评/PBR/360证据仍需独立 Gate；不得用 synthetic/raw、prepare receipt 或 projection 直接宣传高质量 |
