...
> 2026-08-28 `FPS-FORM-04BE-D`：当前为 **577 个 JSON Schema / 127 read + 94 opt-in write = 221 tools**。Runtime 已把 04BE-C 的 left boundary regression、left/right owner-void overlap 与 rear3q owner attribution 缺失转成一个 closed、hash-bound、read-only repair plan；计划 canonical=`d6f74060…85fd`，重启 exact equal且未改变业务表/CAS。它不是视觉修复结果。下一步只能执行 registered half-Y/flat-Z rear-stock source-local repair，然后用原 baseline/批准六相机重建 54 AOV、CrossView、FormArt、fresh FormQualityV2；全门通过前 MCP010F 继续 `QUALITY_TARGET_NOT_MET`。

> 2026-08-28 `FPS-FORM-04BE-C` 已闭合 durable six-view evidence sidecar：当前公共合同为 **575 个 JSON Schema / 126 read + 94 opt-in write = 220 tools**。same-cohort baseline 与 final composite candidate 的 54 AOV、CrossView、proposal FormArt 和隔离重启 GET 均 PASS；但 left 回归、left/right owner-void 侵入与 rear3q owner attribution 失败导致 `rejected-regression / BLOCKED_PROPOSAL_FORM_ART_EVIDENCE / QUALITY_TARGET_NOT_MET`。下一原子 `FPS-FORM-04BE-D` 只从 exact evidence 派生有界 repair plan；FormQualityV2、secondary 与 High→Low→UV→Bake 仍不允许开始。

> 2026-08-28 `FPS-FORM-04BE-B` 已闭合 durable composite candidate seam；当前公共合同为 **572 个 JSON Schema**。Runtime 以 original fresh baseline 作为视觉权威、04BB 作为当前几何 base，只应用产品注册且互斥的 trigger aperture replacement，创建 final reviewable candidate 并通过 Store/CAS/strict GLB/readback/restart GET。该 PASS 仅证明非丢失组合与持久 lineage，不证明视觉改善。下一原子 `FPS-FORM-04BE-C` 固定 6×9 AOV、FormArt 与 no-regression；`QUALITY_TARGET_NOT_MET`、secondary=`NOT_CREATED`、High/Low/UV/Bake=`NOT_RUN` 保持不变。

> 2026-08-28 `FPS-FORM-04BE-A`：当前公共合同集合为 **569 个 JSON Schema**。bounded typed composite plan/source seam 已完成，original fresh baseline 与 current proposal base 分层；registered trigger aperture 在保留 prior rear-stock edit 时通过 23-Part Worker compile，重复 node/Part 与未注册 profile fail closed。下一步 `FPS-FORM-04BE-B` 只补 Store/CAS exact scope loader、composite durable link、prepare/get/restart 和 reviewable candidate，然后以 original 04AZ 对 final composite 跑 exact six-view/negative-space/owner-void/line-flow；任何失败继续 `QUALITY_TARGET_NOT_MET`，不得提前进入 secondary Form 或 High→Low→UV→Bake。

> 2026-08-28 `FPS-FORM-04BA`：首个 curve-architecture repair 已真实物化并进入 six-view gate。BoundaryBridge 在 left/right silhouette 和 right boundary 上产生局部收益，但 left boundary 与 top silhouette 的 Pareto 回归触发 `REJECTED_REGRESSION`，proposal FormArt evidence 不可作为 FormQualityV2 输入。计划因此进入“同 owner、更小影响域”的精修，而不是 High/Low/UV/Bake：先用现有 AOV 确认 left/top 回归来自 Y 深度楔形还是 Z 抬升，再只改一个 profile 维度并保持 approved camera、reference、part/socket/material bindings 不变。

> 2026-08-28 `FPS-FORM-04AZ`：camera registration identity 已闭合。Runtime 使用现有 subject→GeometryProgram registration 与批准的 180° semantic orbit，把中间 `4fa162d6…d43` 重新派生为精确批准 `9d8e590e…3abb`；lineage/RigV2、semantic upright proof、同 cohort fresh baseline 6×9 AOV 与 restart readback 均 PASS。故计划不再停留在相机修复，下一原子进入 `fresh baseline → FormQualityV2 → 单个 bounded curve-architecture repair → six-view owner/negative-space/line-flow gate`。当前 silhouette IoU 与 owner-void 失败仍有效，禁止提前进入 secondary Form、Formal High、Low、UV、Bake 或商业质量声明。

> 2026-08-28 `FPS-FORM-04AY`：六视图 reviewed-form annotations 已从临时输入升级为 Runtime-validated closed profile，真实持久链已到 `camera-calibrated + FormEvidence + FormArt + legacy structural FormQuality`。当前六视图 silhouette IoU 为 front 0.438 / back 0.435 / left 0.209 / right 0.201 / top 0.428 / rear3q 0.277，远低于 0.90 门；owner-void 也未通过。批准 camera hash 与 fresh re-derived hash 不一致，故本计划下一原子固定为 camera registration identity 闭合，不得跳到 secondary Form、Formal High、Low、UV 或 Bake。

> 2026-08-27 `FPS-FORM-04AV`：Runtime 已拥有 `ProductionWeaponOwnerReviewedVoidCalibrationProjection@1` 及默认只读 GET，从 durable FormArt/fresh baseline/RigV2/Part-ID/depth/批准相机派生三视图 `rear-stock` 身份校准。当前公共面为 **568 个 JSON Schema / 124 read + 92 opt-in write = 216 tools**。identity calibration 与 strict owner-void 零侵入已分离，后者仍是不可放宽的 FormArt 门。source/compile 通过，但本轮因 04AU 一次性 D1 Store/CAS 不可用而没有运行真实投影；Stage、secondary、High/Low/UV/Bake 与质量结论不变。
...
> 2026-08-27 `FPS-FORM-04AR` 实际结果：既定 Form-first 路线未改变。用户 orientation authority、CameraLock registration lineage、current-cohort fresh baseline 与 `rear-stock 620×680` OpenFrameNotch 已在真实 D1 完成；Runtime structural/durable/readback PASS，但 six-view 视觉门 `NOT_IMPROVED / REJECTED_REGRESSION`。这证明单一盒体开框解决不了 owner/line-flow 与跨视图形体节奏，下一原子必须是新的 bounded rear-stock art-shape authoring，不得进入 Formal High 或继续参数扫尺寸。

> 2026-08-27 `FPS-FORM-04AL` 当前增量：Runtime-owned durable fresh six-view baseline producer 已接通合同、Store、Runtime 与 MCP `prepare/get`；每个视图绑定 approved registration lineage / RigV2、fresh same-cohort 512×512 九 AOV、camera/mask/compare/quality 与完整 CAS reachability，并以单事务持久化。精确状态为 `PASS_SOURCE_COMPILE_DURABLE_PRODUCER_NOT_RUN_REAL_D1`；真实 D1、orientation approval、fresh baseline、notch、secondary、Stage/confirm/version/export 均未执行。当前公共面 **538 schemas（即 538 个 JSON Schema）/ 118 read + 88 opt-in write = 206 tools**，视觉仍 `QUALITY_TARGET_NOT_MET`。

> 04AK 收口：新增 lineage-bound fresh-baseline preflight，严格固定六视图并拒绝 caller-owned camera/render/path；producer 尚未实现，故旧 FormArt 不得复用，真实 D1 零写。notch durable child commit 已后移，proposal CAS receipt 可失败清理；最终 child commit 后的下游补偿仍待闭合。当前 **533 schemas / 117 read + 87 write = 204 tools**，Form/secondary/quality 未晋级。

> 04AJ 收口：620/680 `AuthoringMeshOpenFrameNotch@1` 已生成 zero-write deterministic proposal；历史 04AC FormArt 仅允许完整回读，不允许作为当前 cohort 的主动视觉基线。Runtime prepare 在任何 durable child/candidate 写入前要求 fresh same-cohort FormArt。用户批准与 lineage 完成后，先刷新 baseline，再执行一次 620/680，并对六视图 Part-ID、owner void、negative space、line-flow 与 Pareto non-regression 做全量判定；失败保留 baseline。

> 04AI 收口：新增 sibling read-only projection tool，保持旧 `@1` 语义不变，并从请求中移除 camera orbit。真实 D1 isolated run 证明 Runtime 可由 screen order 派生 `180°` rear-oblique camera、精确 camera/canonical/proof hashes，且 CAS/SQLite 零写。该结果只允许请求用户 orientation approval，不允许推进 secondary、High 或视觉状态；该轮为 531/116+87，当前由 04AK 更新为 **533 个 JSON Schema / 117 read + 87 write = 204 tools**。

> 04AH 收紧：后 3/4 不允许用 `rotate-180` 解决左右语义错位。原始参考保持自身 authored rotation；另由用户确认 stock/muzzle screen order，Runtime 从封闭 rear-oblique camera orbit 派生最终相机，并投影 source anchors 证明 screen order 与 world `+Y` upright。该 proof 与 orientation receipt 持久绑定后，才能继续一个较小 open-frame notch。

> 2026-08-26 04AC 进展：不改本计划的 Form-first 顺序。当前为 **529 个 JSON Schema**。真实 D1 已用 typed `OpenFrameNotch` 代替无法表达负空间的纯 vertex taper，并形成闭合 U-frame/durable child/strict readback/fresh six-view FormArt。首个 `700/700` 候选被 six-view non-regression Gate 拒绝，因此 D1 仍未通过，不得进入 Formal High。

> 2026-08-26 `FPS-AUTHORING-MESH-V2-03`：当前 **527 schemas / 115 read + 87 opt-in write = 202 tools**。真实 D1 已证明 `MoveVertices` 不只是 kernel：它会生成 durable child，降低为 `forgecad.geometry.authoring-mesh@1`，只替换 `rear-stock`，编译/严格回读并六视图重放。当前 Z taper 有总分改善，但仍存在明确局部回退；Pareto review 只用于保留可审阅的艺术权衡，不改写旧 CrossView。因缺 proposal-side FormArt owner 证据，它仍 blocked，不是 secondary/high-quality PASS。

> 2026-08-26 `FPS-AUTHORING-MESH-V2-02`：真实 D1 的 `rear-stock` 已从 exact candidate-owned `primitive@2` source 派生为 source-bound `AuthoringMeshRevision@2` genesis，并通过 Runtime restart exact readback；当前工具面 **527 schemas / 114 read + 86 write = 200 tools**。该 revision 保留局部 transform 而不烘入顶点，稳定 lineage 不吸收易变 candidate/artifact hash。High Worker V2 bridge 与 closed `FaceExtrude` kernel 已编译；下一步必须补 `MoveVertices`，lower 为 `authoring-mesh@1` 并只替换 rear-stock node，随后复用六视图门。当前无 art edit、无 secondary approval、无 High PASS，quality 仍 `QUALITY_TARGET_NOT_MET`。

> 2026-08-26 `FPS-FORM-04AG` durable 更新：真实 D1 的 proposal-side `ProductionWeaponFormArtProposalEvidence@1` 已完成 CAS/Store/same-key replay/Runtime restart readback。现行 source 为 **528 schemas / 115 read + 87 write = 202 tools**（即 **528 个 JSON Schema**）。Part-ID 六视图 observed，但 owner/open-void、negative-space、line-flow 未全通过，故 durable transport PASS 不改变 `BLOCKED_PROPOSAL_FORM_ART_EVIDENCE / QUALITY_TARGET_NOT_MET`，仍保留 baseline，不推进 secondary/Stage/confirm/version/export。

> 2026-08-26 研究校正：高质量硬表面终态不是 renderer/AOV 或 provisional GLB，而是 Runtime-owned AuthoringMesh、non-destructive High、editable Low、explicit correspondence、Hero UV、zero-fallback per-Part Bake、MaterialLayerGraph、FPS/engine/human 同 hash 闭环。算法采用和逐阶段计划见 `FPS_HERO_WEAPON_PRODUCTION_RESEARCH_20260826.md`。

> 2026-08-26 `FPS-FORM-04AE` 当前源码面：**525 个 JSON Schema / 28 operator entries / 112 read + 84 opt-in write = 196 MCP tools**。CameraLock registration child、AuthoringMesh V2、Native High evaluator、strict High↔Low↔Cage/bake validator 与 MaterialLayerGraph plan 已完成 source compile/link；真实 D1 positive/restart 仍因用户 rear3q 朝向回执缺失而 `NOT_RUN`，且后续完整资产执行仍未开始。这不提升视觉、人评、引擎或商业状态。下方 518/194 等数量均为早先历史切片。
...
> 2026-08-26 `FPS-FORM-04AB` 实测更正：真实 durable FormArt、registered-camera replay 与 live attribution 已通过；`left.open-stock-void` 唯一最高源是 `rear-stock/548px`，次高 `receiver-main/114px`，muzzle-shroud=0。MCP010F 下一原子只允许一个 rear-stock source-node RepairIntent，然后重渲染三张开放枪托视图与六视图；未通过 strict owner-void/non-regression/Codex review 不得 secondary approval。正式商业晋级前另完成 V2 durable semantic camera lineage。

> 2026-08-26 Goal 收口：MCP010F 的真实资产顺序固定为 `D1 secondary Form approval → AuthoringMesh → High → editable Low → Hero UV → Cage/Bake → Material → FPS → Engine/Human`。后序模块可先做到 compile，但不得跳过前序 Stage 写入或把 preview-only 结果 promotion。下一真实动作仅允许 FormArt 归因后的一个 source-node Repair。

2026-08-26 Form attribution 同步：现有只读 FormArt get 已将 fixed-renderer pixel→triangle→Part/source lineage/material zone/mesh/primitive 归因绑定到 exact durable FormArt、candidate/artifact/readback、reference target、CameraLock/rig、FormEvidence view、RenderSet 与派生 masks；04AB 真实 D1 execution=`PASS_UNIQUE_REAR_STOCK_SOURCE / ZERO_WRITE`。该解释性诊断只解锁一次局部修复，不能替代 FormQuality、secondary-form-approved、Formal High、视觉或真人门。

2026-08-26 Formal High public-source 同步：4 个 contracts、Runtime adapter/IPC、MCP `get/prepare` 与 Store scoped idempotency 已 source/focused PASS。合法 positive Stage fixture、cleanup/restart 与 raw current-cohort receipt 尚未通过。故 `FPS-HIGH-05=NOT_PASSED`，Cage/Bake 在真实 D1 仍只能 fail closed；该 source 增量不能被材料、灯光或 Low/UV 诊断补偿。

2026-08-26 最新 MCP010F 增量：Form Stage policy、legacy head mapping、Formal High factory/Store/MCP/Runtime public seam 已 `PASS_SOURCE_COMPILE_FOCUSED`。完整 Formal High positive restart/cleanup=`NOT_RUN`、真实 D1=`ZERO_WRITE / camera-calibrated / QUALITY_TARGET_NOT_MET`；因此 MCP010F 仍 `in_progress`，不能进入 quality done、confirm、version 或 export。

2026-08-26 Cage/Bake public seam 增量：Store 七子表原子 commit、固定 exact-topology Cage、8-map Bake 与 8-texel dilation 已 source PASS。High resolver 已按 `Stage source candidate + distinct derived High candidate` 分别读取并验证真实字段；Formal High 的 Runtime-owned factory、原子 Store seam 与无环 internal materializer 已完成 source/compile/focused。当前下一前置是完整 source-lineage/CAS 正向 materialize→drop/reopen fixture，随后才是独立 public prepare/get，不是新增重复 binding Schema。真实 D1 的 Form 前置未通过且没有 formal positive receipt，因此全新 `production_weapon_high_low_bake_prepare` 仍报告 `FORMAL_HIGH_STAGE_SOURCE_LINEAGE_UNAVAILABLE`，整体 `PRODUCTION_WEAPON_HIGH_LOW_BAKE_PRODUCER_UNAVAILABLE` 且零写。formal High capability/Cage-Bake positive receipt 均 `NOT_RUN/NOT_PROVEN`，不提升 Stage、视觉、人评、引擎、分发或商业结论。证据：`docs/evidence/mcp010f/commercial-weapon-form-stage-policy-formal-high-source-gate-20260826.json` 与 `docs/evidence/mcp010f/commercial-weapon-cage-bake-public-seam-source-gate-20260826.json`。


> 2026-08-26 最新权威 source 口径（取代下方 2026-08-25 的“最新/当前”计数）：**518 schemas / 28 operator entries / 111 read + 83 opt-in write = 194 MCP tools**。Low quad draft 与 Hero UV durable链保持 structural/source；04AB 已在 fresh durable D1 上完成 registered-camera 精确重放并把唯一最高影响 source 锁定为 `rear-stock`。当前 repair blocker 是单 source-node 联合形体修复、rear-three-quarter authored orientation、六视图 non-regression 与 secondary approval，不再是 muzzle/stock 语义错配。以上均不推进 Stage、confirm、version 或 export；`FPS-HIGH-05=NOT_PASSED`、Stage=`camera-calibrated`、`secondary-form-approved=NOT_CREATED`、visual=`QUALITY_TARGET_NOT_MET`、`HQ360=BLOCKED_REFERENCE_COVERAGE`、proposal=`registered=false`。证据见 Hero UV durable receipt 与本文件顶部 FormArt attribution receipt；`FGC-MCP010F` 是唯一 `in_progress`。

> 2026-08-25 历史快照（已由上方 2026-08-26 权威口径取代）—生产作者链子门：**499 schemas / 107 read + 79 opt-in write = 186 tools**。High Worker **6/6 library + 7/7 transport**、当前 cohort Runtime restart **1/1 PASS**；Low quad draft **10/10**（其中 explicit quad 2/2），Hero UV **3/3**，Worker Protocol strict envelope 1/1，Viewer typecheck/build PASS。Low/UV 仍为 Worker-only、High proposal 未注册；package 与 candidate quality 未通过，`FPS-HIGH-05` 仍 `NOT_PASSED`。
...
2026-08-25 Native High 较早 source/transport Gate：6/6/bridge-only 已由顶部 source durable/MCP receipt 取代。proposal 仍 `registered=false`，packaged/candidate quality/Stage evidence 未通过，`FPS-HIGH-05=NOT_PASSED`；Low、Modifier 与总体质量状态不变。

Source receipt：`docs/evidence/mcp010f/native-high-stable-id-fixed-transport-source-gate-20260825.json`。它只登记 supplemental source/transport truth，不替换 Stage0 benchmark。

当前公共合同集合为 **518 个 JSON Schema**、28 operator entries、111 read + 83 opt-in write = 194 MCP tools；Low durable 已接 Contracts/Store/Runtime/MCP，保留 candidate-bound current provenance，prepare replay/drop-reopen/get **1/1 PASS**，仍 `DRAFT_UNREVIEWED / structural_only / promotion_eligible=false`。Hero UV public `hero_uv_durable_get/prepare` 已接 Store/Runtime/MCP，四个 CAS roots 已纳入 linked/GC；仅 structural/source，artist UV review、packaged same-cohort、visual/human/engine/commercial 仍 `NOT_RUN/NOT_PROVEN`。无 Stage/confirm/version/export 推进。

2026-08-25 commercial-quality rebase：本计划的“高质量”必须按 `COMMERCIAL_GAME_WEAPON_QUALITY_PLAN.md` 的完整生产链解释。当前 2K render、Three.js readback、固定 AOV、工具/Schema 数量都不能解锁商业质量；Low 仍非 artist-authored quad topology，Bake 仍有 miss/fallback/cross-part 且 padding=0，Material 仍为 fixed-formula preview。MCP010F 继续是唯一 `in_progress`，先完成 FormQuality/secondary-form；之后才按 AuthoringMesh、High、Low、Hero UV、Cage/Bake、Material、FPS/Engine/Human 顺序实施，不得并行跳阶段。

### 商业级 11 组路线（质量/交付验收）

MCP010A–F 是 ForgeCAD 的工程任务依赖；商业 Hero Weapon 的质量与交付门必须另外按下列 11 阶段逐项闭合。所有阶段绑定同一 `candidate_hash → export_hash`，前阶段未通过时，后阶段只能做隔离诊断，不得推进 Stage、confirm、version 或 export。
...
11 组用于质量/交付工作分解，不是第二套 Stage 状态机；真实 Runtime 晋级只认 19 个 `ProductionStage@3` 值，并要求 `hero-art-review-approved → engine-validated → export-confirmed`。下表不得用于跳过任何 Stage transition。

| # | 阶段 | 独立退出门 | 当前证据与状态 |
|---:|---|---|---|
| 1 | Art Direction / ReferenceViewSet | `WeaponArtBrief@1`、style pillars、授权/IP、平台预算；五个核心视图的 reviewed ReferenceViewSet、CameraLock、silhouette/negative-space/landmark 与 observed/inferred/unknown 分层 | Stage=`camera-calibrated`；CrossView=`QUALITY_TARGET_NOT_MET`、`secondary-form-approved=NOT_CREATED`、depth=`UNKNOWN`；`HQ360=BLOCKED_REFERENCE_COVERAGE` |
| 2 | AuthoringMesh | original/evaluated 分离、稳定 V/E/H/C/F/loop/ring/boundary ID、可编辑局部操作、source map 与 High↔Low correspondence | split/collapse/dissolve 独立 durable full-chain **3/3 PASS**；general correspondence、evaluated retarget、完整编辑历史和商业 editor `NOT_PROVEN` |
| 3 | High | 非破坏 `HighMeshArtifact@1`/`DetailGraph`、布尔/倒角/support loop/weighted normal/Subdivision/floater、高光连续和 strict GLB readback | NativeHigh source/durable GLB/readback 为 structural PASS；两候选 resolver=`PASS_COMPILE_FOCUSED`，但 Runtime-owned derived High + `ProductionWeaponHighArtifact@1` formal positive receipt=`NOT_RUN`；`FPS-HIGH-05=NOT_PASSED`、proposal=`registered=false` |
| 4 | Low / Retopo | artist-authored editable quad flow、hard-edge/UV seam/Part 边界约束、High↔Low correspondence、bake-ready topology | candidate-bound `DRAFT_UNREVIEWED / structural_only / promotion_eligible=false`；prepare replay→drop/reopen→get **1/1 PASS**；artist review/package `NOT_RUN` |
| 5 | Hero UV | 2K/4K texel density、seam/hard-edge、stretch/overlap/OOB、mip padding、UV0/UV1 与 tangent/Mikk replay | 7 contracts 已接 Store/Runtime/MCP public `hero_uv_durable_get/prepare`；prepare/replay/drop/reopen/get **1/1 PASS**、4 Hero CAS roots linked/GC；artist review/package/engine `NOT_RUN/NOT_PROVEN` |
| 6 | Cage / Bake | topology-correspondent Cage、per-Part ray/offset、miss/fallback/cross-part/skew/penetration/dilation 及 8 类 maps 全部通过 | fixed Worker、8-texel dilation、七记录 Store/MCP public seam 与 exact replay=`PASS_SOURCE`；Runtime-owned producer 未闭合，真实 D1=`ZERO_WRITE / PRODUCTION_WEAPON_HIGH_LOW_BAKE_PRODUCER_UNAVAILABLE`，formal positive receipt=`NOT_RUN`。旧失败诊断不是退出证据，正式 Cage/Bake=`NOT_PASSED` |
| 7 | Material | `MaterialLayerGraph@1` 的 Layer/Mask/Generator/Decal/Wear/Microdetail、roughness hierarchy、色彩空间/provenance、多视图一致性 | 当前仅 **4 MaterialZones / 6 formula textures**；embedded PBR/2K 仅 structural preview，commercial PBR=`NOT_PROVEN` |
| 8 | LOD / Collision / Socket | authored LOD0/1/2、误差、collision/socket、同 hash 资源与平台预算 | 仅局部 structural delivery；商业 LOD/collision/socket 与预算 `NOT_RUN` |
| 9 | Viewer / animation / VFX / audio validation | 同 candidate read model、first/third-person fixed cameras、animation、VFX、audio timing/readability 与 accessibility evidence | Viewer source/read-model smoke 是结构证据；Three.js 仅结构消费，animation/VFX/audio、正式 VoiceOver 与真人观看 `NOT_RUN` |
| 10 | Engine | Unreal 或 Unity importer/material/tangent/LOD/collision/socket/animation round-trip 与 draw-call/texture/memory/frame-time/streaming budgets | **Unreal/Unity 均未运行**；commercial engine=`NOT_RUN`，Three.js 不能替代引擎验收 |
| 11 | Independent Hero Art Review | 独立资深 Hero Art Review/盲审、修订清单闭合、同 `export_hash` 的 confirm/version/export/restart readback | `human=NOT_RUN`，不存在 `PASS_HUMAN_ART_REVIEW`；只有 1–11 全通过才可 `HERO_ASSET_APPROVED` |

这张表是 MCP010 质量/交付的唯一阶段顺序；下方历史工程切片仍可保留，但不得把其合并行当作 High、Low、UV、Cage/Bake 的单一 PASS。Three.js、source/transport receipt 和旧 bake 诊断都不能替代正式 Cage/Bake、Unreal/Unity 或独立 Hero Art Review。

2026-08-25 `FPS-FORM-04X` checkpoint：real private coupled stock-upper-profile boundary-translation screen **1/1 PASS（780.85s）**，但 geometry direction=`STOPPED`。`+0.020/+0.040 m` 的 intrusion 分别为 `302/472/587`、`296/416/587`，adjacency milli 分别为 `319/317/248`、`319/307/248`；三视图 identity 保持，formal owner gate 两档均 false，原因是 nonzero intrusion 与 rear3q adjacency `248<250`。该 execution PASS 不提升 quality/secondary，六视图、public sink、stage、confirm/version/export 均未运行或推进。证据：`docs/evidence/mcp010f/production-weapon-real-d1-private-stock-upper-profile-boundary-translation-screen-20260825.json`，log sha256=`19f4935b4a1078cb435773740455ac997bc92679494986a2c219b529e21082fb`。

2026-08-25 `FPS-FORM-04W` checkpoint：real private lip continuation 1/1 PASS（753.14s），intrusion 下降但 boundary adjacency 同步恶化，证明当前单轴 shrink 不能满足 open-stock owner gate。停止 lip_y 外推，下一步只允许证据驱动的 coupled profile 边界形变设计。

2026-08-25 `FPS-FORM-04V` checkpoint：real private lip extrapolation 1/1 PASS（727.10s），`+0.005 m` 首次产生三视图 strong signal，但 intrusion=`280/362/585` 仍远非零，rear3q adjacency=`248`。允许最后一次 `+0.025/+0.045 m` screen，不提升 sink。

2026-08-25 `FPS-FORM-04U` checkpoint：real pixel-owner audit 1/1 PASS（700.95s）。owner-mask Hamming + expected-void/boundary-band + depth/silhouette 诊断证明 04R 是唯一有效轴，但 left/rear3q 变化尚在 reviewed region 外。下一步只允许 `-0.015/+0.005 m` lip_y private 外推；未达 zero intrusion 不得六视图或开放 sink。

2026-08-25 `FPS-FORM-04T` checkpoint：cap-facing inner-lip 第二坐标 real screen 1/1 PASS（648.54s），但两档三视图均与 baseline 完全相同，owner screen blocked。停止该局部轴，下一步先做像素归属/可见边界审计，不提升 sink。

2026-08-25 `FPS-FORM-04S` checkpoint：private profile outer-shoulder real screen 1/1 PASS（621.10s），两候选 owner screen blocked。两档仅将 right intrusion 降至 `488/486`，left/rear3q 维持 `302/587`；立即停止该轴，不提升 sink。下一步先审计 profile 坐标与 camera 投影，不得在 depth=`UNKNOWN` 时借深度参数伪造视图改善。

2026-08-25 `FPS-FORM-04R` checkpoint：private profile-lip real screen 1/1 PASS（595.63s），但两候选 owner screen blocked。`-0.035 m` 仅改善 right（`492→474`），left/rear3q 不变；`-0.075 m` right 回退至 `528`。identity/stable binding/rear-cap/lineage 保持，因此停止 lip-only 搜索，不提升 public sink。下一步仅允许影响 left/rear3q 的单一 bounded 2D profile 控制轴，仍须先过三视图才允许六视图；quality=`NOT_PROVEN`、depth=`UNKNOWN`。

2026-08-25 `FPS-FORM-04Q` checkpoint：private closed upper `profile-loft@2` real screen 1/1 PASS（568.68s），但两完整 profile owner screen blocked。identity 保持，right intrusion 略降，left/rear3q 不变；停止 inner-span-only profile 搜索，不提升 public sink。下一步固定 `0.85 m` inner span，screen bounded profile lip/crown，仍须先过三视图再允许六视图；quality=`NOT_PROVEN`、depth=`UNKNOWN`。

2026-08-25 `FPS-FORM-04P` checkpoint：private upper-inner-span `{0.85,0.75} m` real screen 1/1 PASS（545.10s），两候选 owner screen 均 blocked。侵入下降但 adjacency 同步下降，rear3q identity 均失败；停止单轴 box 收缩，不提升 public sink。下一步设计 closed upper-frame 2D profile，仍须先过三视图 owner screen 才允许六视图；quality=`NOT_PROVEN`、depth=`UNKNOWN`，无 stage/secondary/confirm/version/export。

2026-08-25 `FPS-FORM-04O` real-D1 归因子门：**1/1 PASS（517.50s）**。临时 source Part-ID 上下梁在 left/right/rear3q 对原 `rear-stock` 做零重叠、零缺失的 exact partition；owner intrusion `302/560/587` 全来自 upper，lower 为 `0/0/0`，且 nodes/rear-cap/registered lineage 保持不变。由此停止 symmetric two-beam search，后续只能先做 private bounded upper-only single-variable screen。该子门不提供 public sink，不推进 FormQuality/secondary/confirm/version/export；quality=`NOT_PROVEN`、depth=`UNKNOWN`。证据：`docs/evidence/mcp010f/production-weapon-real-d1-stock-source-attribution-20260825.json`。

2026-08-25 `FPS-FORM-04N` checkpoint：private stock-plane `{-0.10,+0.10} m` real screen 1/1 PASS（504.80s），但两候选 owner screen 均 blocked。负向侵入全变差；正向侵入降至 `191/390/428`，同时 rear3q identity/adjacency 门失败。停止单轴 X 扫描，不提升 public sink；下一步需对 stock frame 的二维形状作新一轮精确 bounded correction，仍先过三视图 owner screen 再跑六视图。

2026-08-25 `FPS-FORM-04M` checkpoint：真实 D1 1/1 PASS（477.36s）。strict owner binding 现在区分 permissive discovery eligibility 与 unique ranked identity，并继续强制 raw intrusion=0；当前 `302/560/587` pixels 与 adjacency `319/420/248` 仍阻断。signed image-space residual 不足以推出 world-X 单边方向，因此下一 bounded correction 只做 private `stock-plane-position {-0.10,+0.10} m` 对称 A/B；public sink、FormQuality/secondary/human/confirm/version/export 不变。

2026-08-25 `FPS-FORM-04K` checkpoint：真实 D1 registered-camera owner-void diagnostic 1/1 PASS，rear3q authored `180°` reference rotation 已使 direct Part-ID identity；left/right/rear3q raw intrusion=`302/560/587`，严格 acceptance 仍 BLOCKED。该切片只闭合 mask/contour/owner diagnostic，不闭合 landmark/region rotation quality；FormQuality、secondary、human、confirm/version/export 不变。下一 bounded correction 必须以这些 raw intrusion 为目标，并重跑同 candidate/frozen lineage 门。

2026-08-25 `FPS-FORM-04J`：真实 D1 fixture **1/1 PASS（360.46s）**，durable CameraLock restart/CAS readback 后生成 transient registered rig；7 views、唯一 primary=`left`、same-candidate/state/artifact、GeometryProgram raw/canonical 与 CameraRig raw/canonical exact，cross-candidate rejected，注册投影零新增 CAS。该子门只闭合 camera lineage；rear3q reference authored registration 与 strict owner-void acceptance 仍是下一原子，CrossView=`QUALITY_TARGET_NOT_MET`、depth=`UNKNOWN`、secondary=`NOT_CREATED`、无 confirm/version/export。

2026-08-25 `FPS-FORM-04I`：合同面现为 **474 JSON Schemas**，MCP surface 仍为 **103 read + 76 write = 179 tools**。subject-frame registration/registered-camera lineage 是公开 source、非 durable、非 MCP、非 quality-promotion 合同；depth=`UNKNOWN`。下一原子只做 same-candidate D1 CameraLock 只读 link。

2026-08-25 `FPS-FORM-04H-SUBJECT-FRAME-REGISTRATION-SOURCE`：D1 semantic anchors 已把 04G 的左右 horizontal-flip 收敛为缺失的 GeometryProgram→canonical SubjectFrame `yaw-180-y` 注册。新增 private typed/read-only/hash-bound projection 与 transient camera materialization；synthetic **7/7 PASS**、真实 D1 source **1/1 PASS（0.05s）**，7 typed sinks 前后相同，geometry/depth 不变。该 source slice 不修改 GeometryProgram/GLB、无 Runtime/Worker/CAS/SQLite 写入，也不把 04G rear-three-quarter vertical-flip 推导为 yaw truth。下一原子需先正式表达 camera/registration lineage，随后重跑同 candidate D1；owner-void acceptance、FormQuality、secondary stage 仍未通过。

2026-08-26 `FPS-FORM-04AE-CAMERA-LOCK-CHILD-SOURCE`：正式 `ProductionCameraLockRegistrationLineage@1` 已完成 5 contracts、SQLite/CAS immutable row、Runtime-owned ordering/orientation/RigV2 materialization、MCP read/write 分类与 Runtime dispatch，focused compile PASS。它解决了“调用方注入伪 matrix/crop/output hash”的结构性缺口，但 real D1 prepare/get/restart 仍必须等 orientation-specific user approval；不得冒充视觉、secondary 或商业质量 PASS。

2026-08-25 `FPS-FORM-04G-REGION-PART-ID-BINDING` supplemental diagnostic：真实 D1 **1/1 PASS（371.07s）**；回执 `docs/evidence/mcp010f/production-weapon-form-region-part-binding-supplemental-20260824.json` 记录 left/right=`horizontal-flip`、rear-three-quarter=`vertical-flip` 的 baseline/trial 一致唯一候选。它们只来自有限 image-space transform discovery；没有 authored orientation/registration contract，且 permissive overlap threshold 禁止作为 acceptance，因此状态仍为 `BLOCKED_AUTHORED_ORIENTATION_OR_REGISTRATION`、FormArt/confirmed contours/depth=`UNKNOWN` 不变。下一原子为 `FPS-FORM-04H-AUTHORED-VIEW-ORIENTATION-REGISTRATION`，无 Stage/secondary/confirm/version/export。

2026-08-24 `FPS-FORM-PART-ID-AUDIT-04F` supplemental diagnostic：`docs/evidence/mcp010f/production-weapon-form-part-id-audit-supplemental-20260824.json` 记录 `BLOCKED_OWNER_BINDING`；mutator/Part-ID changed（owner=`rear-stock`），但 left/right/rear-three-quarter reviewed region↔Part binding 未 ready。FormArt canonical unchanged、depth=`UNKNOWN`，无 stage/confirm/version/export；该 evidence sidecar 不新增 schema/MCP public contract/manifest、不改变 Stage0 hashes，下一原子为 `FPS-FORM-04G-REGION-PART-ID-BINDING`。

本轮 Stage0 同时记录闭合 `BlenderTaskRequest/Result/Error@1` strict protocol 与 `HighLowBake` read-only zero-write preflight（均为 source/contract 事实）；未执行 Blender，未完成 formal Bake、secondary stage 或视觉质量门。

当前合同面为 **518 个 JSON Schema**；MCP source surface为 **111 read + 83 opt-in write = 194 tools**。AuthoringMesh split/collapse/dissolve 独立 full-chain **3/3 PASS**；High 6/6 + 7/7并有当前 cohort Runtime restart 1/1；Low 为 candidate-bound current provenance 且 prepare replay/drop-reopen/get **1/1 PASS**，仍 `DRAFT_UNREVIEWED / structural_only / promotion_eligible=false`；Hero UV public get/prepare 已接 Store/Runtime/MCP，四个 CAS roots linked/GC。它们仍是 structural/source gates；真实 Form仍停在`camera-calibrated`，`secondary-form-approved=NOT_CREATED`、`FPS-HIGH-05=NOT_PASSED`、`QUALITY_TARGET_NOT_MET`、HQ360=`BLOCKED_REFERENCE_COVERAGE`，artist/packaged/visual/human/engine/commercial 仍未运行或未证明。

真实参考来源链现已推进到 reviewed FormArt、CrossView 与 structural-only legacy FormQuality durable receipt；2026-08-24 最新 fixture **1/1 PASS（324.32s）**，same-key replay、restart readback、retarget zero-write、5/7 sink 投影和 FormQuality@2 preflight 零写均通过。未完成项是把真实 CrossView 从 `QUALITY_TARGET_NOT_MET` 修到 hard gate pass，并让 FormArt target observation 严格通过；Stage 仍停在 `camera-calibrated`，不得用 fixture PASS 替代 `secondary-form-approved`。
...
数值口径：当前 source 为 **518 个 JSON Schema / 28 operator catalog entries / 111 read + 83 opt-in write = 194 tools**；下文其余较小数值均是 historical prior snapshot，即使历史文案保留“当前/current”字样也不得覆盖本口径。Hero UV public get/prepare 与 candidate-bound Low provenance 的 source/restart receipt 为 `docs/evidence/mcp010f/commercial-weapon-hero-uv-durable-restart-source-gate-20260826.json`。
...
2026-08-26 现行账本：**518 schemas / 28 operator entries / 111 read + 83 opt-in write = 194 tools**。紧随其后的 191/21/90 条目是 2026-08-14 历史快照，不再代表当前 manifest；现行 Cage/Bake 三工具为 preflight_get/get/prepare，Formal High internal materializer 与 source seam 已有，但完整 positive restart/public surface/current-D1 receipt 缺失。

2026-08-14 historical 账本快照：当时源码合同为 191 个 JSON Schema、21/21 active operators，工具面为 54 个默认只读工具和 36 个显式 opt-in write 工具（共 90）；现行口径见本文顶部 518 schemas / 28 operators / 194 tools。新增 Mechanical Animation Clip 的 prepare/get/preview 只形成 Runtime-owned immutable rigid-action sidecar 与同 cohort Worker replay，不提供角色动画或任意脚本；既有 `FictionalEnergyRifleProfile@1`/`FictionalEnergyRiflePlan@1` 仍仅作 nonfunctional source authoring aid；`job_result_get` 与 `primary_form_repair_job_prepare` 继续把 bounded fit→typed GeometryProgram→strict readback→Render Worker→compare 收口为 Runtime-owned staged prepare，不 confirm/version/export。Agentic projection 与 durable session/checkpoint/RepairIntent prepare/readback 另有独立 receipt；它们仍是结构/编排能力，不是 likeness 通过。
...
## 商业级 FPS 资产分阶段退出门（计划补充，不是 MCP010 当前 PASS）

MCP010 的 source、transport、GLB readback、固定 AOV、2K consumer 和工具/Schema 数量都不是商业质量 KPI。商业 Hero Weapon 必须由同一 candidate/export hash 依次通过下列门；前门未通过时，后门只能运行诊断，不能推进 Stage、confirm、version 或 export。

| 阶段 | 退出门 | 当前真实状态 |
|---|---|---|
| Art Direction / design language | `WeaponArtBrief`、style pillars、color/material hierarchy、第一/第三人称识别、预算和授权/IP审查 | target/missing，独立 Art Director 决策 `NOT_RUN` |
| Silhouette / primary / negative space | front/back/left/right/rear-three-quarter coverage、CameraLock、比例、landmark、闭合负空间和 fixed-camera non-regression | 当前 Stage=`camera-calibrated`，CrossView=`QUALITY_TARGET_NOT_MET`，`secondary-form-approved=NOT_CREATED`，depth=`UNKNOWN` |
| Secondary / tertiary + bevel density | panel/vent/groove/seam/fastener、二三级平面节奏、倒角/倒棱密度、高光连续性、Part/source map；拒绝 triangle padding | bounded Operator source 可运行，但商业视觉 `NOT_PROVEN`；必须在 primary 通过后逐级放行 |
| Authoring topology / edge flow | original/evaluated 分离、quad/loop/ring/crease/hard-edge、稳定 element IDs、可编辑局部修改和跨 revision correspondence | split/collapse/dissolve durable/restart `3/3 PASS` 仅为结构证据；general correspondence、evaluated retarget、产品级 editor/edge flow `NOT_PROVEN` |
| Native High → Low/Retopo → Hero UV → Cage/Bake | High `DetailGraph`/artifact、artist-authored editable Low、Hero UV density/seam/padding、topology-correspondent cage、8-map bake 及 miss/fallback/cross-part/skew/dilation 诊断 | Native High/Low/Hero UV 仍仅 structural/source；Formal High internal materializer 与 Cage/Bake Worker/public seam 仅 source/compile/focused，完整 positive restart/public surface/current-D1 receipt 缺失，new prepare fail-closed。旧 bake diagnostic 继续作为失败证据 |
| Material Layer / PBR | Layer/Mask/Generator/Decal/Wear/Microdetail、roughness hierarchy、channel/color-space/provenance 和多视图材质一致性 | embedded PBR/2K/Three.js 只为 structural consumer evidence；fixed-formula preview，commercial PBR `NOT_PROVEN` |
| FPS/world presentation | hip/ADS/inspect/equip/reload/recoil first-person、third-person/world model、socket/attachment/readability，禁止 camera cheat | `NOT_RUN` |
| Engine / performance / delivery | commercial engine importer/shader/material、LOD0/1/2、collision/socket、draw-call/triangle/texture/memory/frame-time/streaming budgets | commercial engine/performance `NOT_RUN`；Three.js readback 不能替代引擎往返 |
| Independent Art Director / export/restart | 独立真人盲审、修订清单/批准、同 export hash 的 confirm/version/export/restart readback | human=`NOT_RUN`、`PASS_HUMAN_ART_REVIEW` 不存在；export/restart=`NOT_RUN`；总体仍 `QUALITY_TARGET_NOT_MET` |

Native High 当前只能记为 **source-only structural/durable slice**：Worker/GLB/Runtime CAS/Store/restart 与公共 MCP source-focused 证据通过。`packages/forgecad-skills/proposals/native-high/0.1.0` 保持 `registered=false`，不进入 active registry；packaged same-cohort 与 candidate visual/human/engine/distribution 仍 pending/`NOT_RUN`。该 slice 不解锁后续阶段，也不将 `FPS-HIGH-05` 写成 PASS。
...
| 项目 | 当前事实 | MCP010 目标 |
| 说明 | `5143ac3b…6e61`、bfa56、d9c23b 与 `6f00a58a…ed50` 均为历史 package/live receipts；现行 source 口径见本文顶部 515/28/194，同 cohort packaged rebuild 仍需独立 receipt | live 证据仍只证明结构工具链，不提前写成视觉/PBR能力 |
|---|---|---|
| 合同 | MCP006 的 44 与后续 191 均为历史 cohort；2026-08-26 当前 manifest 为 515 schemas，包含 Low/Hero UV durable 与 Cage/Bake formal public contracts | 维持版本化合同；后续任务只可新增有证据的 Viewer/闭环合同 |
...
当前源码工具数量为 111 read + 83 opt-in write = 194。MCP010A/010B 的 30/32-tool Dev.app receipts均为历史 structural cohort；当前源码 Dev.app 的同 cohort install/probe 已通过，Codex Desktop 完整重启 Gate 仍 `NOT_RUN`。Hero UV public `hero_uv_durable_get/prepare` 与 candidate-bound Low provenance 的 prepare replay/drop-reopen/get **1/1 PASS** 只证明 structural/source，四个 CAS roots 已纳入 linked/GC；artist UV review、packaged same-cohort、visual/human/engine/commercial 仍 `NOT_RUN/NOT_PROVEN`。C source raw 已证明 `render_pass_get` image block 和三项视觉证据工具，D 当前 packaged raw 已证明同 cohort Operator/strict readback transport，E raw 及当前 packaged E 已证明 `material_pack_get`、embedded texture 和九 AOV render path，F source 已证明轮廓目标、37 个覆盖全局尺度的粗候选加局部探针相机拟合、`CameraCalibrationRef@1`、Rig/SDF/Part/candidate compare、边界误差读取、Subdivision artifact-lineage read projection、Authoring Mesh source operator、`authoring_mesh_edit_prepare` approval-gated candidate staging、`primary_form_repair_prepare` staged prepare/evaluate 与 `repair_intent_run_prepare` CAS-bound staged run；Agentic projection 与 durable prepare/readback 已通过合同 checker、preflight 顺序、空 reference fail closed 和 Runtime/MCP 重启 probe。packaged Viewer 已有 read-model/window/core-control smoke，但与 attempt35 provisional observation 的 package binding、正式 VoiceOver、真实用户 likeness/PBR likeness、人评阈值和所有 360° evidence仍 `NOT_RUN/BLOCKED`。Stage=`camera-calibrated`、`secondary-form-approved=NOT_CREATED`、`FPS-HIGH-05=NOT_PASSED`、`QUALITY_TARGET_NOT_MET`、`HQ360=BLOCKED_REFERENCE_COVERAGE`、proposal=`registered=false`，无 confirm/version/export。证据：`docs/evidence/mcp010f/commercial-weapon-hero-uv-durable-restart-source-gate-20260826.json`。
