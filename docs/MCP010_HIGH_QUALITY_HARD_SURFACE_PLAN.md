...
当前账本校正：源码合同为 102 个 JSON Schema，工具面为 36 个默认只读工具和 23 个显式 opt-in write 工具（共 59）。新增 `silhouette_part_error_get` 与 `primary_form_repair_prepare`；后者把一次 bounded fit→typed GeometryProgram→strict readback→Render Worker→compare 收口为 Runtime-owned staged prepare，不 confirm/version/export。Agentic `scene_observe_get`/`critic_report_get` 现在在同一只读观察中携带该 Part-error table、推荐焦点和单一 RepairIntent scope；无 Part slice 时保留 unknown/null。Agentic projection 与 durable session/checkpoint/RepairIntent prepare/readback 另有独立 receipt；它们仍是结构/编排能力，不是 likeness 通过。
