...
2026-08-15 最新工具面覆盖：当前为 37 read + 30 opt-in write = 67 tools、118 个 JSON Schema。新增 `repair_apply_confirm` 只消费单视图、fresh-approval 且 lineage revalidated 的 apply intent；`design_action_optimization_proposal_prepare` 只创建独立 CADFit review candidate，不自动 Repair/Confirm；多视图仍走 `cross_view_promotion_confirm`。Rust focused test 与 workspace test 已通过，Codex/ForgeCAD 的视觉质量边界不变。
...
MCP004 当前在 Runtime/authenticated IPC 提供 typed `project_create`、`candidate_prepare`、`candidate_confirm`、`candidate_reject`、`restore_prepare`、`restore_confirm`、`export_prepare`、`export_confirm` 和 `job_cancel` 事务核心；MCP005 新增 `reference_import`/`reference_get`，MCP007/008/009 新增 geometry/appearance/change/quality/version/export。MCP010B/C/D/E/F 源码还提供默认只读的 `operator_catalog_get`、`geometry_program_hash`、`material_pack_get`、`render_pass_get`、`silhouette_target_get`、`camera_fit_prepare`、`silhouette_fit_prepare`、`part_contour_fit_prepare`、`silhouette_part_error_get`、`silhouette_candidate_compare`、`boundary_error_get`，以及 C 的 `reference_compare_prepare`、`visual_review_submit`、`human_visual_review_submit` 显式证据工具；F 的 `reference_mask_prepare`/`reference_mask_refine_prepare` 是显式 opt-in 的 CAS target 写入。Agentic 另提供 `scene_observe_get`、`design_stage_plan_get`、`critic_report_get`、`visual_evidence_bundle_get`、`session_get`、`checkpoint_get` read surface，以及 approval-gated `session_create_or_resume`、`checkpoint_prepare`、`checkpoint_restore_prepare`、`design_action_run_prepare`、`design_stage_run_prepare`、`design_composition_prepare`、`repair_apply_prepare`、`repair_apply_confirm`、`design_action_optimization_proposal_prepare`、`cross_view_promotion_confirm`。因此当前 source manifest 是 37 read + 30 write = 67，合同总数为 118；`design_stage_run_prepare` 只编排最多六个同阶段、彼此独立的 ActionRun，`design_composition_prepare` 记录 2–6 项线性独立动作的可审阅 proposal，并可在显式 `merge.mode=cumulative-program` 下校验完整 GeometryProgram@2 的父哈希链、批次通过后编译一个独立合并候选；`repair_apply_prepare` 只在 source/proposal/RepairIntent/quality/evidence 校验通过后写入 CAS apply intent，不确认版本；`repair_apply_confirm` 只消费单视图 fresh-approval 且重新验证 lineage 的 apply intent；`design_action_optimization_proposal_prepare` 只在 CADFit strict/non-regressing 时物化独立 review candidate；遇到质量门即停止；`cross_view_promotion_confirm` 只在完整多视图 evidence/hash/quality/approval 校验后尝试 promotion，正向成功尚未运行。除 `capabilities_get`、`runtime_status`、`doctor` 外，MCP session 必须先读 `ponytail-preflight@0.1.0`；该会话 policy 不新增模型或 Provider，也不改变 Runtime 唯一写者模型。固定 renderer、D/E AssetPack 与真实机器人质量边界保持原有描述；轮廓 target/camera/Rig/SDF/Part/candidate compare 的调用顺序和停止规则见 `docs/CODEX_SILHOUETTE_FIT_WORKFLOW.md` 与 `docs/CODEX_CONTOUR_SKILL_PACK.md`。这些 source Gate、Agentic projection、durable prepare/readback、bounded batch、cumulative merge prepare、Repair apply prepare/confirm、CADFit continuation 和 promotion boundary Gate 仍不等于用户图片 likeness、PBR、人评、packaged/live、Repair execution 或 360 PASS。
