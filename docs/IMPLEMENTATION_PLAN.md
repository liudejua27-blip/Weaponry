...
- `asset_store.py` 已从约 5210 行降至约 3272 行；connection、migration、object store、Repository/UoW、Job query/command/recovery、asset upload、library/version、Creative Recast 和同步 Create Weapon Provider workflow 已提取；
...
| R1 application services | 进行中 | Job query/command/recovery、Asset、Library、Creative Recast、Create Weapon services；generate/Patch/export 仍待提取 |
...
- 提取 legacy Job、Library、Asset Upload、Creative Recast 和同步 Create Weapon services；
- 将剩余 generate/Patch/provider/export workflows 移出 `asset_store.py`；
...
1. 在已提取同步 Create Weapon workflow 基础上，继续将 generate-3d、Patch 和 Unity export workflow 从 `asset_store.py` 提取为 application services。
