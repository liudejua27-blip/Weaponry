...
- 服务端 `weapon-concept-geometry/1.2` 已从版本绑定的内容寻址 GLB 计算 Mesh/Assembly Findings：包含未连接组件 triangle BVH/SAT/containment、精确 triangle provenance，以及已连接组件超过 `2 mm` 的保守 AABB 表面间隙；桌面 Finding 会聚焦并高亮双方节点与局部 triangle。对称/隐藏几何/密度/LOD 尚未完成；
...
| R5 Mesh/Assembly quality | C07 定位切片完成 | immutable GLB decode、indices/degenerate/normal/UV/topology/bounds、Connector alignment、已连接组件 AABB surface gap、未直连组件 triangle BVH/SAT/closed-mesh containment、triangle provenance、双节点/局部高亮、JobEvent/restart、desktop E2E |
...
已完成 C07 定位切片：索引、三角形数量、退化面、法线、UV0、开放/非流形边、清单 bounds、Connector `0.1 mm / 0.1°` 对齐；未直连组件先走世界 AABB/BVH broad phase，再走 triangle SAT narrow phase，封闭且无表面交叉时补 containment，并把确切 triangle index 与世界坐标写入 Finding。直接 Connector 相连的组件不进入穿插规则，但会检查世界 AABB 的保守表面间隙，超过 `2 mm` 生成 `assembly.connected_surface_gap`。Finding 点击会框选并高亮全部关联节点，叠加局部相交三角形。隐藏几何、密度、对称和 LOD 仍待实现。
...
5. 在已完成 triangle BVH/SAT/containment、已连接组件间隙和 Finding 局部定位基础上，补对称/隐藏几何/密度/LOD，并形成正式资产上的完整 C07 truth set。
