...
`npm run agent:m108-visual-benchmark-workbench-capture` 只在同一真实工作台、同一 renderer/canvas 内依次捕获四领域 iso + `cad_neutral` 视口 PNG，并写出 GLB/screenshot hash、环境、`preview_mode`、`xray` 和 renderer facts。最新真实捕获已验证四领域均是 `ready/glb_pbr`、`preview_mode=committed`、`xray=disabled`和单 WebGL context；`committed` 只表示当前非 ghost 视口，不是 Git 提交。它仍固定标记 `development_visual_audit_only`、`not_scored` 和 `human_benchmark_evidence=false`，只用于开发者发现问题；截图不是独立人工评分，不能把 M108 改为完成或解除 C105 阻塞。
