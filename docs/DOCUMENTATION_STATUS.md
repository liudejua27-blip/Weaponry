...
2026-07-15 用户进一步确认以“3D 机械设计系统”取代 HTML 六面拼接或单一 box 雕刻。G819、Q003、G820–G825 已完成：Extrude/Revolve/受限 Loft/Sweep 已消费 canonical 输入并进入真实 GLB readback；G824A–G824D 完成 Manifold provenance/readback、隔离 cancel/timeout、权威状态原子提升和 macOS/Windows packaged 证据；G825 只接入 ADR-0013 选定的 `manifold3d==3.5.2` 生产 handler，用有界封闭输入执行 union/subtract，并将节点、输入 hash、内核版本、表面/材质来源和结果 hash 回读为不可变 Feature History。失败返回稳定错误且不输出部分 GLB；它仍是受限概念级布尔，不是 B-Rep/工程 CAD。G826 为下一唯一 `ready`。当前 Agent Planner 尚未自动采用新几何语法，Alpha 仍显示三方向和受限三项外观轮换。
...
| 受限 ShapeProgram | 部分实现 | G3、G5、G801–G806、G819–G825、Q003；canonical Profile 可驱动 Extrude/Revolve/Sweep，ordered section set 可驱动受限 Loft；union/subtract 由唯一 Manifold Python handler 执行并回读不可变 Feature History 与 surface/material provenance | G826 的 edge finish/tangent/稳定 zone、自由曲面、精确 CAD、碰撞/运动学未实现；Planner 尚未自动使用新语法 |
...
| CAD 设计能力闭环 | 部分实现 | G819/Q003 统一运行与 readback 真值，G820 冻结 Profile/section，G821 生成 Extrude/Revolve，G822 生成 Loft，G823 生成受限 Sweep，G825 接入单一 Manifold CSG 与 Feature History；后续为 G826 → A003 → F025 → D005 → A004 → M108 → C105 → V003 → F026 → A005 → R007 → D006 | 完整表面事实、内部最佳候选、简洁布局、Skill、真实 PBR、Recipe、参考重建和新领域仍无 Gate |
