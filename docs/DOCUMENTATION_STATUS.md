...
2026-07-25 目标架构增量：ADR-0017 已把上传的 Codex 式设计工作区方案与 `img2threejs` commit `c9077d5e` 的细节清单、固定构建阶段、确定性优先视觉 Gate、多角度检查和有界修复方法合并。目标新增 `DesignWorkspace@1`、`MechanicalMorphologyProgram@1`、`VisualDetailInventory@1`、`DesignBuildLedger@1` 和 `VisualConvergenceReport@1`，但这些仍是目标合同，不是当前用户能力。当前唯一实现任务仍是 C111A：先以冻结 fixture 把参考图关键细节绑定到真实 Recipe/Shape/Material Zone/A005 输出并关闭机械臂黄金资产；之后才按 C112→C113→C114→C115→A006→Q004→E004 建立通用设计自由度。没有引入 img2threejs 依赖、Three.js 生成器、任意 JavaScript 或第二资产真值。

2026-07-26 当前状态增量：C111A iteration 70 exact root 为 10 Parts、9 connections、198 operations、96 outputs。双侧六段 Loft 臂甲、轮廓化 Revolve 腕罩/近端指节和连续倒角指身继续保持；基座三层与回转罩使用共享纵向硬边 Loft，连杆两端 collar 与关节 guard 重叠，缩短的六段甲片暴露真实暗色内壳凹槽，浅石墨/复合嵌条取代刀片状分缝。GLB 携带 `env_forgecad_room_studio_v2`。真实双档为 49,392-triangle preview 与 124,376-triangle/152-primitive production、1K 五通道 PBR，ShapeProgram SHA-256 `5b0040ea399b1938d7b97f7f5a3c0d9e5c2f8340862d9bb68d73071c20b8457d`，production GLB SHA-256 `af2bda0ff7c180970ef6eb1750ba3d6e88c062cfd16db8d28313c8914b66d037`。Inventory 为 24 verified / 3 unresolved / 0 critical，development program 已 sealed。iteration 47 的 release packaged 证据继续作为历史生命周期证明，不能替代 iteration 70 的新 confirm/export/restart；C111A 保持 `in_progress`、`formal_eligible=false`、`human_benchmark_evidence=false`，M108B 继续 blocked。

2026-07-23 当前状态增量：C110A/B/C 的合同、Rust Core 物化和 ChangeSet 编译入口已完成；C110D 已新增 actuator cover、cable guide、wrist tool mount、wrist gripper 四项受审 Recipe。真实 DeepSeek 的两次连续续作使用 Rust-owned `ActiveDesignSnapshot` 只读上下文，Provider 仅选择 `next_reviewed_attachment`，Rust 从 snapshot 的受审操作展开完整 `AssemblyDeltaProgram@1`；两次各自完成 preview GLB→confirm→一个不可变子版本写入，随后新进程恢复最终版本、导出 110,368-triangle production GLB 且无网络调用。脱敏的三份报告为 `output/deepseek-delta-acceptance-20260723-template-two-deltas-final.json`、`-second.json`、`-resume.json`。C110E `ArmGeometryFamily@1` 已完成：serial-chain 的连杆/关节/基座/腕部/末端/线缆/材质意图会在 Rust 中同时修改真实 ShapeProgram 与 AssemblyGraph，并生成 intent/changed-count/ShapeProgram hash 绑定；不同 link language 已由 Core 与 app-server 测试证明会产生不同几何指纹。C110F 已完成 Provider 稳定性最小闭环：`plan_complete_concept` 向 DeepSeek 使用紧凑投影、Rust 仍以完整 schema 最终校验；Action Loop 默认采用 256K 累计上下文/4K 单次输出预留，结构化 JSON 错误最多一次固定 repair，针对首次 synthesis 的错误 AssemblyDelta 与无效 ArmDesignIntent 另有最多两次固定 Product Tool recovery，所有重试仍受硬上限约束。真实 `live_arm_intent_20260722g` acceptance 已通过，`live_turn.arm_intent_bound=true`，证明真实 DeepSeek 已将中文机械臂描述送入 Rust `ArmDesignIntent@1` lowering；取消、local fail-closed、零 Snapshot/资产写入和脱敏报告也通过。现在证明的是一条受审 serial-chain 同资产连续编辑路径；仍不证明真实 DeepSeek 直接生成自由 GLB 创意质量、任意架构/拓扑/自由装配、图片级视觉或 M108B 4/5。性能风险仍是约 10 万三角形 showcase 在 build/readback/preview/export 中被多个受限 Python 子进程重复编译，下一步需做增量 artifact handle 或明确拆分交互预览与 production export。

2026-07-24 当前状态增量：初始机械臂 Turn 已收敛为“一次 Provider 创意计划 + Rust 固定完成五个后续阶段”，完整计数为 1 次 Provider request、6 次 Product Tool call。当前 C110G release bundle 的离线 packaged preview→confirm→同资产增量→production export→新进程恢复重新通过，0 网络、0 凭据读取；这证明自动唯一结果工程闭环，不证明真实 DeepSeek 已选择 parallel-link 或 M108B 视觉达标。所有 live DeepSeek acceptance 现在先验证稳定 macOS 应用身份；Rust Keychain backend 也在任何 read/write/delete 前用 Security.framework 验证 Apple anchor、Team OU、固定 bundle identifier 和严格签名。当前 ad-hoc bundle 和 0 个有效 codesigning identity 会在任何 Keychain API 前阻断，因此开发迭代不再触发密码框。当前源码 bundle 已覆盖 `/Applications/CAD 工作台.app` 且主二进制 hash 一致，Dock 不再打开旧构建。最终联网验收需先配置一个稳定本机 Apple Development 身份并通过 `desktop:macos-stable-identity-require`，再进行一次显式 Keychain 授权。
