...
2026-07-15 用户进一步确认以“3D 机械设计系统”取代 HTML 六面拼接或单一 box 雕刻。G819、Q003、G820–G826 已完成，仍是概念 Mesh/GLB，不是 B-Rep/工程 CAD。A003 已完成 Provider preflight、SSE 生命周期、取消、用量、稳定错误与禁止静默 fallback；F025 已完成 Agent/legacy 控制隔离；D005 已提供四领域各 4 个非工程 Style Token/比例配方。A004 现以 13 个代码所有、Schema 验证的 ForgeCAD Product Tool 建立单 Turn Action Loop，离线 Planner 与 DeepSeek 都能执行候选 build、真实 GLB readback、四视图、硬门和未保存 preview；DeepSeek thinking Tool Call 会在同一短生命周期续传 `reasoning_content`，但不会持久化。M108 进行中：当前源码 GLB 会嵌入并回读内置五通道视觉 PBR、真实 zone→material 映射和固定工作室环境；工作台仍只有一个 renderer，但 Agent blockout 在 GLB 可用时已优先解析该同源 GLB 并断言实际 PBR map 绑定，参数 ShapeProgram 只能作为明确标识的无 GLB 回退。锁定的 Khronos Validator 已对四领域原始 GLB 建立零 error/zero warning CI 门禁。glTF Transform core/extensions 已被锁定为评估依赖，实测其写出会重编号资源且改变 ForgeCAD readback，故只能作采用否决 Gate、不能接管导出；KTX2/BasisU 因未具备已验证的编码、视口解码/回退或 packaged Gate 而未采用。真实 arm64 packaged sidecar 的 PBR/readback、ChangeSet preview/confirm 后导出、undo/redo 恢复对应 PBR GLB、CSG 后导出和重启同 hash 已回归通过；四领域无评分审阅包和独立评审协议已可生成，但人工视觉基准仍未收集。当前 Planner 尚未自动采用新几何语法/Recipe 或只显示唯一最佳结果，Alpha 仍显示三方向和受限三项外观轮换。
...
| Agent 方向与后端 blockout | 部分实现 | G4、G807、G812、G813、G815、G817、G818、R006、A004；三方向稳定匹配四领域受限视觉变体，未保存候选可在同方向三项族中轮换；`quick_sketch`/`showcase` 有受限外观层。A004 在同一 Turn 内为一个候选执行真实 GLB readback/四视图/硬门，但桌面不再自动并发请求三张方向概念图 | 唯一最佳结果等待 V003；真实 Provider质量、自由外观生成与完整 PBR 等待后续，视觉层不等于真实材料、孔槽、散热或电气设计 |
...
| 可编辑参数声明与语义比例 | 已实现（受限） | G808–G811；D005 四领域 Style Token/语义槽/真实 binding+GLB provenance、preview/confirm/restart/undo/redo Gate | 自由参数与工程尺寸明确不在当前范围；Agent 自动选配方等待 A004 |
...
| CAD 设计能力闭环 | 部分实现 | G819/Q003、G820–G826、A003、F025、A004；D005 四领域语义比例已绑定真实参数/readback；后续为 M108 → C105 → V003 → F026 → A005 → R007 → D006 | 内部最佳候选、简洁布局、Skill、真实 PBR、组件 Recipe、参考重建和新领域仍无 Gate |
