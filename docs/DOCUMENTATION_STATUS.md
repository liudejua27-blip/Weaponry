...
2026-07-15 用户进一步确认以“3D 机械设计系统”取代 HTML 六面拼接或单一 box 雕刻。G819、Q003、G820–G824D 已完成：Extrude/Revolve/受限 Loft/Sweep 已消费 canonical 输入并进入真实 GLB readback；G824A–G824C 完成 Manifold provenance/readback、隔离 cancel/timeout、权威状态原子提升和 macOS packaged 预算/许可证证据；G824D 又由 GitHub Actions run `29383382978` 在真实 Windows x64 frozen executable 内通过同源证据并上传报告。ADR-0013 因而选择 Manifold Python 作为 G825 的唯一生产候选，G825 为 `ready`。默认 handler 和生产依赖仍未改变，不能把当前 box CSG 写成稳健通用布尔。当前 Agent Planner 尚未自动采用新几何语法，Alpha 仍显示三方向和受限三项外观轮换。
...
| 受限 ShapeProgram | 部分实现 | G3、G5、G801–G806、G819–G824D、Q003；canonical Profile 可驱动 Extrude/Revolve/Sweep，ordered section set 可驱动受限 Loft；CSG 候选的双平台 packaged/provenance/生命周期证据通过，ADR-0013 已选择 Python，但未接入默认 handler | G825 稳健生产 CSG、tangent/稳定 zone、自由曲面、精确 CAD、碰撞/运动学未实现；Planner 尚未自动使用新语法 |
