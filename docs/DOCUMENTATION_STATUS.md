...
2026-07-15 用户进一步确认以“3D 机械设计系统”取代 HTML 六面拼接或单一 box 雕刻。G819、Q003、G820–G826 已完成：Extrude/Revolve/受限 Loft/Sweep 已消费 canonical 输入并进入真实 GLB readback；G824A–G824D 完成 Manifold provenance/readback、隔离 cancel/timeout、权威状态原子提升和 macOS/Windows packaged 证据；G825 只接入 ADR-0013 选定的 `manifold3d==3.5.2` 生产 handler并回读不可变 Feature History；G826 为所有主要输出补齐受控 edge finish、normal/UV0/tangent 和稳定 face/source-face→part/zone GLB 事实，损坏、退化、空/重叠 zone 或预算失败均拒绝。它们仍是概念 Mesh/GLB，不是 B-Rep/工程 CAD，也没有完整纹理。A003 为下一唯一 `ready`。当前 Agent Planner 尚未自动采用新几何语法，Alpha 仍显示三方向和受限三项外观轮换。
...
| CAD 设计能力闭环 | 部分实现 | G819/Q003 统一运行与 readback 真值，G820 冻结 Profile/section，G821 生成 Extrude/Revolve，G822 生成 Loft，G823 生成受限 Sweep，G825 接入单一 Manifold CSG 与 Feature History，G826 建立完整几何侧表面事实；后续为 A003 → F025 → D005 → A004 → M108 → C105 → V003 → F026 → A005 → R007 → D006 | 内部最佳候选、简洁布局、Skill、真实 PBR、Recipe、参考重建和新领域仍无 Gate |
