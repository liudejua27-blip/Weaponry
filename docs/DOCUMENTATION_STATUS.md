...
| CAD 设计能力闭环 | 部分实现 | G819/Q003、G820–G826、A003、F025、A004；D005 四领域语义比例已绑定真实参数/readback；M108 当前 v3 128×128 材质专属五通道纹理、role/material、实际使用扩展、受限 bevel 和 320 mm 视觉 UV 重复已进入源码 PBR/readback Gate，v1/v2 只读兼容历史 GLB；G826 锁定封闭 primitive 外向绕序与非退化三角。最新四领域真实工作台捕获已验证 `ready/glb_pbr`、`committed`、xray 关闭、实时环境 recipe hash、PBR 颜色空间、520 mm 展示尺度、GPU 预算和单 context，但它仍是无评分开发审计。本轮 tracked macOS arm64 sidecar 已从最终源码重建，require-ready、packaged sidecar Alpha 和 Tauri check 通过，并覆盖 v3 PBR readback、CSG、undo/redo、重启和 `provider_calls=0`；packaged Tauri 因当前 ForgeCAD Agent 占用 8000 端口未重复运行。仍等待独立人工视觉基准；通过后依次为 C105 → V003 → F026 → A005 → R007 → D006 | 独立视觉达标、内部最佳候选、简洁布局、Skill、组件 Recipe、参考重建和新领域仍无完整 Gate |
