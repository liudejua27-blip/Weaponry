...
版本：2026-08-13
状态：模块权责文档；描述当前已实现边界与 ADR-0026 目标模块。Agentic observe/plan projection、嵌套只读 projection conformance 与受批准的 durable session/checkpoint/RepairIntent prepare/readback 已进入 Runtime/MCP/Viewer；durable/reference/DesignSpec 完整 producer、单动作 orchestrator 和 Repair 应用仍未进入当前边界。
