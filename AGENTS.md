...
4. MCP006 已完成其历史 44 个 typed contracts、十个独立 first-party declarative Bundle、Registry 只读暴露、DAG/单位/finite/预算/hash/license/SBOM/provenance 负向 Gate；MCP010B 当前源码在此基础上新增 8 个合同（`GeometryProgram@2`、`OperatorCatalog@1`、两个 authoring hash 合同、`ArtifactReadback@2`、`GeometryPrepareResult@2`、`GeometryQualityReport@2`、`GeometryCandidateEvidence@1`），共 52 个；MCP010F 轮廓求解 slice 又新增 Rig、Fit、PartFit、CandidateCompare 五个合同及 Runtime-owned Rig hash request/result 两个合同，并补充 CameraCalibrationRef/target binding 合同，共 78 个。当前 source-focused Gate 已通过；已安装 Dev.app receipt 仍是历史 cohort，不能把它冒充为本轮 package/live Desktop 证据；
