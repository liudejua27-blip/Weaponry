...
`FGC-VP201`–`FGC-VP204`、`FGC-E005-R1`、`FGC-U001`、文档清理任务 `FGC-U001A`、`FGC-U002`、`FGC-U003` 与 Provider 主权切片 `FGC-U004A` 已完成；E005-R2/R3 已形成的 sealed reference、视觉 patch、预算、恢复、PBR 和 receipt Core 保留为通用链可复用底座，但真实四模态输入、完整阶段计时和正式真人门仍未完成。当前唯一 `in_progress` 是 `FGC-U004`：扩展 DeepSeek/千问驱动的受限设计语言、本地 deformable/procedural/hybrid 表示和 Appearance Compiler。旧 `mesh_seed.generic_v1` 只作 schema 兼容并保持 unavailable。2026-07-31 新增开发期离线 `procedural.generic_visual_exterior_v1`：车辆、家具、建筑、植物、动物、角色、机械设备、虚构游戏道具及未知对象均可进入 Rust 校验的轻量外观代理；它不是照片级重建，也不替代各类别正式质量门。E005 后续工作降为 procedural hard-surface 回归子基准。
